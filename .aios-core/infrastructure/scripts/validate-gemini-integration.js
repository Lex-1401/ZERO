...
function validateGeminiIntegration(options = {}) {
  ...
  const resolved = {
    ...
    extensionDir:
      options.extensionDir || path.join(projectRoot, "packages", "gemini-aios-extension"),
    sourceAgentsDir:
  ...
  if (!commandFiles.includes("aios-menu.toml")) {
    errors.push(
      `Missing Gemini command file: ${path.relative(resolved.projectRoot, path.join(resolved.commandsDir, "aios-menu.toml"))}`,
    );
  }
