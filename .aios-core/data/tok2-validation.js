...
function run() {
  ...
  check(4, "Tier 3 tools deferred via best available strategy", () => {
    const caps = JSON.parse(
      fs.readFileSync(path.join(PROJECT_ROOT, ".aios", "runtime-capabilities.json"), "utf8"),
    );
    return (
  ...
  check(7, "Search accuracy validated (7/7 test queries pass)", () => {
    const { validate } = require(
      path.join(PROJECT_ROOT, ".aios-core", "data", "tool-search-validation.js"),
    );
    return validate();
  });
  ...
  check(9, "Essential MCP servers defined in tool-registry.yaml", () => {
    const registry = fs.readFileSync(
      path.join(PROJECT_ROOT, ".aios-core", "data", "tool-registry.yaml"),
      "utf8",
    );
  ...
  check(10, "Non-essential servers can be re-enabled per-session", () => {
    const mod = fs.readFileSync(
      path.join(PROJECT_ROOT, ".aios-core", "data", "mcp-discipline.js"),
      "utf8",
    );
