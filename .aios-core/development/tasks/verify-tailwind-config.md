# Verify Tailwind v4 Configuration & Utility Health

**Purpose**: Review Tailwind CSS v4 setup to guarantee @theme layering, content scanning, utility hygiene, and performance baselines are correct.
...
7. **Report & Remediation**
   - Summarize findings (pass/warn/fail) in `docs/reports/tailwind-verification.md`
   - Provide prioritized action list (tokens to add, utilities to remove, config fixes)
   - Update `.state.yaml` with verification timestamp, benchmark data, outstanding actions

## Output

- Verification report (`docs/reports/tailwind-verification.md`)
...
- [ ] `.state.yaml` updated (`tailwind_theme_validated: true/false`) and verification timestamp logged

## Tools & Commands

- Tailwind CLI build: `npx tailwindcss -i ./app.css -o ./dist.css --watch`
- Utility verification: `npx @tailwindcss/oxide --analyze`
...
- Encourage automated linting (ESLint + prettier-plugin-tailwindcss) post-verification
