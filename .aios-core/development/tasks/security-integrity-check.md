# Task: Security Integrity Check

**Purpose**: Comprehensive database security and quality check (RLS coverage, schema design, full system)

**Elicit**: true

**Consolidated From (Story 6.1.2.3):**

- `db-rls-integrity.md` - RLS policy coverage checking
- `db-schema-integrity.md` - Schema design quality validation
...
```yaml
post-conditions:
  - [ ] Scan completed; issues reported; no scan errors
    tipo: post-condition
    blocker: true
    validação: |
      Verify scan completed; issues reported; no scan errors
    error_message: "Post-condition failed: Scan completed; issues reported; no scan errors"
```
...
```yaml
acceptance-criteria:
  - [ ] No critical issues; all checks passed
    tipo: acceptance-criterion
    blocker: true
    validação: |
      Assert no critical issues; all checks passed
    error_message: "Acceptance criterion not met: No critical issues; all checks passed"
```
...
- **Tool:** security-scanner
  - **Purpose:** Static security analysis and issue detection
  - **Source:** npm: eslint-plugin-security or similar

- **Tool:** dependency-checker
  - **Purpose:** Check for problematic dependencies
  - **Source:** npm verification or snyk
...
2. **Error:** Critical Issue Detected
   - **Cause:** High-severity security issue found
   - **Resolution:** Review report, apply patches
...
**Prompt user to select check scope:**

```
Select security verification scope:

1. **rls** - RLS policy coverage only (quick)
2. **schema** - Schema design quality only (quick)
3. **full** - Complete security verification (comprehensive)
...
\echo '=== RLS Coverage Verification ==='
\echo ''
...
```bash
psql "$SUPABASE_DB_URL" -v ON_ERROR_STOP=1 <<'SQL'
\echo '=== Schema Design Quality Verification ==='
\echo ''
...
**Executes:** Both RLS verification + Schema verification sequentially
...
```
=== Schema Design Quality Verification ===
...
- `*policy-apply {table} {mode}` - Install RLS policies after verification
...
**Note:** This consolidated task replaces `db-rls-integrity.md` and `db-schema-integrity.md` (deprecated in v3.0)
