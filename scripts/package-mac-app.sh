...
# Build and bundle ZERO into a minimal .app we can open.
# Outputs to dist/ZERO.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ZERO.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Zero"
BUNDLE_ID="${BUNDLE_ID:-com.zero.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/zero/zero/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ZERO/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ZEROBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ZEROGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ZERO"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ZERO"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ZERO"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ZERO" 2>/dev/null || true
...
cp -R "$ROOT_DIR/apps/macos/Sources/ZERO/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ZEROKit resources"
ZEROKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ZEROKit_ZEROKit.bundle"
if [ -d "$ZEROKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ZEROKit_ZEROKit.bundle"
  cp -R "$ZEROKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ZEROKit_ZEROKit.bundle"
else
  echo "WARN: ZEROKit resource bundle not found at $ZEROKIT_BUNDLE (continuing)" >&2
fi
...
rm -f "$ROOT_DIR/dist/"*.dmg "$ROOT_DIR/dist/"*.zip

echo "📦 Embedding Gateway assets"
mkdir -p "$APP_ROOT/Contents/Resources/dist"
# Only copy if dist exists and is not the app itself (recursion check)
if [ -d "$ROOT_DIR/dist" ]; then
  rsync -a --exclude="ZERO.app" "$ROOT_DIR/dist/" "$APP_ROOT/Contents/Resources/dist/"
fi
cp "$ROOT_DIR/package.json" "$APP_ROOT/Contents/Resources/"

# Embed native dependencies that are tricky to resolve globally
mkdir -p "$APP_ROOT/Contents/Resources/node_modules/@zero"
if [ -d "$ROOT_DIR/node_modules/@zero/ratchet" ]; then
  cp -R "$ROOT_DIR/node_modules/@zero/ratchet" "$APP_ROOT/Contents/Resources/node_modules/@zero/"
fi

echo "⏹  Stopping any running ZERO"
killall -q ZERO 2>/dev/null || true
