...
#
# Suggested cron: */30 * * * * /home/admin/zero/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ZERO notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., zero-alerts)
...
STATE_FILE="$HOME/.zero/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ZERO if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use zero
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ZERO to $NOTIFY_PHONE..."
            zero send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ZERO is down. Run: ssh l36 '~/zero/scripts/mobile-reauth.sh'" "urgent"
    exit 1
