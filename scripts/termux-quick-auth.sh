...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/zero/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ZERO Re-Auth" -c "After getting key, run: ssh $SERVER '~/zero/scripts/mobile-reauth.sh'" --id zero-auth
        ;;
    *)
