#!/usr/bin/env bash
# Reset ZERO like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.
...
WAIT_FOR_LOCK=0
LOG_PATH="${ZERO_RESTART_LOG:-/tmp/zero-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ZERO_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.zero/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ZERO_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      ...
      log "  rm ~/.zero/disable-launchagent"
      log ""
...
kill_all_zero() {
  for _ in {1..10}; do
    ...
    if ! pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
       ...
       && ! pgrep -x "ZERO" >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.3
  done
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/com.zero.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing ZERO instances"
kill_all_zero
stop_launch_agent
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/ZERO.app" ]]; then
    APP_BUNDLE="/Applications/ZERO.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/ZERO.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/ZERO.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/ZERO.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set ZERO_APP_BUNDLE to your installed ZERO.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".zero", "zero.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: ZERO is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/com.zero.gateway.plist' | head -n 40 || true"
fi
