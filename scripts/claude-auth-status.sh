...
# Checks both Claude Code and ZERO auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
ZERO_AUTH="$HOME/.zero/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    zero models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    zero_status=$(check_zero_auth 2>/dev/null || true)

    claude_expires=0
    zero_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        zero_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        zero_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ZERO_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            zero: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$zero_status" == EXPIRED* ]] || [[ "$zero_status" == MISSING* ]]; then
        echo "ZERO_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$zero_status" == EXPIRING* ]]; then
        echo "ZERO_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "ZERO Auth (~/.zero/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$ZERO_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$ZERO_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'zero doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active zero >/dev/null 2>&1; then
    echo -e "ZERO service: ${GREEN}running${NC}"
else
    echo -e "ZERO service: ${RED}NOT running${NC}"
fi
