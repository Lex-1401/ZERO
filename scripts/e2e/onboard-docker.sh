...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/zero-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/zero-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/zero-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/zero"
    config_path="$HOME/.zero/zero.json"
    sessions_dir="$HOME/.zero/agents/main/sessions"
