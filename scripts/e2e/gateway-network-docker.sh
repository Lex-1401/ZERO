...
IMAGE_NAME="zero-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="zero-net-e2e-$$"
GW_NAME="zero-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ZERO_GATEWAY_TOKEN=$TOKEN" \
	  -e "ZERO_SKIP_CHANNELS=1" \
	  -e "ZERO_SKIP_GMAIL_WATCHER=1" \
	  -e "ZERO_SKIP_CRON=1" \
	  -e "ZERO_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
