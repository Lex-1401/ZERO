...
IMAGE_NAME="zero-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/zero-bin:$PATH"
  mkdir -p /tmp/zero-bin

  cat > /tmp/zero-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/zero-bin/systemctl

  cat > /tmp/zero-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm_bin="/tmp/npm-prefix/bin/zero"
  npm_entry="/tmp/npm-prefix/lib/node_modules/zero/dist/entry.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/zero-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/zero-gateway.service"
    if [ ! -f "$unit_path" ]; then
