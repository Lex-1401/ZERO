...
ZIP=${1:?"Usage: $0 ZERO-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/zero/zero/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^ZERO-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/zero/zero/releases/download/v${VERSION}/"}
