...
BASE_IMAGE="${BASE_IMAGE:-zero-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-zero-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=zero.sandbox=1)
NOTE
