...
## 6. Workflow Anti-Vibe Coding (Protocolo de Precisão)

Este workflow é mandatório para qualquer nova funcionalidade ou refatoração complexa. O descumprimento bloqueia o merge.

Phase: Research (Pesquisa)

Ação: Localizar arquivos relevantes, documentações internas e identificar padrões de implementação existentes.

Artefato: Criar PRD.md na pasta .agent/plans/ contendo os achados técnicos.

Phase: Spec (Especificação Tática)

Ação: Com base no PRD.md, listar todos os arquivos impactados e descrever cada modificação necessária.

Artefato: Criar Spec.md na pasta .agent/plans/ com o plano de implementação detalhado.

Phase: Code (Implementação)

Ação: Implementar rigorosamente o que foi definido na Spec.md.

Phase: Inspec (Auditoria)

Ação: Realizar auditoria cruzada (Cross-Audit) da implementação contra a Spec.md e o governance.md.
