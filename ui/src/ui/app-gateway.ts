...
export function connectGateway(host: GatewayHost) {
  ...
  host.client = new GatewayBrowserClient({
    ...
    onGap: ({ expected, received }) => {
      host.lastError = `lacuna de eventos detectada (esperada seq ${expected}, recebida ${received}); atualização recomendada`;
    },
  });
  host.client.start();
}
...
function handleGatewayEventUnsafe(host: GatewayHost, evt: GatewayEventFrame) {
  ...
  if (evt.event === "cron" && host.tab === "cron") {
    void syncCronInternal(host as unknown as Parameters<typeof syncCronInternal>[0]);
  }
  ...
  if (evt.event === "exec.approval.requested") {
    ...
    if (entry) {
      ...
      // Pilot Mode (Autopilot): Automatically allow command execution
      if (host.settings.autopilot) {
        void (host as unknown as ZEROApp).handleExecApprovalDecision("allow-once");
      }
