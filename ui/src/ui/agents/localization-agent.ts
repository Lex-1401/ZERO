...
export class LocalizationAgent {
  ...
  private async processChunk(state: AppViewState, keys: string[], targetLang: string) {
    ...
    console.log(
      `[LocalizationAgent] Enviando chunk de ${pendingKeys.length} chaves para [${targetLang}]...`,
    );
  ...
  async validateTranslation(state: AppViewState, key: string, lang: string, approved: boolean) {
    console.log(
      `[LocalizationAgent] Persistindo feedback: ${key} [${lang}] -> ${approved ? "APROVADO" : "REJEITADO"}`,
    );
  ...
  async runStressTest(state: AppViewState, targetLang: string) {
    ...
    const keysToTest = [
      ...
      "config.action.apply",
      "config.diff.title",
      "config.diag.critical",
      "config.appearance.theme.desc",
      "config.section.agents.label",
      "config.section.auth.label",
      "config.section.ui.label",
      "config.field.apiKey",
      "config.field.baseUrl",
