...
export function formatCronState(job: CronJob) {
  ...
  return `${status} · próx. ${next} · últ. ${last}`;
}

export function formatCronSchedule(job: CronJob) {
  const s = job.schedule;
  if (s.kind === "at") return `Em ${formatMs(s.atMs)}`;
  if (s.kind === "every") return `A cada ${formatDurationMs(s.everyMs)}`;
  return `Cron ${s.expr}${s.tz ? ` (${s.tz})` : ""}`;
}

export function formatCronPayload(job: CronJob) {
  const p = job.payload;
  if (p.kind === "systemEvent") return `Sistema: ${p.text}`;
  return `Agente: ${p.message}`;
}
