...
beforeEach(() => {
  ...
  window.__ZERO_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/zero/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/zero");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/zero/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ZERO_CONTROL_UI_BASE_PATH__ = "/zero";
    const app = mountApp("/zero/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/zero");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/zero/sessions");
  });
