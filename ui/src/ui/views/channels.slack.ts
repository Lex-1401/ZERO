...
export function renderSlackCard(params: {
  ...
  const { props, slack } = params;

  return html`
    <div class="animate-fade-in">
        <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 24px;">
            <div>
                <div class="section-title" style="margin: 0;">Slack Protocol</div>
                <div style="font-size: 13px; color: var(--text-muted); margin-top: 4px;">Integração via Socket Mode para workspaces corporativos.</div>
            </div>
                ...
                <div class="group-list">
                    <div class="group-item">
                        <div class="group-label">
                            ...
                            <div class="group-desc">${slack?.running ? "Operacional" : "Parado"}</div>
                        </div>
                        <div class="group-content">
                            <div class="status-orb ${slack?.running ? "success" : "danger"}"></div>
                        </div>
                    </div>
                    <div class="group-item">
                        <div class="group-label"><div class="group-title">Configurado</div></div>
                        <div class="group-content"><div class="badge ${slack?.configured ? "active" : ""}">${slack?.configured ? "SIM" : "NÃO"}</div></div>
                    </div>
                    <div class="group-item">
                        <div class="group-label">
                            <div class="group-title">Último Início</div>
                            <div class="group-desc">${slack?.lastStartAt ? formatAgo(slack.lastStartAt) : "Nunca iniciado."}</div>
                        </div>
                    </div>
                    <div class="group-item">
                        <div class="group-label">
                            <div class="group-title">Última Sondagem</div>
                            <div class="group-desc">${slack?.lastProbeAt ? formatAgo(slack.lastProbeAt) : "Sem dados recentes."}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="group-list">
                    <div class="group-item" style="flex-direction: column; align-items: flex-start; gap: 12px; padding: 20px;">
                        <div style="display: flex; align-items: center; gap: 12px; width: 100%;">
                            <div style="width: 40px; height: 40px; border-radius: 50%; background: #4A154B; display: flex; align-items: center; justify-content: center; color: white;">
                                ${icons.plug}
                            </div>
                            <div>
                                <div style="font-weight: 600; font-size: 13px; color: var(--text-main);">Socket Mode</div>
                                <div style="font-size: 11px; color: var(--text-dim);">Slack RTM/Socket API</div>
                            </div>
                        </div>
                        ...
                        ${slack?.probe ? html`
                            ...
                            <div style="width: 100%;">
                                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                                    ...
                                    <span class="badge ${slack.probe.ok ? "active" : "danger"}">${slack.probe.ok ? "OK" : "FALHA"}</span>
                                </div>
                                <div style="font-family: var(--font-mono); font-size: 10px; color: var(--text-muted); background: rgba(0,0,0,0.2); padding: 8px; border-radius: 6px;">
                                    ${slack.probe.status ?? "Status nominal"}
