...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; label: string }> = [
  ...
  { value: "allowlist", label: "Lista de Permissões" },
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px; align-items: start;">
            ...
            <div>
                <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 8px;">
                    <div class="section-title" style="margin: 0;">Nós de Computação</div>
                ...
                <div class="group-list">
                    ${props.nodes.length === 0 ? html`
                        <div class="group-item" style="padding: 40px; justify-content: center; color: var(--text-dim);">Nenhum nó de computação associado.</div>
                    ` : props.nodes.map(n => renderNode(n))}
                </div>

                <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 8px; margin-top: 40px;">
                    <div class="section-title" style="margin: 0;">Vínculos de Execução</div>
                    <button class="btn primary btn--sm" ?disabled=${bindingState.disabled || !bindingState.configDirty} @click=${bindingState.onSave}>
                        ${bindingState.configSaving ? "Salvando…" : "Salvar"}
                    </button>
                </div>
                <div class="group-list">
                    <div class="group-item">
                        <div class="group-label">
                            <div class="group-title">Vínculo Global Padrão</div>
                            <div class="group-desc">Host utilizado por qualquer agente não substituído.</div>
                        </div>
                        <div class="group-content">
                            <select class="select-native" style="width: 240px;" @change=${(e: any) => bindingState.onBindDefault(e.target.value || null)}>
                                <option value="" .selected=${!bindingState.defaultBinding}>Qualquer Nó</option>
            ...
            <div>
                <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 8px;">
                    <div class="section-title" style="margin: 0;">Segurança e Políticas</div>
                    <div style="display: flex; gap: 8px;">
                        <button class="btn btn--sm" @click=${approvalsState.onLoad}>Sincronizar</button>
                        <button class="btn primary btn--sm" ?disabled=${!approvalsState.dirty} @click=${approvalsState.onSave}>Salvar Políticas</button>
                    </div>
                </div>
                <div class="group-list">
                     <div class="group-item">
                        <div class="group-label"><div class="group-title">Alvo da Política</div></div>
                        <div class="group-content" style="gap: 8px;">
                            <select class="select-native" style="width: 120px;" .value=${approvalsState.target} @change=${(e: any) => approvalsState.onSelectTarget(e.target.value, approvalsState.targetNodeId)}>
                                ...
                                <option value="node">Nó Local</option>
                            </select>
                ...
                <div class="section-title">Escopo de Permissões</div>
                <div style="display: flex; gap: 4px; flex-wrap: wrap; margin-bottom: 16px;">
                    <button class="btn btn--sm ${approvalsState.selectedScope === EXEC_APPROVALS_DEFAULT_SCOPE ? "primary" : ""}" @click=${() => approvalsState.onSelectScope(EXEC_APPROVALS_DEFAULT_SCOPE)}>Padrões Global</button>
                ...
                <div class="group-list">
                    <div class="group-item">
                        <div class="group-label"><div class="group-title">Nível de Segurança</div></div>
                        <div class="group-content">
                ...
                <div class="section-title">Segurança de Hardware</div>
                <div class="group-list">
                    ${(props.devicesList?.pending || []).length === 0 && (props.devicesList?.paired || []).length === 0 ? html`
                         <div class="group-item" style="padding: 40px; justify-content: center; color: var(--text-dim);">Sem dispositivos registrados.</div>
                    ` : nothing}
                    ${(props.devicesList?.pending || []).map(req => html`
                        <div class="group-item">
                            <div class="group-label">
                                <div class="group-title">${req.displayName || req.deviceId}</div>
                                <div class="group-desc">Solicitação Pendente • ${req.remoteIp || "Local"}</div>
                            </div>
                            <div class="group-content">
                                <button class="btn btn--sm primary" @click=${() => props.onDeviceApprove(req.requestId)}>Aprovar</button>
                                <button class="btn btn--sm danger" @click=${() => props.onDeviceReject(req.requestId)}>Negar</button>
                            </div>
                        </div>
                    `)}
                    ${(props.devicesList?.paired || []).map(dev => html`
                        <div class="group-item">
                            <div class="group-label">
                                ...
                                <div class="group-desc">Pareado • ${dev.platform || "Nativo"}</div>
                            </div>
                            <div class="group-content">
                                <button class="btn btn--sm danger" @click=${() => props.onDeviceRevoke(dev.deviceId, dev.roles[0])}>Revogar</button>
                            </div>
                        </div>
                    `)}
                </div>
  ...
  return html`
    <div class="group-item">
      <div class="group-label">
        ...
        <div class="group-desc">${node.id}</div>
      ...
      <div class="group-content" style="flex-direction: column; align-items: flex-end; gap: 4px;">
        <div class="badge active">${status}</div>
        <div style="font-size: 11px; color: var(--text-dim);">${node.kind}</div>
      </div>
    </div>
  `;
}
