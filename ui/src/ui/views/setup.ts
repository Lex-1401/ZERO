...
export function renderSetup(props: {
  ...
  if (props.step === "persona") {
    return html`
            <div style="max-width: 900px; width: 100%; display: grid; grid-template-columns: 1fr 1.5fr; background: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: 20px; box-shadow: var(--shadow-floating); overflow: hidden; animation: card-appear 0.6s var(--ease-premium);">
                
                <!-- Sidebar -->
                <div style="background: rgba(var(--accent-blue-rgb), 0.05); padding: 40px; display: flex; flex-direction: column; justify-content: space-between; border-right: 1px solid var(--border-subtle);">
                     <div>
                        <div style="margin-bottom: 32px; display: flex; justify-content: center;">
                            <img src="/zerologo_without_background.png" alt="ZERO Logo" style="height: 64px; width: auto; filter: drop-shadow(0 0 20px rgba(var(--accent-blue-rgb), 0.3));" />
                        </div>
                ...
                <!-- Content -->
                <div style="padding: 40px; overflow-y: auto; background: var(--bg-surface);">
                    <div style="display: grid; grid-template-columns: 1fr; gap: 16px;">
                          ${[
                            ...
                            (p) => html`
                            <div class="hover-lift" style="cursor: pointer; padding: 20px; background: rgba(255,255,255,0.02); border: 1px solid var(--border-subtle); border-radius: 12px; position: relative;" 
                                 @click=${() => props.onPersonaSelect(p.id)}
                            >
                                <div style="display: flex; gap: 16px; align-items: center; width: 100%;">
                                    <div style="width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; color: ${p.color};">
                                    ...
                                    <div style="color: var(--text-dim); opacity: 0.5;">${icons.arrowRight}</div>
                                </div>
                            </div>
                        `,
                          )}
