...
function formatToolOutput(value: unknown): string | null {
  ...
  return `${truncated.text}\n\n… truncado (${truncated.total} caracteres, mostrando os primeiros ${truncated.text.length}).`;
}
...
/**
 * Maps an agent event to a 'thinking' state, inspired by Crabwalk's parser.ts
 */
export function isThinkingEvent(payload: AgentEventPayload): boolean {
  const stream = payload.stream;
  const data = payload.data ?? {};
  const phase = typeof data.phase === "string" ? data.phase : "";

  // Lifecycle start usually means the agent is beginning to think/process
  if (stream === "lifecycle" && phase === "start") return true;

  // Assistant stream active means it's still generating/thinking
  if (stream === "assistant") return true;

  // Tool use/result implies active processing
  if (stream === "tool" && (phase === "start" || phase === "update")) return true;
