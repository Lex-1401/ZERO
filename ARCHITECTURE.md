...
1. **Kernel Nativo (Rust-Core)**: O coração performático do sistema, focado em segurança, telemetria e tarefas de altíssima latência-zero.
...
4. **Resolução de RAG**: O sistema de memória (`src/memory/`) busca instâncias passadas e vetores para formar o `system_prompt` atualizado.
