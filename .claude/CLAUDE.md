...
## 🔄 PROTOCOLO DE EXECUÇÃO ANTI-VIBE (MANDATÓRIO)

Sempre que uma tarefa for iniciada, siga rigorosamente estes passos. **Solicite a limpeza da janela de contexto (clear context) após cada artefato gerado.**

1. **[RESEARCH] - Pesquisa & Descoberta**
   - **Ação:** Localizar arquivos relevantes, documentações internas e identificar padrões de implementação no projeto ZERO.
   - **Output:** Criar `.agent/plans/PRD.md`.
   - **Gatilho:** "PRD criado. Favor limpar janela de contexto para iniciar a Spec."

2. **[SPEC] - Especificação Tática**
   - **Ação:** Utilizar o PRD para listar todos os arquivos impactados e detalhar as modificações (o que e onde mudar).
   - **Output:** Criar `.agent/plans/Spec.md`.
   - **Gatilho:** "Spec criada. Favor limpar janela de contexto para iniciar o Coding."

3. **[CODE] - Implementação Técnica**
   - **Ação:** Implementar rigorosamente o plano contido na `Spec.md`.
   - **Regra:** Proibido improvisar fora do que foi validado na fase de Spec.

4. **[INSPEC] - Auditoria Cruzada**
   - **Ação:** Realizar auditoria da funcionalidade contra a Spec e os padrões do `governance.md` (Segurança Sentinel e Modularidade Atômica).
