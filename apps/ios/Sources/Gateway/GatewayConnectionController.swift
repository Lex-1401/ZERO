...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [ZeroCapability.canvas.rawValue, ZeroCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(ZeroCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(ZeroCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = ZeroLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(ZeroCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            ZeroSystemCommand.notify.rawValue,
            ZeroSystemCommand.which.rawValue,
            ZeroSystemCommand.run.rawValue,
            ZeroSystemCommand.execApprovalsGet.rawValue,
            ZeroSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(ZeroCapability.camera.rawValue) {
            commands.append(ZeroCameraCommand.list.rawValue)
            commands.append(ZeroCameraCommand.snap.rawValue)
            commands.append(ZeroCameraCommand.clip.rawValue)
        }
        if caps.contains(ZeroCapability.location.rawValue) {
            commands.append(ZeroLocationCommand.get.rawValue)
        }
