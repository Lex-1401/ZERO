...
struct IOSGatewayChatTransport: ZeroChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ZeroChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ZeroChatSessionsListResponse.self, from: res)
    }
    ...
    func patchSession(sessionKey: String, model: String?, thinkingLevel: String?) async throws {
        ...
        let params = Params(key: sessionKey, model: model, thinkingLevel: thinkingLevel)
    ...
    func requestHistory(sessionKey: String) async throws -> ZeroChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ZeroChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ZeroChatAttachmentPayload]) async throws -> ZeroChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ZeroChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(ZeroChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(ZeroGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ZeroGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ZeroChatEventPayload.self)
                        {
