...
private final class TestChatTransport: @unchecked Sendable, ZeroChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ZeroChatHistoryPayload]
    private let sessionsResponses: [ZeroChatSessionsListResponse]

    private let stream: AsyncStream<ZeroChatTransportEvent>
    private let continuation: AsyncStream<ZeroChatTransportEvent>.Continuation

    init(
        historyResponses: [ZeroChatHistoryPayload],
        sessionsResponses: [ZeroChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ZeroChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ZeroChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ZeroChatHistoryPayload(
        ...
        attachments _: [ZeroChatAttachmentPayload]) async throws -> ZeroChatSendResponse
    {
        ...
        return ZeroChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ZeroChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ZeroChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ZeroChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ZeroChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ZeroChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ZeroChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ZeroChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ZeroChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ZeroChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ZeroChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
