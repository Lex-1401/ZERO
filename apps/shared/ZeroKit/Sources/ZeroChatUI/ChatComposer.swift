...
@MainActor
struct ZeroChatComposer: View {
    @Bindable var viewModel: ZeroChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ZeroChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ZeroChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        Picker(
            "Modelo",
            selection: Binding(
                get: { self.viewModel.currentModel ?? "" },
                set: { self.viewModel.setModel($0.isEmpty ? nil : $0) }))
        {
            Text("Modelo").tag("")
            Text("Gemini Pro").tag("google/gemini-3-pro")
            Text("Claude Opus").tag("anthropic/claude-opus-4-5")
            Text("GPT-5.2").tag("openai/gpt-5.2")
            Text("Llama 3.3").tag("ollama/llama3.3")
        }
        .labelsHidden()
        .pickerStyle(.menu)
        .controlSize(.small)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ZeroPendingAttachment.id)
                { (att: ZeroPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ZeroPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ZeroChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ZeroChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ZeroChatTheme.subtleCard)
        .clipShape(Capsule())
    }
