...
@MainActor
public struct ZeroChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: ZeroChatViewModel
    ...
    private var visibleMessages: [ZeroChatMessage] {
        let base: [ZeroChatMessage]
    ...
    private func mergeToolResults(in messages: [ZeroChatMessage]) -> [ZeroChatMessage] {
    ...
    private func isToolResultMessage(_ message: ZeroChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: ZeroChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: ZeroChatMessage) -> String {
...
private struct ChatNoticeCard: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ZStack {
                ...
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [self.tint.opacity(0.8), self.tint],
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        )
                    )
                    .frame(width: 56, height: 56)
                    .shadow(color: self.tint.opacity(0.3), radius: 8, x: 0, y: 4)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 28, style: .continuous)
                .stroke(
                    LinearGradient(colors: [.white.opacity(0.35), .clear, .white.opacity(0.15)], startPoint: .topLeading, endPoint: .bottomTrailing),
                    lineWidth: 1
                )
        )
