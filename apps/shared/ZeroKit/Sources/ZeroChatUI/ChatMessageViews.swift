...
struct ChatMessageBubble: View {
    let message: ZeroChatMessage
    let style: ZeroChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ZeroChatTheme.userText : ZeroChatTheme.assistantText
    ...
    private var inlineAttachments: [ZeroChatMessageContent] {
    ...
    private var toolCalls: [ZeroChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ZeroChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ZeroChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ZeroChatTheme.onboardingAssistantBubble
        }
        return ZeroChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ZeroChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ZeroChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ZeroChatTheme.userText : ZeroChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ZeroChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ZeroChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ZeroChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ZeroChatTheme.assistantText)
            }
        }
    }
}
