...
private let chatUILogger = Logger(subsystem: "com.zero", category: "ZeroChatUI")

@MainActor
@Observable
public final class ZeroChatViewModel {
    public private(set) var messages: [ZeroChatMessage] = []
    ...
    public var attachments: [ZeroPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ZeroChatPendingToolCall] = []
    public private(set) var sessions: [ZeroChatSessionEntry] = []
    private let transport: any ZeroChatTransport
    ...
    private var pendingToolCallsById: [String: ZeroChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ZeroChatTransport) {
    ...
    public var sessionChoices: [ZeroChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [ZeroChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [ZeroChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ZeroPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ZeroChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ZeroChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [ZeroChatMessage]) -> [ZeroChatMessage] {
        var result: [ZeroChatMessage] = []
    ...
    private static func dedupeKey(for message: ZeroChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ZeroChatMessageContent] = [
            ZeroChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ZeroChatAttachmentPayload in
            ZeroChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ZeroChatSessionEntry {
        ZeroChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleChatEvent(_ chat: ZeroChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ZeroAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ZeroChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ZeroPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
    ...
    public func setThinkingLevel(_ level: String) {
        let normalized = self.normalizeThinking(level)
        guard normalized != self.thinkingLevel else { return }
        self.thinkingLevel = normalized
        Task { [weak self] in
            guard let self else { return }
            try? await self.transport.patchSession(sessionKey: self.sessionKey, model: self.currentModel, thinkingLevel: normalized)
        }
    }

    public func setModel(_ model: String?) {
        guard model != self.currentModel else { return }
        self.currentModel = model
        Task { [weak self] in
            guard let self else { return }
            try? await self.transport.patchSession(sessionKey: self.sessionKey, model: model, thinkingLevel: self.thinkingLevel)
        }
    }

    private func normalizeThinking(_ raw: String) -> String {
