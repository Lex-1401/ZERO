...
enum ZeroPlatformImageFactory {
    static func image(_ image: ZeroPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
