...
import com.zero.android.CameraHudKind
    ...
    remember(cameraHud, screenRecordActive, isForeground, statusText, voiceWakeStatusText) {
      ...
      if (!isForeground) {
        return@remember StatusActivity(
          ...
          contentDescription = "Primeiro plano necessário",
        )
      }
      ...
      if (lowerStatus.contains("pairing") || lowerStatus.contains("approval")) {
        return@remember StatusActivity(
          ...
          contentDescription = "Aprovação pendente",
        )
      }
      ...
      if (screenRecordActive) {
        return@remember StatusActivity(
          ...
          contentDescription = "Gravando tela",
        ...
        return@remember when (hud.kind) {
          CameraHudKind.Photo ->
            StatusActivity(
              ...
              contentDescription = "Tirando foto",
            )
          ...
          CameraHudKind.Success ->
            StatusActivity(
              ...
              contentDescription = "Captura finalizada",
            )
          CameraHudKind.Error ->
            StatusActivity(
              title = hud.message,
              icon = Icons.Default.Error,
              contentDescription = "Falha na captura",
              tint = androidx.compose.ui.graphics.Color.Red,
            )
        }
      }
      ...
      if (voiceWakeStatusText.contains("Microphone permission", ignoreCase = true)) {
        return@remember StatusActivity(
          title = "Permissão de mic",
          icon = Icons.Default.Error,
          contentDescription = "Permissão de microfone necessária",
        )
      }
      if (voiceWakeStatusText == "Paused") {
        val suffix = if (!isForeground) " (em segundo plano)" else ""
        return@remember StatusActivity(
          title = "Ativação por Voz pausada$suffix",
          icon = Icons.Default.RecordVoiceOver,
          contentDescription = "Ativação por Voz pausada",
        )
      }
    ...
    remember(serverName, statusText) {
      when {
        ...
        statusText.contains("connecting", ignoreCase = true) ||
          statusText.contains("conectando", ignoreCase = true) ||
          statusText.contains("reconnecting", ignoreCase = true) ||
          statusText.contains("reconectando", ignoreCase = true) -> GatewayState.Connecting
        statusText.contains("error", ignoreCase = true) ||
          statusText.contains("erro", ignoreCase = true) -> GatewayState.Error
  ...
  Popup(alignment = Alignment.TopEnd, properties = PopupProperties(focusable = false)) {
    Column(
      ...
      OverlayIconButton(
        ...
        icon = { Icon(Icons.Default.ChatBubble, contentDescription = "Conversa") },
      )
      ...
      OverlayIconButton(
        ...
        icon = {
          Icon(
            ...
            contentDescription = "Modo de Fala",
          )
        },
      )
  ...
  AndroidView(
    ...
    factory = {
      WebView(context).apply {
        ...
        if (isDebuggable) {
          Log.d("ZeroWebView", "userAgent: ${settings.userAgentString}")
        }
        ...
        webViewClient =
          object : WebViewClient() {
            override fun onReceivedError(
              ...
              Log.e("ZeroWebView", "onReceivedError: ${error.errorCode} ${error.description} ${request.url}")
            }
            ...
            override fun onPageFinished(view: WebView, url: String?) {
              if (isDebuggable) {
                Log.d("ZeroWebView", "onPageFinished: $url")
              }
...
private class CanvasA2UIActionBridge(private val onMessage: (String) -> Unit) {
  ...
  companion object {
    const val interfaceName: String = "zeroCanvasA2UIAction"
  }
}
