package com.zero.android.ui.chat
  ...
  Surface(
    ...
    Column(modifier = Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
      Row(
        ...
        Box {
          FilledTonalButton(
            ...
            Text("Sessão: $currentSessionLabel")
          }
        ...
        Box {
          FilledTonalButton(
            ...
            Text("Pensando: ${thinkingLabel(thinkingLevel)}")
          }
        ...
        Box {
          ...
          DropdownMenu(expanded = showModelMenu, onDismissRequest = { showModelMenu = false }) {
            ModelMenuItem("google/gemini-3-pro", currentModel, onSetModel) { showModelMenu = false }
            ModelMenuItem("anthropic/claude-opus-4-5", currentModel, onSetModel) { showModelMenu = false }
            ModelMenuItem("openai/gpt-5.2", currentModel, onSetModel) { showModelMenu = false }
            ModelMenuItem("ollama/llama3.3", currentModel, onSetModel) { showModelMenu = false }
          }
        }
        ...
        FilledTonalIconButton(onClick = onPickImages, modifier = Modifier.size(42.dp)) {
          Icon(Icons.Default.AttachFile, contentDescription = "Adicionar imagem")
        }
      }
      ...
      OutlinedTextField(
        ...
        modifier = Modifier.fillMaxWidth(),
        placeholder = { Text("Mensagem Zero…") },
        minLines = 2,
        maxLines = 6,
      )
      ...
      Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        ...
        if (pendingRunCount > 0) {
          FilledTonalIconButton(
            ...
            Icon(Icons.Default.Stop, contentDescription = "Abortar")
          }
        } else {
          FilledTonalIconButton(onClick = {
            ...
            Icon(Icons.Default.ArrowUpward, contentDescription = "Enviar")
          }
        }
      }
...
private fun ConnectionPill(sessionLabel: String, healthOk: Boolean) {
  Surface(
    ...
    Row(
      ...
      Text(
        if (healthOk) "Conectado" else "Conectando…",
...
private fun modelLabel(raw: String?): String {
  return when (raw?.trim()) {
    ...
    "ollama/llama3.3" -> "Llama 3.3 (Local)"
    else -> "Modelo"
  }
}
