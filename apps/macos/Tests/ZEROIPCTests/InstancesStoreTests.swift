...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ZeroProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ZeroProtocol.AnyCodable] = [
            "presence": .init([ZeroProtocol.AnyCodable(entry)]),
        ]
        let payload = ZeroProtocol.AnyCodable(payloadMap)
