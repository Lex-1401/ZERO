...
struct ZeroMacCLI {
    static func main() async {
        ...
        default:
            fputs("zero-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      ...
      zero-mac discover [--timeout <ms>] [--json] [--include-local]
      zero-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      zero-mac connect
      zero-mac connect --url ws://127.0.0.1:18789 --json
      zero-mac discover --timeout 3000 --json
      zero-mac wizard --mode local
    """)
}
