...
private let onboardingWizardLogger = Logger(subsystem: "com.zero", category: "onboarding.wizard")

// MARK: - Swift 6 AnyCodable Bridging Helpers

// Bridge between ZeroProtocol.AnyCodable and the local module to avoid
// Swift 6 strict concurrency type conflicts.

private typealias ProtocolAnyCodable = ZeroProtocol.AnyCodable
...
final class OnboardingWizardModel {
    ...
    private func restartIfSessionLost(error: Error) -> Bool {
        ...
        self.currentStep = nil
        self.status = nil
        self.errorMessage = "Sessão do assistente perdida. Reiniciando…"
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            case "action":
                EmptyView()
            default:
                Text("Tipo de passo não suportado")
                    .foregroundStyle(.secondary)
            }

            Button(action: self.submit) {
                Text(wizardStepType(self.step) == "action" ? "Executar" : "Continuar")
                    .frame(minWidth: 120)
            }
