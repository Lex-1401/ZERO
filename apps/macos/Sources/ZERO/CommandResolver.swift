...
enum CommandResolver {
    private static let projectRootDefaultsKey = "zero.gatewayProjectRootPath"
    private static let helperName = "zero"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/zero.js").path
    ...
    static func projectRoot() -> URL {
        // PRIORITY 1: Embedded gateway in Resources (for distributed .app bundles)
        if let resourcePath = Bundle.main.resourcePath {
            let embeddedRoot = URL(fileURLWithPath: resourcePath)
            let entry = embeddedRoot.appendingPathComponent("dist/index.js").path
            if FileManager().fileExists(atPath: entry) {
                return embeddedRoot
            }
        }

        // PRIORITY 2: Stored preference (user-configured)
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/zero")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        let zeroPaths = self.zeroManagedPaths(home: home)
        if !zeroPaths.isEmpty {
            extras.insert(contentsOf: zeroPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + zeroPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func zeroManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".zero")
    ...
    static func zeroExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectZeroExecutable(projectRoot: URL? = nil) -> String? {
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/zero.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyZeroInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.zeroExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let zeroPath = self.projectZeroExecutable(projectRoot: root) {
                return [zeroPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "zero", subcommand] + extraArgs
            }
            if let zeroPath = self.zeroExecutable(searchPaths: searchPaths) {
                return [zeroPath, subcommand] + extraArgs
            }

            let missingEntry = """
            zero entrypoint missing (looked for dist/index.js or bin/zero.js); run pnpm build.
            """
    ...
    // Existing callers still refer to zeroCommand; keep it as node alias.
    static func zeroCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real zero CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/zero"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/zero.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/zero.js"
            node "$PRJ/bin/zero.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent zero"
          pnpm --silent zero \(quotedArgs);
        else
          echo "zero CLI missing on remote host"; exit 127;
        fi
        """
    ...
    {
        let root = configRoot ?? ZeroConfigFile.loadDict()
