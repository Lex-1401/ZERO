...
private let webChatSwiftLogger = Logger(subsystem: "com.zero", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ZeroChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ZeroChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ZeroChatSessionsListResponse.self, from: data)
    }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ZeroChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ZeroChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ZeroChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ZeroChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ZeroChatView>) -> NSViewController
    {
