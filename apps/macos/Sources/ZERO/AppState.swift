...
final class AppState {
    ...
    var onboardingSeen: Bool {
        didSet { self.ifNotPreview { UserDefaults.standard.set(self.onboardingSeen, forKey: "zero.onboardingSeen") }
        }
    }

    var debugPaneEnabled: Bool {
        didSet {
            self.ifNotPreview { UserDefaults.standard.set(self.debugPaneEnabled, forKey: "zero.debugPaneEnabled") }
    ...
    init(preview: Bool = false) {
        ...
        let onboardingSeen = UserDefaults.standard.bool(forKey: "zero.onboardingSeen")
        self.isPaused = UserDefaults.standard.bool(forKey: pauseDefaultsKey)
        self.launchAtLogin = false
        self.onboardingSeen = onboardingSeen
        self.debugPaneEnabled = UserDefaults.standard.bool(forKey: "zero.debugPaneEnabled")
        ...
        let configRoot = ZeroConfigFile.loadDict()
    ...
    private func startConfigWatcher() {
        let configUrl = ZeroConfigFile.url()
    ...
    private func syncGatewayConfigIfNeeded() {
        ...
        Task { @MainActor in
            var root = ZeroConfigFile.loadDict()
            var gateway = root["gateway"] as? [String: Any] ?? [:]
            var changed = false

            changed = self.updateGatewayModeInDict(&gateway, mode: desiredMode)

            if connectionMode == .remote {
                var remote = gateway["remote"] as? [String: Any] ?? [:]
                if self.updateGatewayRemoteInDict(
                    &remote,
            ...
            ZeroConfigFile.saveDict(root)
        }
    }

    private func updateGatewayModeInDict(_ gateway: inout [String: Any], mode: String?) -> Bool {
    ...
    private func updateGatewayRemoteInDict(
        ...
        var changed = false
        if transport == .direct {
            changed = self.updateGatewayDirectRemoteInDict(&remote, url: url) || changed
        } else {
            changed = self.updateGatewaySSHRemoteInDict(
                &remote,
                target: target,
                identity: identity,
                host: host) || changed
    ...
    private func updateGatewayDirectRemoteInDict(_ remote: inout [String: Any], url: String) -> Bool {
        var changed = false
    ...
    private func updateGatewaySSHRemoteInDict(
        _ remote: inout [String: Any],
        target: String,
        identity: String,
        host: String?
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/zero"
        state.remoteCliPath = ""
        return state
    }
}
