...
extension ConfigSettings {
        ...
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 8) {
                if self.sections.isEmpty {
                    Text("Nenhuma seção de configuração disponível.")
                        .font(.caption)
    ...
    private var emptyDetail: some View {
        VStack(alignment: .leading, spacing: 8) {
            self.header
            Text("Selecione uma seção de configuração para ver os ajustes.")
                .font(.callout)
                .foregroundStyle(.secondary)
        }
    ...
    private var header: some View {
        ...
        Text(self.isNixMode
            ? "Esta aba é apenas para leitura no modo Nix. Edite a configuração via Nix e recompile."
            : "Edite ~/.zero/zero.json usando o formulário baseado em esquema.")
            .font(.callout)
            .foregroundStyle(.secondary)
    }
    ...
    private var actionRow: some View {
        HStack(spacing: 10) {
            ...
            Button(self.store.isSavingConfig ? "Salvando…" : "Salvar") {
