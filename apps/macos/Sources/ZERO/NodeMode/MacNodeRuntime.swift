...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        if self.isCanvasCommand(command), !Self.canvasEnabled() {
            return BridgeInvokeResponse(
                ...
                error: ZeroNodeError(
                    code: .unavailable,
                    message: "CANVAS_DISABLED: habilite o Canvas nos Ajustes"))
        }
        do {
            switch command {
            case ZeroCanvasCommand.present.rawValue,
                 ZeroCanvasCommand.hide.rawValue,
                 ZeroCanvasCommand.navigate.rawValue,
                 ZeroCanvasCommand.evalJS.rawValue,
                 ZeroCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case ZeroCanvasA2UICommand.reset.rawValue,
                 ZeroCanvasA2UICommand.push.rawValue,
                 ZeroCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleA2UIInvoke(req)
            case ZeroCameraCommand.snap.rawValue,
                 ZeroCameraCommand.clip.rawValue,
                 ZeroCameraCommand.list.rawValue:
                return try await self.handleCameraInvoke(req)
            case ZeroLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case MacNodeScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            case ZeroSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case ZeroSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case ZeroSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case ZeroSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case ZeroSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZeroCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(ZeroCanvasPresentParams.self, from: req.paramsJSON)) ??
                ZeroCanvasPresentParams()
        ...
        case ZeroCanvasCommand.hide.rawValue:
            let sessionKey = self.mainSessionKey
            await MainActor.run {
                CanvasManager.shared.hide(sessionKey: sessionKey)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ZeroCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(ZeroCanvasNavigateParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
            try await MainActor.run {
                _ = try CanvasManager.shared.show(sessionKey: sessionKey, path: params.url)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ZeroCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(ZeroCanvasEvalParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
        ...
        case ZeroCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(ZeroCanvasSnapshotParams.self, from: req.paramsJSON)
            ...
            guard let image = NSImage(data: data) else {
                return Self.errorResponse(req, code: .unavailable, message: "falha ao decodificar captura do canvas")
            }
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ZeroCanvasA2UICommand.push.rawValue,
             ZeroCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        guard Self.cameraEnabled() else {
            return BridgeInvokeResponse(
                ...
                error: ZeroNodeError(
                    code: .unavailable,
                    message: "CAMERA_DISABLED: habilite a Câmera nos Ajustes"))
        }
        switch req.command {
        case ZeroCameraCommand.snap.rawValue:
            let params = (try? Self.decodeParams(ZeroCameraSnapParams.self, from: req.paramsJSON)) ??
                ZeroCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ZeroCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(ZeroCameraClipParams.self, from: req.paramsJSON)) ??
                ZeroCameraClipParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ZeroCameraCommand.list.rawValue:
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        guard mode != .off else {
            return BridgeInvokeResponse(
                ...
                error: ZeroNodeError(
                    code: .unavailable,
                    message: "LOCATION_DISABLED: habilite a Localização nos Ajustes"))
        }
        let params = (try? Self.decodeParams(ZeroLocationGetParams.self, from: req.paramsJSON)) ??
            ZeroLocationGetParams()
        ...
        if !hasPermission {
            return BridgeInvokeResponse(
                ...
                error: ZeroNodeError(
                    code: .unavailable,
                    message: "LOCATION_PERMISSION_REQUIRED: conceda permissão de Localização"))
        }
        do {
            ...
            let payload = ZeroLocationPayload(
            ...
            return BridgeInvokeResponse(
                ...
                error: ZeroNodeError(
                    code: .unavailable,
                    message: "LOCATION_TIMEOUT: sem sinal a tempo"))
        } catch {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        if let format = params.format?.lowercased(), !format.isEmpty, format != "mp4" {
            return Self.errorResponse(
                req,
                code: .invalidRequest,
                message: "INVALID_REQUEST: formato de tela deve ser mp4")
        }
        let services = await self.mainActorServices()
    ...
    private func handleA2UIReset(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        (() => {
          if (!globalThis.zeroA2UI) return JSON.stringify({ ok: false, error: "missing zeroA2UI" });
          return JSON.stringify(globalThis.zeroA2UI.reset());
        })()
        """)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleA2UIPush(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        let messages: [ZeroKit.AnyCodable]
        if command == ZeroCanvasA2UICommand.pushJSONL.rawValue {
            let params = try Self.decodeParams(ZeroCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
            messages = try ZeroCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
        } else {
            do {
                let params = try Self.decodeParams(ZeroCanvasA2UIPushParams.self, from: req.paramsJSON)
        ...
        let messagesJSON = try ZeroCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
        let js = """
        (() => {
          try {
            if (!globalThis.zeroA2UI) return JSON.stringify({ ok: false, error: "missing zeroA2UI" });
            const messages = \(messagesJSON);
            return JSON.stringify(globalThis.zeroA2UI.applyMessages(messages));
          } catch (e) {
    ...
    private func ensureA2UIHost() async throws {
        ...
        guard let a2uiUrl = await self.resolveA2UIHostUrl() else {
            throw NSError(domain: "Canvas", code: 30, userInfo: [
                NSLocalizedDescriptionKey: "A2UI_HOST_NOT_CONFIGURED: o gateway não anunciou o host do canvas",
            ])
        }
        let sessionKey = self.mainSessionKey
        ...
        throw NSError(domain: "Canvas", code: 31, userInfo: [
            NSLocalizedDescriptionKey: "A2UI_HOST_UNAVAILABLE: host A2UI não acessível",
        ])
    }
    ...
    private func resolveA2UIHostUrl() async -> String? {
        ...
        return baseUrl.appendingPathComponent("__zero__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func isA2UIReady(poll: Bool = false) async -> Bool {
        ...
        while true {
            do {
                ...
                (() => String(Boolean(globalThis.zeroA2UI)))()
                """)
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(ZeroSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(ZeroSystemWhichParams.self, from: req.paramsJSON)
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(ZeroSystemNotifyParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> ZeroLocationMode {
        let raw = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        return ZeroLocationMode(rawValue: raw) ?? .off
    }
        ...
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: ZeroNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: ZeroCanvasSnapshotFormat,
        maxWidth: Int?,
