...
private let gatewayConnectionLogger = Logger(subsystem: "com.zero", category: "gateway.connection")
...
actor GatewayConnection {
        ...
        timeoutMs: Double? = nil) async throws -> Data
    {
        ...
        guard let client else {
            throw NSError(domain: "Gateway", code: 0, userInfo: [NSLocalizedDescriptionKey: "gateway não configurado"])
        }
                        ...
                        do {
                            guard let client = self.client else {
                                throw NSError(
                                    domain: "Gateway",
                                    code: 0,
                                    userInfo: [NSLocalizedDescriptionKey: "gateway não configurado"])
                            }
                            return try await client.request(method: method, params: params, timeoutMs: timeoutMs)
                        } catch {
                            lastError = error
    ...
    private func sessionDefaultString(_ defaults: [String: ZeroProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func sendAgent(_ invocation: GatewayAgentInvocation) async -> (ok: Bool, error: String?) {
        ...
        guard !trimmed.isEmpty else { return (false, "mensagem vazia") }
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(ZeroGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> ZeroSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return ZeroSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        sessionKey: String,
        limit: Int? = nil,
        timeoutMs: Int? = nil) async throws -> ZeroChatHistoryPayload
    {
    ...
    func chatSend(
        ...
        attachments: [ZeroChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> ZeroChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        var params: [String: AnyCodable] = [
