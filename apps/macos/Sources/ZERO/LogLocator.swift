...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ZERO_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/zero")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("zero-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("zero-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/zero/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("zero") && $0.pathExtension == "log" }
            .max { lhs, rhs in
