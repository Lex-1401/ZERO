...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "com.zero", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ZeroSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ZeroSessionPreviewEntry,
    ...
    private static func cache(payload: ZeroSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ZeroChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ZeroKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ZeroKit.AnyCodable]) -> [ZeroChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ZeroChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ZeroChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ZeroChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ZeroChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ZeroChatMessage) -> String? {
