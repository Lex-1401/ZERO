...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Zero token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: ZeroOAuthStore.AnthropicOAuthStatus = ZeroOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "com.zero", category: "anthropic-oauth")
...
enum ZeroOAuthStore {
    ...
    private static let zeroOAuthDirEnv = "ZERO_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Zero OAuth token file not found"
            case .unreadableFile: "Zero OAuth token file not readable"
            case .invalidJSON: "Zero OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Zero OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Zero OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.zeroOAuthDirEnv]?
