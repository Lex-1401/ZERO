...
enum DebugActions {
    private static let verboseDefaultsKey = "zero.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "zero.onboardingSeen"
    ...
    static func openLog() {
        ...
        guard FileManager().fileExists(atPath: path) else {
            let alert = NSAlert()
            alert.messageText = "Arquivo de log não encontrado"
    ...
    static func openSessionStore() {
        if AppStateStore.shared.connectionMode == .remote {
            let alert = NSAlert()
            alert.messageText = "Modo remoto"
            alert.informativeText = "O armazenamento de sessões reside no host do gateway no modo remoto."
            alert.runModal()
            return
        }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Zero", body: "Notificação de teste", sound: nil)
    }

    static func sendDebugVoice() async -> Result<String, DebugActionError> {
        let message = """
        Este é um teste de depuração do app Mac. Responda com "O teste de depuração funciona (e um trocadilho engraçado)" \
        se você recebeu isso.
        """
        let result = await VoiceWakeForwarder.forward(transcript: message)
        switch result {
        case .success:
            return .success("Enviado. Aguarde a resposta.")
        case let .failure(error):
            let detail = error.localizedDescription.trimmingCharacters(in: .whitespacesAndNewlines)
            return .failure(.message("Falha no envio: \(detail)"))
        }
    }
    ...
    static func resetGatewayTunnel() async -> Result<String, DebugActionError> {
        ...
        do {
            ...
            return .success("Túnel SSH redefinido.")
        } catch {
            Task { await HealthStore.shared.refresh(onDemand: true) }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".zero/zero.json")
        guard
    ...
    static func killProcess(_ pid: Int) async -> Result<Void, DebugActionError> {
        ...
        let detail = force.message ?? primary.message ?? "falha ao encerrar"
