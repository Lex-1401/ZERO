...
extension ChannelsSettings {
    ...
    var whatsAppDetails: String? {
        ...
        if let e164 = status.`self`?.e164 ?? status.`self`?.jid {
            lines.append("Vinculado como \(e164)")
        }
        if let age = status.authAgeMs {
            lines.append("Idade da autorização \(msToAge(age))")
        }
        if let last = self.date(fromMs: status.lastConnectedAt) {
            lines.append("Última conexão \(relativeAge(from: last))")
        }
        if let disconnect = status.lastDisconnect {
            let when = self.date(fromMs: disconnect.at).map { relativeAge(from: $0) } ?? "desconhecido"
            let code = disconnect.status.map { "status \($0)" } ?? "status desconhecido"
            let err = disconnect.error ?? "desconexão"
            lines.append("Última desconexão \(code) · \(err) · \(when)")
        }
        if status.reconnectAttempts > 0 {
            lines.append("Tentativas de reconexão \(status.reconnectAttempts)")
        }
        if let msgAt = self.date(fromMs: status.lastMessageAt) {
            lines.append("Última mensagem \(relativeAge(from: msgAt))")
        }
        if let err = status.lastError, !err.isEmpty {
            lines.append("Erro: \(err)")
        }
    ...
    var telegramDetails: String? {
        ...
        if let source = status.tokenSource {
            lines.append("Fonte do token: \(source)")
        }
        ...
        if let probe = status.probe {
            if probe.ok {
                ...
                let code = probe.status.map { String($0) } ?? "desconhecido"
                lines.append("Sonda falhou (\(code))")
            }
        }
        if let last = self.date(fromMs: status.lastProbeAt) {
            lines.append("Última sonda \(relativeAge(from: last))")
        }
        if let err = status.lastError, !err.isEmpty {
            lines.append("Erro: \(err)")
        }
        ...
        if let probe = status.probe {
            if probe.ok {
                ...
                if let elapsed = probe.elapsedMs {
                    lines.append("Sonda \(Int(elapsed))ms")
                }
            } else {
                let code = probe.status.map { String($0) } ?? "desconhecido"
                lines.append("Sonda falhou (\(code))")
    ...
    var googlechatDetails: String? {
        ...
        if let source = status.credentialSource {
            lines.append("Credencial: \(source)")
        }
    ...
    var signalDetails: String? {
        ...
        var lines: [String] = []
        lines.append("URL Base: \(status.baseUrl)")
        if let probe = status.probe {
            if probe.ok {
                if let version = probe.version, !version.isEmpty {
                    lines.append("Versão \(version)")
                }
                if let elapsed = probe.elapsedMs {
                    lines.append("Sonda \(Int(elapsed))ms")
                }
    ...
    var imessageDetails: String? {
        ...
        if let dbPath = status.dbPath, !dbPath.isEmpty {
            lines.append("Banco de dados: \(dbPath)")
        }
        if let probe = status.probe, !probe.ok {
            let err = probe.error ?? "sonda falhou"
            lines.append("Erro na sonda: \(err)")
        }
        if let last = self.date(fromMs: status.lastProbeAt) {
            lines.append("Última sonda \(relativeAge(from: last))")
        }
    ...
    func channelSummary(_ channel: ChannelItem) -> String {
        ...
        default:
            if self.channelHasError(channel) { return "Erro" }
            if self.channelEnabled(channel) { return "Ativo" }
    ...
    func channelLastCheckText(_ channel: ChannelItem) -> String {
        guard let date = self.channelLastCheck(channel) else { return "nunca" }
