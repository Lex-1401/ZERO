...
struct SystemRunSettingsView: View {
    ...
    private var allowlistView: some View {
        VStack(alignment: .leading, spacing: 10) {
            Toggle("Auto-permitir CLIs de habilidades", isOn: Binding(
            ...
            if self.model.autoAllowSkills, !self.model.skillBins.isEmpty {
                Text("CLIs de Habilidades: \(self.model.skillBins.joined(separator: ", "))")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }

            if self.model.isDefaultsScope {
                Text("As listas de permissões são por agente. Selecione um agente para editar sua lista.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            } else {
                HStack(spacing: 8) {
                    TextField("Adicionar padrão à lista (globs insensíveis a maiúsculas)", text: self.$newPattern)
                ...
                if self.model.entries.isEmpty {
                    Text("Nenhum comando na lista de permissões ainda.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                } else {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Comandos na lista de permissões:")
    ...
    private var scopeMessage: String {
        if self.model.isDefaultsScope {
            return "Os padrões se aplicam quando um agente não tem substituições. " +
                "O controle 'Perguntar' define o comportamento de prompt; o 'Fallback' é usado quando nenhuma interface está acessível."
        }
        return "Os controles de segurança definem se o system.run pode ser executado neste Mac quando emparelhado como um nó. " +
            "O controle 'Perguntar' define o comportamento de prompt; o 'Fallback' é usado quando nenhuma interface está acessível."
    }
}
...
private enum ExecApprovalsSettingsTab: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .policy: "Acesso"
        case .allowlist: "Lista de Permissões"
        }
    }
}
...
struct ExecAllowlistRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 8) {
                TextField("Padrão", text: self.patternBinding)
                    .textFieldStyle(.roundedBorder)
            ...
            if let lastUsedAt = self.entry.lastUsedAt {
                ...
                Text("Último uso \(Self.relativeFormatter.localizedString(for: date, relativeTo: Date()))")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }

            if let lastUsedCommand = self.entry.lastUsedCommand, !lastUsedCommand.isEmpty {
                Text("Último comando: \(lastUsedCommand)")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }

            if let lastResolvedPath = self.entry.lastResolvedPath, !lastResolvedPath.isEmpty {
                Text("Caminho resolvido: \(lastResolvedPath)")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
...
final class ExecApprovalsSettingsModel {
    ...
    func label(for id: String) -> String {
        if id == Self.defaultsScopeId { return "Padrões" }
        return id
    }
