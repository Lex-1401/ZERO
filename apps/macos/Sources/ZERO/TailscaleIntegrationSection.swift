...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.zero/zero.json. Prefer ZERO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.zero/zero.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.zero/zero.json. Restart the gateway to apply."
        }
