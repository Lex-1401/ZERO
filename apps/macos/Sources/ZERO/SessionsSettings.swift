    ...
    private var header: some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                Text("Sessões")
                    .font(.headline)
                Text("Dê uma olhada nos buckets de conversa armazenados que a CLI reutiliza para contexto e limites de taxa.")
                    .font(.footnote)
    ...
    private var content: some View {
        Group {
            if self.rows.isEmpty, self.errorMessage == nil {
                Text("Nenhuma sessão ainda. Elas aparecem após a primeira mensagem recebida ou pulsação.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                    .padding(.top, 6)
            } else {
    ...
    private func sessionRow(_ row: SessionRow) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 10) {
                ...
                self.label(icon: "arrow.down.left", text: "\(row.tokens.input) ent")
                self.label(icon: "arrow.up.right", text: "\(row.tokens.output) sai")
