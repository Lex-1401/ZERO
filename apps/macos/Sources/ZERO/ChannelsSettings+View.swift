...
extension ChannelsSettings {
        ...
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 8) {
                if !self.enabledChannels.isEmpty {
                    self.sidebarSectionHeader("Configurado")
                ...
                if !self.availableChannels.isEmpty {
                    self.sidebarSectionHeader("Disponível")
    ...
    private var emptyDetail: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Selecione um canal para visualizar o status e os ajustes.")
                .font(.callout)
    ...
    private func detailHeader(for channel: ChannelItem) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 10) {
                Text("Última verificação \(self.channelLastCheckText(channel))")
                    .font(.caption)
                    .foregroundStyle(.secondary)
