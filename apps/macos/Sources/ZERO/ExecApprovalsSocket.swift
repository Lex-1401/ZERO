...
enum ExecApprovalsPromptPresenter {
    ...
    static func prompt(_ request: ExecApprovalPromptRequest) -> ExecApprovalDecision {
        ...
        alert.messageText = "Permitir este comando?"
        alert.informativeText = "Revise os detalhes do comando antes de permitir."
        alert.accessoryView = self.buildAccessoryView(request)

        alert.addButton(withTitle: "Permitir uma vez")
        alert.addButton(withTitle: "Sempre permitir")
        alert.addButton(withTitle: "Não permitir")
    ...
    private static func buildAccessoryView(_ request: ExecApprovalPromptRequest) -> NSView {
        ...
        let commandTitle = NSTextField(labelWithString: "Comando")
        ...
        let contextTitle = NSTextField(labelWithString: "Contexto")
        ...
        if !trimmedCwd.isEmpty {
            self.addDetailRow(title: "Diretório de trabalho", value: trimmedCwd, to: contextStack)
        }
        let trimmedAgent = request.agentId?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        if !trimmedAgent.isEmpty {
            self.addDetailRow(title: "Agente", value: trimmedAgent, to: contextStack)
        }
        let trimmedPath = request.resolvedPath?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        if !trimmedPath.isEmpty {
            self.addDetailRow(title: "Executável", value: trimmedPath, to: contextStack)
        }
        ...
        if let security = request.security?.trimmingCharacters(in: .whitespacesAndNewlines), !security.isEmpty {
            self.addDetailRow(title: "Segurança", value: security, to: contextStack)
        }
        if let ask = request.ask?.trimmingCharacters(in: .whitespacesAndNewlines), !ask.isEmpty {
            self.addDetailRow(title: "Modo de pergunta", value: ask, to: contextStack)
        }

        if contextStack.arrangedSubviews.isEmpty {
            let empty = NSTextField(labelWithString: "Nenhum contexto adicional fornecido.")
        ...
        let footer = NSTextField(labelWithString: "Isso será executado nesta máquina.")
...
private final class ExecApprovalsSocketServer: @unchecked Sendable {
    private let logger = Logger(subsystem: "com.zero", category: "exec-approvals.socket")
