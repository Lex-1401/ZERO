...
struct DebugSettings: View {
    ...
    var body: some View {
        ...
        .alert(item: self.$pendingKill) { listener in
            Alert(
                title: Text("Encerrar \(listener.command) (\(listener.pid))?"),
                message: Text("Este processo parece esperado para o modo atual. Encerrar mesmo assim?"),
    ...
    private var launchdSection: some View {
        GroupBox("Inicialização do Gateway") {
            VStack(alignment: .leading, spacing: 8) {
                Toggle("Apenas anexar (pular instalação do launchd)", isOn: self.$launchAgentWriteDisabled)
                ...
                Text(
                    "Quando ativado, o Zero não instalará nem gerenciará o \(gatewayLaunchdLabel). " +
                        "Ele apenas se anexará a um Gateway existente.")
                    .font(.caption)
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("Ferramentas para diagnosticar problemas locais (Gateway, portas, logs, Canvas).")
                .font(.callout)
                .foregroundStyle(.secondary)
        }
    }
        ...
        GroupBox("Gateway") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("Copiar URL de exemplo") {
                        let msg = "Olá via deep link"
                        let encoded = msg.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? msg
                        let url = "zero://agent?message=\(encoded)&key=\(key)"
                        NSPasteboard.general.clearContents()
                ...
                Text("Deep links (zero://…) estão sempre ativados; a chave controla execuções não assistidas.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    HStack(spacing: 8) {
                        if self.canRestartGateway {
                            Button("Reiniciar Gateway") { DebugActions.restartGateway() }
                        }
                        Button("Limpar log") { GatewayProcessManager.shared.clearLog() }
                        Spacer(minLength: 0)
                    }
                    .buttonStyle(.bordered)
                }
        ...
        GroupBox("Logs") {
            Grid(alignment: .leadingFirstTextBaseline, horizontalSpacing: 14, verticalSpacing: 10) {
                GridRow {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack(spacing: 8) {
                            Button("Abrir") { DebugActions.openLog() }
                                .buttonStyle(.bordered)
                ...
                GridRow {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        .pickerStyle(.menu)
                        .labelsHidden()
                        .help("Controla a verbosidade do log do app macOS.")

                        Toggle("Escrever log de diagnóstico rotativo (JSONL)", isOn: self.$diagnosticsFileLogEnabled)
                            .toggleStyle(.checkbox)
                            .help(
                                "Escreve um log rotativo e apenas local em ~/Library/Logs/Zero/. " +
                                    "Ative apenas durante a depuração ativa.")
        ...
        GroupBox("Portas") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                if self.portReports.isEmpty, !self.portCheckInFlight {
                    Text("Verifica qual processo detém a porta \(GatewayEnvironment.gatewayPort()) e sugere correções.")
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                } else {
                    ForEach(self.portReports) { report in
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Porta \(report.port)")
    ...
    private var pathsSection: some View {
        GroupBox("Caminhos") {
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("Raiz do projeto Zero")
                        .font(.caption.weight(.semibold))
                    HStack(spacing: 8) {
                        TextField("Caminho para o repositório zero", text: self.$gatewayRootInput)
                            .textFieldStyle(.roundedBorder)
                            .font(.caption.monospaced())
                            .onSubmit { self.saveRelayRoot() }
                        Button("Salvar") { self.saveRelayRoot() }
                            .buttonStyle(.borderedProminent)
                        Button("Redefinir") {
                            let def = FileManager().homeDirectoryForCurrentUser
                                .appendingPathComponent("Projetos/zero").path
                            self.gatewayRootInput = def
                            self.saveRelayRoot()
                        }
                        .buttonStyle(.bordered)
                    }
                    Text("Usado para fallback de pnpm/node e população do PATH ao iniciar o gateway.")
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                }
                ...
                Grid(alignment: .leadingFirstTextBaseline, horizontalSpacing: 14, verticalSpacing: 10) {
                    GridRow {
                        self.gridLabel("Armazenamento de sessões")
                        VStack(alignment: .leading, spacing: 6) {
                            HStack(spacing: 8) {
                                TextField("Path", text: self.$sessionStorePath)
                                    ...
                                    .font(.caption.monospaced())
                                    .frame(width: 360)
                                Button("Salvar") { self.saveSessionStorePath() }
                                    .buttonStyle(.borderedProminent)
                            }
                            if let sessionStoreSaveError {
                                ...
                                Text("Usado pelo carregador de sessões da CLI; armazenado em ~/.zero/zero.json.")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }
                    ...
                    GridRow {
                        self.gridLabel("Catálogo de modelos")
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            HStack(spacing: 8) {
                                Button {
                                    ...
                                    Label("Escolher models.generated.ts…", systemImage: "folder")
                                }
                                .buttonStyle(.bordered)
                                ...
                                Button {
                                    ...
                                    Label(
                                        self.modelsLoading ? "Recarregando…" : "Recarregar modelos",
                            ...
                            if let modelsError {
                                ...
                                Text("Carregado \(modelsCount) modelos")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                            Text("Fallback local para o seletor de modelos quando models.list do gateway estiver indisponível.")
                                .font(.footnote)
        ...
        GroupBox("Ações rápidas") {
            VStack(alignment: .leading, spacing: 10) {
                HStack(spacing: 8) {
                    ...
                    Button("Abrir eventos do agente") {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Button {
                        ...
                        Label(
                            self.debugSendInFlight ? "Enviando voz de depuração…" : "Enviar voz de depuração",
                    ...
                    if !self.debugSendInFlight {
                        if let debugSendStatus {
                            ...
                            Text(
                                """
                                Usa o caminho de Ativação por Voz: encaminha via SSH quando configurado, \
                                caso contrário, executa localmente via RPC.
                                """)
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(
                        "Nota: o macOS pode exigir a reinicialização do Zero após ativar Acessibilidade ou Gravação de Tela.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)

                    Button {
                        LaunchdManager.startZero()
                    } label: {
                        Label("Reiniciar Zero", systemImage: "arrow.counterclockwise")
                    }
                    .buttonStyle(.bordered)
                    .controlSize(.small)
                }
                ...
                HStack(spacing: 8) {
                    Button("Reiniciar app") { DebugActions.restartApp() }
                    Button("Reiniciar onboarding") { DebugActions.restartOnboarding() }
                    Button("Revelar app no Finder") { self.revealApp() }
                    Spacer(minLength: 0)
                }
                .buttonStyle(.bordered)
            }
    ...
    private var canvasSection: some View {
        GroupBox("Canvas") {
            VStack(alignment: .leading, spacing: 10) {
                Text("Ative/desative o Canvas nas configurações Gerais.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                ...
                HStack(spacing: 8) {
                    ...
                    .buttonStyle(.borderedProminent)
                    Button("Ocultar painel") {
                        CanvasManager.shared.hideAll()
                        self.canvasStatus = "oculto"
                        self.canvasError = nil
                    }
                    .buttonStyle(.bordered)
                    Button("Escrever página de exemplo") {
                ...
                if let canvasError {
                    ...
                    Text("Dica: o diretório da sessão é retornado por “Mostrar painel”.")
                        .font(.caption2)
                        .foregroundStyle(.tertiary)
                }
            }
    ...
    private var experimentsSection: some View {
        GroupBox("Experimentos") {
            Grid(alignment: .leadingFirstTextBaseline, horizontalSpacing: 14, verticalSpacing: 10) {
                GridRow {
                    self.gridLabel("Substituição de ícone")
    ...
    private func killConfirmed(_ pid: Int32) async {
        ...
        switch result {
        case .success:
            self.portKillStatus = "Pedido de encerramento enviado para \(pid)."
            await self.runPortCheck()
        case let .failure(err):
            self.portKillStatus = "Falha ao encerrar \(pid): \(err.localizedDescription)"
        }
    }

    private func chooseCatalogFile() {
        let panel = NSOpenPanel()
        panel.title = "Selecionar models.generated.ts"
...
extension DebugSettings {
    static func exerciseForTesting() async {
        ...
        view.gatewayRootInput = "/tmp/zero"
