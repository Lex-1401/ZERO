...
enum ZeroLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = ZeroOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ZeroFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ZeroLogging.bootstrapIfNeeded()
        let label = ZeroLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ZeroOSLogHandler: LogHandler {
...
struct ZeroFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ZeroLogging.parseLabel(self.label)
