...
enum CritterIconRenderer {
        ...
        badge: Badge? = nil) -> NSImage
    {
        ...
        if let mascotImage = NSImage(contentsOfFile: mascotPath) ?? NSImage(named: "AppIcon_Official") {
            ...
            guard let context = NSGraphicsContext(bitmapImageRep: rep) else { return mascotImage }
        ...
        // Fallback to original drawing if mascot missing
    ...
    private static func drawBody(in canvas: Canvas, geometry: Geometry) {
        canvas.context.setFillColor(NSColor.white.cgColor)
