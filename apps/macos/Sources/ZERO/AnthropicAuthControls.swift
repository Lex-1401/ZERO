...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ZeroOAuthStore.AnthropicOAuthStatus = ZeroOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ZeroOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ZeroOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ZeroOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ZeroOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Zero can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ZeroOAuthStore.AnthropicOAuthStatus,
