...
struct VoiceWakeSettings: View {
    ...
    var body: some View {
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 14) {
                SettingsToggleRow(
                    title: "Ativar Ativação por Voz",
                    subtitle: "Ouvir uma frase de ativação (por exemplo, \"Claude\") antes de executar comandos de voz. "
                        + "O reconhecimento de voz é executado totalmente no dispositivo.",
                    binding: self.voiceWakeBinding)
                    .disabled(!voiceWakeSupported)

                SettingsToggleRow(
                    title: "Pressione Option Direita para falar",
                    subtitle: """
                    Modo apertar para falar que começa a ouvir enquanto você segura a tecla
                    e mostra a sobreposição de pré-visualização.
                    """,
                    binding: self.$state.voicePushToTalkEnabled)
                    .disabled(!voiceWakeSupported)

                if !voiceWakeSupported {
                    Label("A Ativação por Voz requer o macOS 26 ou mais recente.", systemImage: "exclamationmark.triangle.fill")
                        .font(.callout)
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 0) {
                ForEach(self.$triggerEntries) { $entry in
                    HStack(spacing: 8) {
                        TextField("Palavra de ativação", text: $entry.value)
                        ...
                        .help("Remover palavra de ativação")
                        .frame(width: 24)
                    }
                    .padding(8)
            ...
            Text(
                "O Zero reage quando qualquer gatilho aparece em uma transcrição. "
                    + "Mantenha-os curtos para evitar falsos positivos.")
                .font(.footnote)
    ...
    private func toggleTest() {
        guard voiceWakeSupported else {
            self.testState = .failed("A Ativação por Voz requer o macOS 26 ou mais recente.")
            return
        }
        if self.isTesting {
                ...
                self.testTimeoutTask = Task { @MainActor in
                    ...
                    if self.isTesting {
                           ...
                           let command = Self.textOnlyCommand(from: text, triggers: triggers)
                        {
                            ...
                            self.testState = .failed("Tempo esgotado: nenhum gatilho ouvido")
                        }
                        self.isTesting = false
                    }
                }
    ...
    private func chimeRow(title: String, selection: Binding<VoiceWakeChime>) -> some View {
        HStack(alignment: .center, spacing: 10) {
            ...
            Menu {
                Button("Sem Som") { self.selectChime(.none, binding: selection) }
                Divider()
                ...
                Divider()
                Button("Escolher arquivo…") { self.chooseCustomChime(for: selection) }
            } label: {
                HStack(spacing: 6) {
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                Text("Microfone")
                    .font(.callout.weight(.semibold))
                    .frame(width: self.fieldLabelWidth, alignment: .leading)
                Picker("Microfone", selection: self.$state.voiceWakeMicID) {
                    Text("Padrão do sistema").tag("")
                    if self.isSelectedMicUnavailable {
                        Text(self.state.voiceWakeMicName.isEmpty ? "Indisponível" : self.state.voiceWakeMicName)
            ...
            if self.isSelectedMicUnavailable {
                HStack(spacing: 10) {
                    ...
                    Text("Desconectado (usando o padrão do sistema)")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(1)
                }
    ...
    private var localePicker: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                Text("Idioma de reconhecimento")
                    .font(.callout.weight(.semibold))
                    .frame(width: self.fieldLabelWidth, alignment: .leading)
                Picker("Idioma", selection: self.$state.voiceWakeLocaleID) {
                    let current = Locale(identifier: Locale.current.identifier)
                    Text("\(self.friendlyName(for: current)) (Sistema)").tag(Locale.current.identifier)
            ...
            if !self.state.voiceWakeAdditionalLocaleIDs.isEmpty {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Button {
                        ...
                        Label("Adicionar idioma", systemImage: "plus")
                    }
                ...
                Button {
                    ...
                    Label("Adicionar idioma adicional", systemImage: "plus")
                }
                .buttonStyle(.link)
                .disabled(self.availableLocales.isEmpty)
                .padding(.top, 4)
            ...
            Text("Os idiomas são testados em ordem. Os modelos podem precisar de um download no primeiro uso no macOS 26.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
