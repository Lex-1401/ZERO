...
final class VoiceWakeTester {
    ...
    private let logger = Logger(subsystem: "com.zero", category: "voicewake")
        ...
        onUpdate: @escaping @Sendable (VoiceWakeTestState) -> Void) async throws
    {
        ...
        guard let recognizer, recognizer.isAvailable else {
            throw NSError(
                ...
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Reconhecimento de fala indisponível"])
        }
        ...
        guard Self.hasPrivacyStrings else {
            throw NSError(
                ...
                userInfo: [
                    ...
                    Descrições de privacidade de microfone/fala ausentes. Recompile o aplicativo mac (scripts/restart-mac.sh) \
                    para incluir descrições de uso.
                    """,
                ])
        }
        ...
        guard granted else {
            throw NSError(
                domain: "VoiceWakeTester",
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: "Permissão de microfone ou fala negada"])
        }
        ...
        guard format.channelCount > 0, format.sampleRate > 0 else {
            ...
            throw NSError(
                domain: "VoiceWakeTester",
                code: 4,
                userInfo: [NSLocalizedDescriptionKey: "Nenhuma entrada de áudio disponível"])
        }
        ...
        onUpdate: @escaping @Sendable (VoiceWakeTestState) -> Void) async
    {
        ...
        if isFinal {
            ...
            let state: VoiceWakeTestState = text.isEmpty
                ? .failed("Nenhuma fala detectada")
                : .failed("Nenhum gatilho ouvido: “\(text)”")
