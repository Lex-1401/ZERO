...
extension MenuSessionsInjector {
    ...
    private func inject(into menu: NSMenu) {
        ...
        if let snapshot = self.cachedSnapshot {
            ...
            if rows.isEmpty {
                menu.insertItem(
                    self.makeMessageItem(text: "Nenhuma sessão ativa", symbolName: "minus", width: width),
                    at: cursor)
                cursor += 1
            } else {
                for row in rows {
            ...
            if !isConnected {
                menu.insertItem(
                    self.makeMessageItem(
                        text: "Conecte o gateway para ver as sessões",
    ...
    private func injectNodes(into menu: NSMenu) {
        ...
        if case .connecting = ControlChannel.shared.state {
            menu.insertItem(
                self.makeMessageItem(text: "Conectando…", symbolName: "circle.dashed", width: width),
                at: cursor)
            cursor += 1
            return
        }
        ...
        if entries.isEmpty {
            let title = self.nodesStore.isLoading ? "Carregando dispositivos..." : "Nenhum dispositivo encontrado"
            menu.insertItem(
            ...
            if entries.count > 8 {
                ...
                moreItem.title = "Mais Dispositivos..."
    ...
    private func insertCostUsageSection(into menu: NSMenu, at cursor: Int, width: CGFloat) -> Int {
        ...
        let item = NSMenuItem(title: "Custo de uso (30 dias)", action: nil, keyEquivalent: "")
        item.tag = self.tag
        item.isEnabled = true
    ...
    private func controlChannelStatusText(for state: ControlChannel.ConnectionState) -> String {
        ...
        case let .degraded(message):
            message.nonEmpty ?? "Gateway desconectado"
        case .disconnected:
...
extension MenuSessionsInjector {
    ...
    private func refreshCache(force: Bool) async {
        ...
        guard self.isControlChannelConnected else {
            if self.cachedSnapshot != nil {
                self.cachedErrorText = "Gateway desconectado (exibindo cache)"
            } else {
    ...
    private func compactUsageError(_ error: Error) -> String {
        ...
        if message.isEmpty { return "Uso indisponível" }
    ...
    private func compactError(_ error: Error) -> String {
        if let loadError = error as? SessionLoadError {
            ...
            case .gatewayUnavailable:
                return "Sem conexão com o gateway"
            case .decodeFailed:
                return "Sessões indisponíveis"
            }
        }
        return "Sessões indisponíveis"
    }
...
extension MenuSessionsInjector {
    ...
    private func buildSubmenu(for row: SessionRow, storePath: String) -> NSMenu {
        ...
        menu.addItem(self.makeSessionPreviewItem(
            sessionKey: row.key,
            title: "Mensagens recentes (últimas 10)",
            width: width,
            maxLines: 3))

        let morePreview = NSMenuItem(title: "Mais pré-visualização…", action: nil, keyEquivalent: "")
        morePreview.submenu = self.buildPreviewSubmenu(sessionKey: row.key, width: width)
        menu.addItem(morePreview)

        menu.addItem(NSMenuItem.separator())

        let thinking = NSMenuItem(title: "Raciocínio", action: nil, keyEquivalent: "")
        thinking.submenu = self.buildThinkingMenu(for: row)
        menu.addItem(thinking)

        let verbose = NSMenuItem(title: "Detalhado", action: nil, keyEquivalent: "")
        ...
        let reset = NSMenuItem(title: "Redefinir Sessão", action: #selector(self.resetSession(_:)), keyEquivalent: "")
        reset.target = self
        reset.representedObject = row.key
        menu.addItem(reset)

        let compact = NSMenuItem(
            title: "Compactar Log da Sessão",
        ...
        if row.key != self.mainSessionKey, row.key != "global" {
            let del = NSMenuItem(title: "Excluir Sessão", action: #selector(self.deleteSession(_:)), keyEquivalent: "")
            del.target = self
    ...
    private func buildPreviewSubmenu(sessionKey: String, width: CGFloat) -> NSMenu {
        ...
        menu.addItem(self.makeSessionPreviewItem(
            ...
            title: "Mensagens recentes (expandido)",
            width: width,
            maxLines: 8))
        return menu
    }
    ...
    private func buildNodeSubmenu(entry: NodeInfo, width: CGFloat) -> NSMenu {
        ...
        if let name = entry.displayName?.nonEmpty {
            menu.addItem(self.makeNodeCopyItem(label: "Nome", value: name))
        }
        ...
        if let platform = NodeMenuEntryFormatter.platformText(entry) {
            menu.addItem(self.makeNodeCopyItem(label: "Plataforma", value: platform))
        }

        if let version = NodeMenuEntryFormatter.detailRightVersion(entry)?.nonEmpty {
            menu.addItem(self.makeNodeCopyItem(label: "Versão", value: version))
        }

        menu.addItem(self.makeNodeDetailItem(label: "Conectado", value: entry.isConnected ? "Sim" : "Não"))
        menu.addItem(self.makeNodeDetailItem(label: "Pareado", value: entry.isPaired ? "Sim" : "Não"))
    ...
    private func compactSession(_ sender: NSMenuItem) {
        ...
        Task { @MainActor in
            guard SessionActions.confirmDestructiveAction(
                title: "Compactar log da sessão?",
                message: "Mantém as últimas 400 linhas e arquiva o arquivo antigo.",
                action: "Compactar")
            else { return }
    ...
    private func deleteSession(_ sender: NSMenuItem) {
        ...
        Task { @MainActor in
            guard SessionActions.confirmDestructiveAction(
                ...
                message: "Exclui a entrada “\(key)” e arquiva sua transcrição.",
                action: "Excluir")
            else { return }
