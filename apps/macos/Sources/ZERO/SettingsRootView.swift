...
struct SettingsRootView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            TabView(selection: self.$selectedTab) {
                ...
                ChannelsSettings()
                    .tabItem { Label("Canais", systemImage: "link") }
                    .tag(SettingsTab.channels)

                VoiceWakeSettings(state: self.state, isActive: self.selectedTab == .voiceWake)
                    .tabItem { Label("Ativação por Voz", systemImage: "waveform.circle") }
                    .tag(SettingsTab.voiceWake)

                ConfigSettings()
                    .tabItem { Label("Configuração", systemImage: "slider.horizontal.3") }
                    .tag(SettingsTab.config)

                InstancesSettings()
                    .tabItem { Label("Instâncias", systemImage: "network") }
                    .tag(SettingsTab.instances)

                SessionsSettings()
                    .tabItem { Label("Sessões", systemImage: "clock.arrow.circlepath") }
                ...
                if self.state.debugPaneEnabled {
                    DebugSettings(state: self.state)
                        .tabItem { Label("Depuração", systemImage: "ant") }
        ...
        .onReceive(NotificationCenter.default.publisher(for: .zeroSelectSettingsTab)) { note in
    ...
    private var nixManagedBanner: some View {
        ...
        let configPath = self.snapshotPaths.configPath ?? ZeroPaths.configURL.path
        let stateDir = self.snapshotPaths.stateDir ?? ZeroPaths.stateDirURL.path

        return VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                Image(systemName: "gearshape.2.fill")
                    .foregroundStyle(.secondary)
                Text("Gerenciado pelo Nix")
...
enum SettingsTab: CaseIterable {
    ...
    var title: String {
        ...
        case .instances: "Instâncias"
        case .voiceWake: "Ativação por Voz"
        case .permissions: "Permissões"
        case .debug: "Depuração"
        case .about: "Sobre"
        }
    }
...
extension Notification.Name {
    static let zeroSelectSettingsTab = Notification.Name("zeroSelectSettingsTab")
}
