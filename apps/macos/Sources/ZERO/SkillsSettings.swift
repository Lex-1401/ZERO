...
struct SkillsSettings: View {
        ...
        if self.model.skills.isEmpty {
            ...
            List {
                ...
                if !self.model.skills.isEmpty, self.filteredSkills.isEmpty {
                    Text("Nenhuma habilidade corresponde a este filtro.")
                        .font(.callout)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private var headerFilter: some View {
        Picker("Filtrar", selection: self.$filter) {
        ...
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                if self.skill.disabled {
                    Text("Desativado na configuração")
                        .font(.caption)
                        .foregroundStyle(.secondary)
    ...
    private var missingSummary: some View {
        VStack(alignment: .leading, spacing: 4) {
            if self.shouldShowMissingBins {
                Text("Binários ausentes: \(self.missingBins.joined(separator: ", "))")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            if !self.missingEnv.isEmpty {
                Text("Ambiente ausente: \(self.missingEnv.joined(separator: ", "))")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            if !self.missingConfig.isEmpty {
                Text("Requer configuração: \(self.missingConfig.joined(separator: ", "))")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private var envActionRow: some View {
        HStack(spacing: 8) {
            ForEach(self.missingEnv, id: \.self) { envKey in
                ...
                Button(isPrimary ? "Definir Chave de API" : "Definir \(envKey)") {
                ...
                ForEach(self.installOptions, id: \.id) { (option: SkillInstallOption) in
                    HStack(spacing: 6) {
                        if self.showGatewayInstall {
                            Button("Instalar no Gateway") { self.onInstall(option, .gateway) }
                                .buttonStyle(.borderedProminent)
                                .disabled(self.isBusy)
                        }
                        if self.showGatewayInstall {
                            Button("Instalar neste Mac") { self.onInstall(option, .local) }
                                .buttonStyle(.bordered)
                                .disabled(self.isBusy)
                                .help(
                                    self.localInstallNeedsSwitch
                                        ? "Muda para o modo Local para instalar neste Mac."
                                        : "")
                        } else {
                            Button("Instalar neste Mac") { self.onInstall(option, .local) }
                                .buttonStyle(.borderedProminent)
...
private struct EnvEditorView: View {
    ...
    private var title: String {
        self.editor.isPrimary ? "Definir Chave de API" : "Definir Variável de Ambiente"
    }

    private var subtitle: String {
        "Habilidade: \(self.editor.skillName)"
    }
}
        ...
        await self.withBusy(skill.skillKey) {
            do {
                if target == .local, AppStateStore.shared.connectionMode != .local {
                    ...
                    self.statusMessage = "Mudou para o modo Local para instalar neste Mac"
                }
        ...
        await self.withBusy(skillKey) {
            do {
                _ = try await GatewayConnection.shared.skillsUpdate(
                    skillKey: skillKey,
                    enabled: enabled)
                self.statusMessage = enabled ? "Habilidade ativada" : "Habilidade desativada"
            } catch {
                self.statusMessage = error.localizedDescription
            }
            await self.refresh()
            ...
            do {
                if isPrimary {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        apiKey: value)
                    self.statusMessage = "Chave de API salva"
                } else {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        env: [envKey: value])
                    self.statusMessage = "Salvo \(envKey)"
                }
            } catch {
                self.statusMessage = error.localizedDescription
            }
