...
struct GeneralSettings: View {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        HStack {
                            ...
                            Picker("Tema", selection: .constant("Escuro")) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            Button {
                                NotificationCenter.default.post(name: .zeroSelectSettingsTab, object: SettingsTab.config)
                            } label: {
                                Label("Configurar Provedores de IA", systemImage: "cpu")
                            }
                            .buttonStyle(.link)
                        }
                    }
    ...
    private var gatewayInstallerCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            {
                Text("Instalado: \(gatewayVersion) · Requerido: \(required)")
                    .font(.caption)
                    .foregroundStyle(.secondary)
    ...
    private var healthCard: some View {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            ...
            if let snap = snapshot {
                ...
                Text("\(linkLabel) idade de auth: \(healthAgeString(linkAge))")
                    .font(.caption)
                    .foregroundStyle(.secondary)
...
extension GeneralSettings {
    ...
    private func formatSSHFailure(_ response: Response, target: String) -> String {
           ...
           trimmed.localizedCaseInsensitiveContains("host key verification failed")
        {
            ...
            return "Verificação SSH falhou: Falha na verificação da chave do host. Remova a chave antiga com " +
                "`ssh-keygen -R \(host)` e tente novamente."
        }
        if let trimmed, !trimmed.isEmpty {
            if let message = response.message, message.hasPrefix("exit ") {
                return "Verificação SSH falhou: \(trimmed) (\(message))"
            }
            return "Verificação SSH falhou: \(trimmed)"
        }
        if let message = response.message {
            return "Verificação SSH falhou (\(message))"
        }
        return "Falha na verificação SSH"
    }
...
private func healthAgeString(_ ms: Double?) -> String {
    guard let ms else { return "desconhecido" }
    return msToAge(ms)
}
