...
struct InstancesSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if self.store.instances.isEmpty {
                Text("Nenhuma instância reportada ainda.")
                    .foregroundStyle(.secondary)
            } else {
    ...
    private var header: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text("Instâncias Conectadas")
                    .font(.headline)
                Text("Últimos sinais de presença de nós Zero. Atualizados periodicamente.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
            Spacer()
    ...
    private func instanceRow(_ inst: InstanceInfo) -> some View {
        ...
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 8) {
                    Text(inst.host ?? "host desconhecido").font(.subheadline.bold())
                ...
                if !isGateway, self.shouldShowUpdateRow(inst) {
                    HStack(spacing: 8) {
                        ...
                        if let secs = inst.lastInputSeconds {
                            self.label(icon: "clock", text: "há \(secs)s")
                        }
        ...
        .help(inst.text)
        .contextMenu {
            Button("Copiar Resumo de Depuração") {
    ...
    private func presenceIndicator(_ inst: InstanceInfo) -> some View {
        ...
        .font(.caption)
        .help("Presença atualizada \(inst.ageDescription).")
        .accessibilityLabel("\(status.label) presença")
    }
    ...
    private func presenceStatus(for inst: InstanceInfo) -> (label: String, color: Color) {
        ...
        if ageSeconds <= 120 { return ("Ativo", .green) }
        if ageSeconds <= 300 { return ("Inativo", .yellow) }
        return ("Expirado", .gray)
    }
    ...
    private func presenceUpdateSourceShortText(_ reason: String) -> String? {
        ...
        case "disconnect":
            return "Desconectar"
        case "node-connected":
            return "Conexão de nó"
        case "node-disconnected":
            return "Desconexão de nó"
        case "launch":
    ...
    private func presenceUpdateSourceHelp(_ reason: String) -> String {
        let trimmed = reason.trimmingCharacters(in: .whitespacesAndNewlines)
        if trimmed.isEmpty {
            return "Por que esta entrada de presença foi atualizada pela última vez (marcador de depuração)."
        }
        return "Por que esta entrada de presença foi atualizada pela última vez (marcador de depuração). Bruto: \(trimmed)"
    }
}
