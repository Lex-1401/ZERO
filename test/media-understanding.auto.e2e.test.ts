...
import type { ZerobotConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("zero-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("zero-bin-sherpa-");
      const modelDir = await makeTempDir("zero-sherpa-model-");
      ...
      const cfg: ZerobotConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("zero-bin-whispercpp-");
      const modelDir = await makeTempDir("zero-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("zero-bin-gemini-");
      ...
      const cfg: ZerobotConfig = { tools: { media: { image: {} } } };
