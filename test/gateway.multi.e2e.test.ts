...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `zero-e2e-${name}-`));
  const configDir = path.join(homeDir, ".zero");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          ZERO_CONFIG_PATH: configPath,
          ZERO_STATE_DIR: stateDir,
          ZERO_GATEWAY_TOKEN: "",
          ZERO_GATEWAY_PASSWORD: "",
          ZERO_SKIP_CHANNELS: "1",
          ZERO_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        ZERO_GATEWAY_TOKEN: inst.gatewayToken,
        ZERO_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ZERO_GATEWAY_PORT: String(gwA.port),
          ZERO_GATEWAY_TOKEN: gwA.gatewayToken,
          ZERO_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ZERO_GATEWAY_PORT: String(gwB.port),
          ZERO_GATEWAY_TOKEN: gwB.gatewayToken,
          ZERO_GATEWAY_PASSWORD: "",
        }),
