...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ZERO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.ZERO_STATE_DIR = path.join(base, ".zero");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "zero-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".zero", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
