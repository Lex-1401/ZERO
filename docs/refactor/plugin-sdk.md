...
Delivery:
- Publish as `@zero/plugin-sdk` (or export from core under `zero/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `ZEROPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ZEROConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ZEROConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ZEROConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: ZEROConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ZEROConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ZEROConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@zero/plugin-sdk`.
- Add `api.runtime` to `ZEROPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `zeroRuntime: ">=2026.2.0"`).
