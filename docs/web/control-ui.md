...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/zero`)
...
If the page fails to load, start the Gateway first: `zero gateway`.
...
- Config: view/edit `~/.zero/zero.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. ZERO
...
```bash
zero gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
ZERO **blocks** Control UI connections without device identity.
...
```bash
ZERO_CONTROL_UI_BASE_PATH=/zero/ pnpm ui:build
```
