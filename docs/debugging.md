...
## Registro de Chunks Brutos

Para capturar **chunks brutos compatíveis com OpenAI** antes de serem processados em blocos, o ZERO expõe um registrador separado:

```bash
# Habilitar via env
ZERO_RAW_STREAM_PATH=~/.zero/logs/raw-openai-completions.jsonl
```

Se este caminho estiver definido, todas as respostas JSON parciais do provedor de modelo são salvas linha por linha em `~/.zero/logs/raw-openai-completions.jsonl`. Isso é crucial para depurar problemas de parsing de tokens de controle ou erros de streaming.
