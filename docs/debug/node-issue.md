...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ZERO
---

# Node + tsx "__name is not a function" crash

## Summary
Running ZERO via Node with `tsx` fails at startup with:

```
[zero] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `zero status` and `gateway:watch` worked.
