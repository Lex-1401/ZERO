---
summary: "Use MiniMax M2.1 in ZERO"
read_when:
  - You want MiniMax models in ZERO
...
Configure manually via `zero.json`:
...
- Use `zero models list` and `zero models set minimax/MiniMax-M2.1` to switch.
...
- Running `zero configure` and selecting **MiniMax M2.1**, or
