---
summary: "Use Venice AI privacy-focused models in ZERO"
read_when:
  - You want privacy-focused inference in ZERO
...
```bash
zero onboard --auth-choice venice-api-key
```
...
```bash
zero chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, ZERO shows all available Venice models. Pick based on your needs:
...
```bash
zero models set venice/claude-opus-45
zero models set venice/llama-3.3-70b
```

List all available models:

```bash
zero models list | grep venice
```

## Configure via `zero configure`

1. Run `zero configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ZERO automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
zero chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
zero chat --model venice/claude-opus-45

# Use uncensored model
zero chat --model venice/venice-uncensored

# Use vision model with image
zero chat --model venice/qwen3-vl-235b-a22b

# Use coding model
zero chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `zero models list` to see currently available models. Some models may be temporarily offline.
