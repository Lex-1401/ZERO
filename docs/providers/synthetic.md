---
summary: "Use Synthetic's Anthropic-compatible API in ZERO"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. ZERO registers it as the
...
```bash
zero onboard --auth-choice synthetic-api-key
```
...
Note: ZERO's Anthropic client appends `/v1` to the base URL, so use
