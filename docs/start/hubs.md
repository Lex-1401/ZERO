---
summary: "Hubs que vinculam a todos os documentos do ZERO"
read_when:
  - Você quer um mapa completo da documentação
---

# Hubs de Documentação

Use estes hubs para descobrir cada página, incluindo mergulhos profundos e documentos de referência que não aparecem na navegação esquerda.

## Comece por aqui

- [Índice](/)
- [Primeiros Passos](/start/getting-started)
- [Integração](/start/onboarding)
- [Assistente](/start/wizard)
- [Configuração](/start/setup)
- [Dashboard (Gateway local)](http://127.0.0.1:18789/)
- [Ajuda](/help)
- [Configuração](/gateway/configuration)
- [Exemplos de configuração](/gateway/configuration-examples)
- [Assistente ZERO (Zero)](/start/zero)
- [Showcase](/start/showcase)
- [Lore](/start/lore)

## Instalação + atualizações

- [Docker](/install/docker)
- [Nix](/install/nix)
- [Atualização / Rollback](/install/updating)
- [Fluxo Bun (experimental)](/install/bun)

## Conceitos fundamentais

- [Arquitetura](/concepts/architecture)
- [Hub de rede](/network)
- [Runtime do Agente](/concepts/agent)
- [Espaço de trabalho do Agente](/concepts/agent-workspace)
- [Memória](/concepts/memory)
- [Loop do Agente](/concepts/agent-loop)
- [Streaming + Fragmentação](/concepts/streaming)
- [Roteamento multiagente](/concepts/multi-agent)
- [Compactação](/concepts/compaction)
- [Sessões](/concepts/session)
- [Sessões (alias)](/concepts/sessions)
- [Poda de sessões](/concepts/session-pruning)
- [Ferramentas de sessão](/concepts/session-tool)
- [Fila](/concepts/queue)
- [Comandos de barra](/tools/slash-commands)
- [Adaptadores RPC](/reference/rpc)
- [Esquemas TypeBox](/concepts/typebox)
- [Gerenciamento de fuso horário](/concepts/timezone)
- [Presença](/concepts/presence)
- [Descoberta + transportes](/gateway/discovery)
- [Bonjour](/gateway/bonjour)
- [Roteamento de canais](/concepts/channel-routing)
- [Grupos](/concepts/groups)
- [Mensagens de grupo](/concepts/group-messages)
- [Failover de modelo](/concepts/model-failover)
- [OAuth](/concepts/oauth)

## Provedores + ingresso

- [Hub de canais de chat](/channels)
- [Hub de provedores de modelos](/providers/models)
- [WhatsApp](/channels/whatsapp)
- [Telegram](/channels/telegram)
- [Telegram (notas grammY)](/channels/grammy)
- [Slack](/channels/slack)
- [Discord](/channels/discord)
- [Mattermost](/channels/mattermost) (plugin)
- [Signal](/channels/signal)
- [iMessage](/channels/imessage)
- [Processamento de localização](/channels/location)
- [WebChat](/web/webchat)
- [Webhooks](/automation/webhook)
- [Gmail Pub/Sub](/automation/gmail-pubsub)

## Gateway + operações

- [Manual do Gateway](/gateway)
- [Pareamento do Gateway](/gateway/pairing)
- [Bloqueio do Gateway](/gateway/gateway-lock)
- [Processo em segundo plano](/gateway/background-process)
- [Saúde](/gateway/health)
- [Heartbeat](/gateway/heartbeat)
- [Doctor](/gateway/doctor)
- [Logging](/gateway/logging)
- [Sandboxing](/gateway/sandboxing)
- [Dashboard](/web/dashboard)
- [Controle UI](/web/control-ui)
- [Acesso remoto](/gateway/remote)
- [README do gateway remoto](/gateway/remote-gateway-readme)
- [Tailscale](/gateway/tailscale)
- [Segurança](/gateway/security)
- [Resolução de problemas](/gateway/troubleshooting)

## Ferramentas + automação

- [Superfície de ferramentas](/tools)
- [OpenProse](/prose)
- [Referência da CLI](/cli)
- [Ferramenta Exec](/tools/exec)
- [Modo elevado](/tools/elevated)
- [Cron jobs](/automation/cron-jobs)
- [Cron vs Heartbeat](/automation/cron-vs-heartbeat)
- [Thinking + Verbose](/tools/thinking)
- [Modelos](/concepts/models)
- [Subagentes](/tools/subagents)
- [CLI de envio do agente](/tools/agent-send)
- [UI de Terminal](/tui)
- [Controle de navegador](/tools/browser)
- [Navegador (resolução de problemas Linux)](/tools/browser-linux-troubleshooting)
- [Enquetes](/automation/poll)

## Nós, mídia, voz

- [Visão geral de nós](/nodes)
- [Câmera](/nodes/camera)
- [Imagens](/nodes/images)
- [Áudio](/nodes/audio)
- [Comando de localização](/nodes/location-command)
- [Despertar por voz](/nodes/voicewake)
- [Modo de conversa](/nodes/talk)

## Plataformas

- [Visão geral de plataformas](/platforms)
- [macOS](/platforms/macos)
- [iOS](/platforms/ios)
- [Android](/platforms/android)
- [Windows (WSL2)](/platforms/windows)
- [Linux](/platforms/linux)
- [Superfícies Web](/web)

## App acompanhante para macOS (avançado)

- [Configuração de dev macOS](/platforms/mac/dev-setup)
- [Barra de menu macOS](/platforms/mac/menu-bar)
- [Despertar por voz macOS](/platforms/mac/voicewake)
- [Sobreposição de voz macOS](/platforms/mac/voice-overlay)
- [WebChat macOS](/platforms/mac/webchat)
- [Canvas macOS](/platforms/mac/canvas)
- [Processo filho macOS](/platforms/mac/child-process)
- [Saúde macOS](/platforms/mac/health)
- [Ícone macOS](/platforms/mac/icon)
- [Logging macOS](/platforms/mac/logging)
- [Permissões macOS](/platforms/mac/permissions)
- [Acesso remoto macOS](/platforms/mac/remote)
- [Assinatura macOS](/platforms/mac/signing)
- [Lançamento macOS](/platforms/mac/release)
- [Gateway integrado macOS (launchd)](/platforms/mac/bundled-gateway)
- [XPC macOS](/platforms/mac/xpc)
- [Habilidades macOS](/platforms/mac/skills)
- [Peekaboo macOS](/platforms/mac/peekaboo)

## Espaço de Trabalho + templates

- [Habilidades](/tools/skills)
- [ZeroHub](/tools/zerohub)
- [Configuração de Habilidades](/tools/skills-config)
- [Padrão AGENTS](/reference/AGENTS.default)
- [Templates: AGENTS](/reference/templates/AGENTS)
- [Templates: BOOTSTRAP](/reference/templates/BOOTSTRAP)
- [Templates: HEARTBEAT](/reference/templates/HEARTBEAT)
- [Templates: IDENTITY](/reference/templates/IDENTITY)
- [Templates: SOUL](/reference/templates/SOUL)
- [Templates: TOOLS](/reference/templates/TOOLS)
- [Templates: USER](/reference/templates/USER)

## Experimentos (exploratório)

- [Protocolo de configuração de integração](/experiments/onboarding-config-protocol)
- [Notas de endurecimento do Cron](/experiments/plans/cron-add-hardening)
- [Notas de endurecimento da política de grupo](/experiments/plans/group-policy-hardening)
- [Pesquisa: memória](/experiments/research/memory)
- [Exploração de configuração de modelo](/experiments/proposals/model-config)

## Testes + lançamento

- [Testes](/reference/test)
- [Checklist de lançamento](/reference/RELEASING)
- [Modelos de dispositivos](/reference/device-models)
