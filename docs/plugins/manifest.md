...
read_when:
  - You are building a ZERO plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (zero.plugin.json)

Every plugin **must** ship a `zero.plugin.json` file in the **plugin root**.
ZERO uses this manifest to validate configuration **without executing plugin
