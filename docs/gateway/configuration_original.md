---
summary: "All configuration options for ~/.zero/zero.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

ZERO reads an optional **JSON5** config from `~/.zero/zero.json` (comments + trailing commas allowed).

If the file is missing, ZERO uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/zero`). You usually only need a config to:
...
ZERO only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `zero doctor`, `zero logs`, `zero health`, `zero status`, `zero service`, `zero help`).
- Run `zero doctor` to see the exact issues.
- Run `zero doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `zero config set`. Keep a backup of `~/.zero/zero.json`.
...
```bash
zero gateway call config.get --params '{}' # capture payload.hash
...
```json5
{
  agents: {
    ...
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@zero", "reisponde"] }
      }
    ]
  },
  channels: {
...
{ "$include": "/etc/zero/base.json5" } // absolute
...
ZERO reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, ZERO runs your login shell and imports only the missing expected keys (never overrides).
...
ZERO stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.zero/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.zero/credentials/oauth.json` (or `$ZERO_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.zero/agent/*` (migrated by `zero doctor` into `~/.zero/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `ZERO_OAUTH_DIR`
- Agent dir (default agent root override): `ZERO_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, ZERO imports `oauth.json` entries into `auth-profiles.json`.
...
If set, ZERO derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/zero/zero-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/zero/zero.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.zero/credentials/whatsapp/biz
...
- The legacy single-account Baileys auth dir is migrated by `zero doctor` into `whatsapp/default`.
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@zero"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/zero-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.zero/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/zero-home" },
      { id: "work", workspace: "~/zero-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `zero.json`).
...
ZERO starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["zero-dm"]          // optional group DM allowlist
      },
      guilds: {
...
ZERO starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `zero plugins install @zero/mattermost` (or `./extensions/mattermost` from a git checkout).
...
ZERO starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
ZERO spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, ZERO
...
When a file exceeds this limit, ZERO logs a warning and injects a truncated
...
message envelopes). If unset, ZERO uses the host timezone at runtime.
...
`[zero]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[zero]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[zero]"`, ZERO will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, ZERO generates audio
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"Zero"`).
...
ZERO also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, ZERO currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `zero configure --section web`, or use `BRAVE_API_KEY` env var)
...
Provider keys accept either `provider` (e.g. `google-cloud-auth`) or `provider/model`
(e.g. `openai/gpt-5.2`).

Example (keep global coding profile, but minimal tools for Google Google Cloud Auth):
```json5
{
  tools: {
    profile: "coding",
...
- `group:nodes`: `nodes`
- `group:zero`: all built-in ZERO tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.zero/sandboxes`
...
ZERO uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.zero/agents/<agentId>/agent/models.json` or by defining the same schema inside your
ZERO config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, ZERO writes/merges a `models.json` into
`~/.zero/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. ZERO uses
...
- Shortcut: `zero onboard --auth-choice opencode-zen`.
...
Shortcut: `zero onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `zero onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `zero onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `zero onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `zero onboard --auth-choice minimax-api`.
...
- Override the agent config root with `ZERO_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.zero/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.zero/agents/<agentId>/sessions/sessions.json
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, ZERO stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.zero/skills` (workspace skills
...
from `~/.zero/extensions`, `<workspace>/.zero/extensions`, plus any
...
### `browser` (zero-managed browser)

ZERO can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for zero and expose a small loopback control service.
...
- profile color: `#FF4500` (void-orange)
- Note: the control server is started by the running gateway (ZERO.app menubar, or `zero gateway`).
...
- When a connection comes from one of these IPs, ZERO uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `zero gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `ZERO_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `ZERO_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. ZERO
...
macOS app behavior:
- ZERO.app watches `~/.zero/zero.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.zero/zero.json` (or `ZERO_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.zero/zero.json` (or `ZERO_CONFIG_PATH`)
...
- `ZERO_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `zero --dev …` → uses `~/.zero-dev` + shifts ports from base `19001`
- `zero --profile <name> …` → uses `~/.zero-<name>` (port via config/env/flags)
...
Gmail helper config (used by `zero webhooks gmail setup` / `run`):
...
- Set `ZERO_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, ZERO defaults `serve.path` to `/` so
...
Default root: `~/zero/canvas`  
Default port: `18793` (chosen to avoid the zero browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__zero/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__zero__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.zero/bridge/tls/bridge-{cert,key}.pem when omitted.
...
Controls LAN mDNS discovery broadcasts (`_zero-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_zero-bridge._tcp` under `~/.zero/dns/` using the standard discovery domain `zero.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `zero.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `zero.internal` via that server
