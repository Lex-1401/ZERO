...
read_when:
  - You installed ZERO but `zero` is “command not found”
...
ZERO’s runtime baseline is **Node 22+**.

If you can run `npm install -g zero@latest` but later see `zero: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
