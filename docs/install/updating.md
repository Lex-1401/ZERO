---
summary: "Updating ZERO safely (global install or source), plus rollback strategy"
read_when:
  - Updating ZERO
...
ZERO is moving fast (pre “1.0”). Treat updates like shipping infra: update → run checks → restart (or use `zero update`, which restarts) → verify.

## Recommended: re-run the website installer (upgrade in place)

The **preferred** update path is to re-run the installer from the website. It
detects existing installs, upgrades in place, and runs `zero doctor` when
needed.
...
- Snapshot your tailoring:
  - Config: `~/.zero/zero.json`
  - Credentials: `~/.zero/credentials/`
  - Workspace: `~/zero`
...
- Installs deps, builds, builds the Control UI, and runs `zero doctor`.
- Restarts the gateway by default (use `--no-restart` to skip).

If you installed via **npm/pnpm** (no git metadata), `zero update` will try to update via your package manager. If it can’t detect the install, use “Update (global install)” instead.
...
- Switching between global and git installs is easy: install the other flavor, then run `zero doctor` so the gateway service entrypoint is rewritten to the current install.

## Always run: `zero doctor`
...
- Detect and migrate older gateway services (launchd/systemd; legacy schtasks) to current ZERO services.
...
- Windows (WSL2): `systemctl --user restart zero-gateway[-<profile>].service`
  - `launchctl`/`systemctl` only work if the service is installed; otherwise run `zero gateway install`.
...
```bash
npm i -g zero@<version>
```

```bash
pnpm add -g zero@<version>
```

Tip: to see the current published version, run `npm view zero version`.
