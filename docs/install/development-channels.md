...
ZERO ships three update channels:
...
When you **explicitly** switch channels with `--channel`, ZERO also aligns
the install method:

- `dev` ensures a git checkout (default `~/zero`, override with `ZERO_GIT_DIR`),
...
When you switch channels with `zero update`, ZERO also syncs plugin sources:
