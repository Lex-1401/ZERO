---
summary: "Run ZERO Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Tip: keep this VM **stateful**. ZERO stores state under `~/.zero/` and `~/zero/`.
...
```bash
ssh -N -L 18789:127.0.0.1:18789 zero.exe.xyz
```
...
```bash
export ZERO_GATEWAY_TOKEN="$(openssl rand -hex 32)"
zero gateway --bind lan --port 8080 --token "$ZERO_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.zero/zero.json`:
...
```bash
ssh exe.dev share port zero 8080
```
...
On Linux, ZERO uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status zero-gateway[-<profile>].service
```
