...
ZERO features that can generate provider usage or paid API calls.
...
- `zero status --usage` and `zero channels list` show provider **usage windows**
...
ZERO can pick up credentials from:
...
### 8) Model scan / probe
`zero models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
