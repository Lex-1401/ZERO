...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "ZERO_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "zero-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ZERO_STATE_DIR = path.join(home, ".zero");
    ...
    await fs.mkdir(path.join(home, ".zero"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.zero${path.sep}media`);
