...
const ETC_ZERO_DIR = path.join(ROOT_DIR, "etc", "zero");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "zero.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcZEROPath(...parts: string[]) {
  return path.join(ETC_ZERO_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcZEROPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "zero.json"))).toEqual({
      shared: true,
    });
  });
});
