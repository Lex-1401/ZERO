...
export type BrowserConfig = {
  enabled?: boolean;
  /** Base URL of the zero browser control server. Default: http://127.0.0.1:18791 */
  controlUrl?: string;
  /**
   * Shared token for the browser control server.
   * If set, clients must send `Authorization: Bearer <token>`.
   *
   * Prefer `ZERO_BROWSER_CONTROL_TOKEN` env for ephemeral setups; use this for "works after reboot".
   */
  controlToken?: string;
  ...
  /** Accent color for the zero browser profile (hex). Default: #FF4500 */
  color?: string;
