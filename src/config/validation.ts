...
import {
  ...
  applyModelDefaults,
  applyPathTildeExpansion,
  applySessionDefaults,
  applyTalkApiKey,
} from "./defaults.js";
import { findLegacyConfigIssues } from "./legacy.js";
import type { ZEROConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ZEROConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: ZEROConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ZEROSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ZEROConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ZEROConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const config = validated.data as ZEROConfig;
  return {
    ok: true,
    config: applyPathTildeExpansion(
      applyContextPruningDefaults(
        applyCompactionDefaults(
          applyTalkApiKey(
            applyLoggingDefaults(
              applyModelDefaults(
                applyAgentDefaults(applyMessageDefaults(applySessionDefaults(config)))
              )
            )
          )
        )
