...
export function parseZEROVersion(raw: string | null | undefined): ZEROVersion | null {
  if (!raw) return null;
...
export function compareZEROVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseZEROVersion(a);
  const parsedB = parseZEROVersion(b);
