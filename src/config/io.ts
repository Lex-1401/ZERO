...
import type { ZEROConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareZEROVersions } from "./version.js";
...
function coerceConfig(value: unknown): ZEROConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ZEROConfig;
}
...
function stampConfigVersion(cfg: ZEROConfig): ZEROConfig {
...
function warnIfConfigFromFuture(cfg: ZEROConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareZEROVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ZERO (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: ZEROConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ZEROConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ZEROConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ZEROConfig) {
    clearConfigCache();
