  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.zero/logs/zero.log" },
        hooks: {
          path: "~/.zero/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".zero", "logs", "zero.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".zero", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".zero", "telegram.token"),
      );
