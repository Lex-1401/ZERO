...
function expandTilde(val: any): any {
  if (typeof val === "string" && val.startsWith("~")) {
    return resolveUserPath(val);
  }
  return val;
}

export function applyPathTildeExpansion(cfg: ZEROConfig): ZEROConfig {
  if (!cfg) return cfg;
  const next = { ...cfg };
  ...
  if (next.plugins?.load?.paths) {
    next.plugins.load = {
      ...next.plugins.load,
      paths: next.plugins.load.paths.map(expandTilde)
    };
  }

  if (next.agents?.defaults?.workspace) {
    next.agents.defaults.workspace = expandTilde(next.agents.defaults.workspace);
  }

  if (next.agents?.list) {
    next.agents.list = next.agents.list.map(agent => ({
      ...agent,
      workspace: agent.workspace ? expandTilde(agent.workspace) : agent.workspace,
      agentDir: agent.agentDir ? expandTilde(agent.agentDir) : agent.agentDir,
      sandbox: agent.sandbox ? {
        ...agent.sandbox,
        workspaceRoot: agent.sandbox.workspaceRoot ? expandTilde(agent.sandbox.workspaceRoot) : agent.sandbox.workspaceRoot
      } : agent.sandbox
    }));
  }

  if (next.channels?.whatsapp?.accounts) {
    for (const id of Object.keys(next.channels.whatsapp.accounts)) {
      const acc = next.channels.whatsapp.accounts[id];
      if (acc?.authDir) acc.authDir = expandTilde(acc.authDir);
    }
  }
...
function resolveAnthropicDefaultAuthMode(cfg: ZEROConfig): AnthropicAuthDefaultsMode | null {
...
export function applyMessageDefaults(cfg: ZEROConfig): ZEROConfig {
...
export function applyTalkApiKey(config: ZEROConfig): ZEROConfig {
...
export function applyModelDefaults(cfg: ZEROConfig): ZEROConfig {
  let mutated = false;
  let nextCfg = cfg;
...
export function applyAgentDefaults(cfg: ZEROConfig): ZEROConfig {
...
export function applyLoggingDefaults(cfg: ZEROConfig): ZEROConfig {
...
export function applyContextPruningDefaults(cfg: ZEROConfig): ZEROConfig {
  const agents = cfg.agents ?? {};
  const defaults = agents.defaults ?? {};
...
export function applyCompactionDefaults(cfg: ZEROConfig): ZEROConfig {
