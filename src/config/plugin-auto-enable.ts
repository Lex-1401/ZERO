...
const PROVIDER_PLUGIN_IDS: Array<{ pluginId: string; providerId: string }> = [
  { pluginId: "google-cloud-auth-auth", providerId: "google-cloud-auth" },
...
function resolveChannelConfig(cfg: ZEROConfig, channelId: string): Record<string, unknown> | null {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  const entry = channels?.[channelId];
  return isRecord(entry) ? entry : null;
}

function isTelegramConfigured(cfg: ZEROConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: ZEROConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: ZEROConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: ZEROConfig): boolean {
...
function isIMessageConfigured(cfg: ZEROConfig): boolean {
...
function isWhatsAppConfigured(cfg: ZEROConfig): boolean {
...
function isGenericChannelConfigured(cfg: ZEROConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: ZEROConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ZEROConfig, providerId: string): boolean {
  ...
  if (profiles && typeof profiles === "object") {
    for (const profile of Object.values(profiles)) {
      ...
      const providerStr = typeof profile.provider === "string" ? profile.provider : "";
...
function resolveConfiguredPlugins(cfg: ZEROConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: ZEROConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: ZEROConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: ZEROConfig, pluginId: string): ZEROConfig {
...
function enablePluginEntry(cfg: ZEROConfig, pluginId: string): ZEROConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
