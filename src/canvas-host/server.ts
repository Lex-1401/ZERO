...
(() => {
  ...
  const hasIOS = () => !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.zeroCanvasA2UIAction);
  const hasAndroid = () => !!(window.zeroCanvasA2UIAction && typeof window.zeroCanvasA2UIAction.postMessage === "function");
  const hasHelper = () => typeof window.zeroSendUserAction === "function";
  
  // HIGH-002: Correção de XSS - Usar DOM manipulation ao invés de innerHTML
  statusEl.textContent = ""; // Limpar conteúdo
  ...
  window.addEventListener("zero:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ZERO node canvas.");
      return;
    }
    const ok = window.zeroSendUserAction({
      name,
      surfaceId: "main",
      sourceComponentId,
      context: { t: Date.now() },
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ZERO_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "zero", "canvas"));
    ...
    try {
      ...
      if (!filePath) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ZERO Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
