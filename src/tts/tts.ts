...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.ZERO_TTS_PREFS?.trim();
...
export function buildTtsSystemPromptHint(cfg: ZEROConfig): string | undefined {
...
function resolveSummaryModelRef(cfg: ZEROConfig, config: ResolvedTtsConfig): SummaryModelSelection {
...
export async function listElevenLabsVoices(params: {
  ...
  const url = `${normalizeElevenLabsBaseUrl(baseUrl)}/v1/voices`;
