...
describe("createZEROCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from zero read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "zero-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "zero-workspace"),
    ...
    const tools = createZEROCodingTools({ sandbox });
