...
/**
 * Cleanup suspended ZERO CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: ZEROConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-cli-images-"));
  const paths: string[] = [];
