...
import { wrapToolWithInterceptors } from "./tools/interceptor.js";
...
export function createZEROTools(options?: {
  ...
  const interceptorCtx = {
    agentId: resolveSessionAgentId({ sessionKey: options?.agentSessionKey, config: options?.config }),
    sessionKey: options?.agentSessionKey
  };

  return allTools.map(t => wrapToolWithInterceptors(t, interceptorCtx));
}
