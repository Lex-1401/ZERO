...
describe("sanitizeSessionHistory (google thinking)", () => {
  ...
  it("drops unsigned thinking blocks for GoogleCloudAuth Claude", async () => {
    const sessionManager = SessionManager.inMemory();
    const input = [
      {
        role: "user",
    ...
    const out = await sanitizeSessionHistory({
      ...
      sessionManager,
      sessionId: "session:google-cloud-auth-claude",
    });
  ...
  it("maps base64 signatures to thinkingSignature for GoogleCloudAuth Claude", async () => {
    const sessionManager = SessionManager.inMemory();
    const input = [
      {
        role: "user",
