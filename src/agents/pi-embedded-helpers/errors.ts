...
export function formatRawAssistantErrorForUi(raw?: string): string {
  const trimmed = (raw ?? "").trim();
  if (!trimmed) return "A solicitação LLM falhou com um erro desconhecido.";
...
export function formatAssistantErrorText(
  ...
  opts?: { cfg?: ZEROConfig; sessionKey?: string },
): string | undefined {
  // Also format errors if errorMessage is present, even if stopReason isn't "error"
  const raw = (msg.errorMessage ?? "").trim();
  if (msg.stopReason !== "error" && !raw) return undefined;
  if (!raw) return "A solicitação LLM falhou com um erro desconhecido.";
  ...
  if (isContextOverflowError(raw)) {
    return (
      "Estouro de contexto: prompt muito grande para o modelo. " +
      "Tente novamente com menos entrada ou um modelo de contexto maior."
    );
  }
  ...
  if (
    ...
    return (
      "Conflito de ordenação de mensagens - tente novamente. " +
      "Se isso persistir, use /new para iniciar uma nova sessão."
    );
  }

  const invalidRequest = raw.match(/"type":"invalid_request_error".*?"message":"([^"]+)"/);
  if (invalidRequest?.[1]) {
    return `Solicitação LLM rejeitada: ${invalidRequest[1]}`;
  }

  if (isOverloadedErrorMessage(raw)) {
    return "O serviço de IA está temporariamente sobrecarregado. Tente novamente em um momento.";
  }
...
export function sanitizeUserFacingText(text: string): string {
  ...
  if (ERROR_PREFIX_RE.test(trimmed)) {
    ...
    if (isTimeoutErrorMessage(trimmed)) {
      return "A solicitação LLM expirou.";
    }
    return formatRawAssistantErrorForUi(trimmed);
  }
