...
function resolveSafeBins(_bins: any) { return []; }
function requiresExecApproval(_opts: any) { return false; }
import {
...
export function createExecTool(defaults?: ExecToolDefaults): AgentTool<any, ExecToolDetails> {
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      let security: any = requestedSecurity ?? configuredSecurity;
      if (elevatedRequested && elevatedMode === "full") security = "full";

      const configuredAsk = defaults?.ask ?? "on-miss";
      const requestedAsk = normalizeExecAsk(params.ask);
      let ask: any = requestedAsk ?? configuredAsk;
