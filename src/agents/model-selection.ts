...
// removed normalizeGoogleModelId
...
export function isCliProvider(provider: string, cfg?: ZEROConfig): boolean {
...
export function resolveConfiguredModelRef(params: {
  ...
  if (rawModel) {
    ...
    if (!trimmed.includes("/")) {
      ...
      console.warn(
        `[zero] Model "${trimmed}" specified without provider. Falling back to "anthropic/${trimmed}". Please use "anthropic/${trimmed}" in your config.`,
      );
...
export async function routeModel(params: {
  ...
  // If specific model requested, respect it.
  if (params.provider && params.model) {
    return { provider: params.provider, model: params.model };
  }

  // Heuristic: Simple tasks go to local model (Ollama) if available.
  // "Simple" = Short (< 200 chars), no code blocks, no complex keywords.
  const isSimple =
    params.prompt.length < 200 &&
    !params.prompt.includes("```") &&
    !/complex|analyze|reason|code|programming|refactor/i.test(params.prompt);

  if (isSimple) {
    try {
      const localModels = await discoverOllamaModels();
      // Look for llama3 or similar lightweight models
      const localCandidate = localModels.find(
        (m) =>
      ...
      if (localCandidate) {
        return { provider: "ollama", model: localCandidate.id };
      }
    } catch {
      // Ignore discovery errors, fall back to default
    }
  }

  const dProvider = (params.provider ?? DEFAULT_PROVIDER).trim() || DEFAULT_PROVIDER;
  const dModel = (params.model ?? DEFAULT_MODEL).trim() || DEFAULT_MODEL;

  return { provider: dProvider, model: dModel };
}
