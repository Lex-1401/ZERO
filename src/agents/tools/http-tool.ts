...
export function createHttpTool(options?: {
    ...
    return {
        label: "HTTP Client",
        name: "http_client",
        description: "Make HTTP requests with secure auth profiles. Use this to interact with external APIs natively without leaking your context keys.",
        ...
        execute: async (_toolCallId, args) => {
            ...
            if (authProfile) {
                ...
                if (cred.type === "api_key") {
                    ...
                    const token = oauthCred.accessToken ?? oauthCred.access_token ?? oauthCred.idToken ?? oauthCred.id_token;
                    if (token) {
