...
type WebSearchConfig = NonNullable<ZEROConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ZEROConfig): WebSearchConfig {
...
async function runWebSearch(params: {
  ...
  if (params.provider === "perplexity") {
    ...
    const flexPayload = payload as Record<string, unknown>;
    if (params.config) {
      const sentinel = new SentinelAgent({ config: params.config });
      flexPayload.content = await sentinel.summarize(content, `Perplexity Search: ${params.query}`);
      flexPayload.sentinel = true;
    }

    writeCache(SEARCH_CACHE, cacheKey, flexPayload, params.cacheTtlMs);
    return flexPayload;
  }
  ...
  if (params.config) {
    const sentinel = new SentinelAgent({ config: params.config });
    const rawContext = mapped.map((r) => `[${r.title}](${r.url}): ${r.description}`).join("\n\n");
    const summary = await sentinel.summarize(rawContext, `Brave Search: ${params.query}`);
    flexPayload.summary = summary;
    // For security, we might want to hide the raw descriptions and only show the summary + titles/links
    flexPayload.results = mapped.map((r) => ({
      title: r.title,
      url: r.url,
