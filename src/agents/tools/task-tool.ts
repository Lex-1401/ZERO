...
const TaskUpdateSchema = Type.Object({
  ...
  name: Type.Optional(
    Type.String({ description: "Nome legível da tarefa (obrigatório para create)." }),
  ),
  ...
  progress: Type.Optional(
    Type.Number({ description: "Progresso de 0 a 100.", minimum: 0, maximum: 100 }),
  ),
...
export function createTaskTool(opts: { agentSessionKey: string }): AnyAgentTool {
  return {
    ...
    name: "task_progress",
    description:
      "Gerencia a visibilidade de progresso de tarefas longas no TaskBoard do usuário. Use para reportar status de missões complexas.",
