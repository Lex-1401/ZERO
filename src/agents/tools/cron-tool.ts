...
const CRON_ACTIONS = ["status", "list", "add", "update", "remove", "run", "runs", "wake", "schedule_objective"] as const;
...
const CronToolSchema = Type.Object({
  ...
  prompt_payload: Type.Optional(Type.String()),
});
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        ...
        case "schedule_objective": {
          const expr = readStringParam(params, "cron_expression", { required: true });
          const payloadText = readStringParam(params, "prompt_payload", { required: true });
          const agentId = opts?.agentSessionKey
            ? resolveSessionAgentId({ sessionKey: opts.agentSessionKey, config: loadConfig() })
            : undefined;

          const newJob = {
            name: `Objective: ${truncateText(payloadText, 30)}`,
            description: payloadText,
            agentId,
            schedule: { kind: "cron", expr },
            payload: { kind: "agentTurn", message: payloadText },
            wakeMode: "now"
          };

          const normalizedJob = normalizeCronJobCreate(newJob) ?? newJob;
          return jsonResult(await callGatewayTool("cron.add", gatewayOpts, normalizedJob));
        }
        default:
