...
const CdpBrowserSchema = Type.Object({
    ...
    expression: Type.Optional(Type.String({ description: "JS expression to evaluate (required for 'evaluate')." })),
});
...
export function createCdpBrowserTool(): AnyAgentTool {
    return {
        name: "cdp_browser",
        label: "CDP Browser",
        description: "Minimalist Chrome DevTools Protocol (CDP) client. Lightweight alternative to Playwright for simple navigation, snapshotting, and JS evaluation.",
        ...
        execute: async (_toolCallId, args) => {
            ...
            try {
                ...
                if (action === "snapshot") {
                    ...
                    return jsonResult({ snapshotLength: String(result?.data).length, note: "MHTML Snapshot extracted." });
                }
                if (action === "evaluate") {
