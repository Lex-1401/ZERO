...
function wrapIntegrityGuard(tool: AnyAgentTool): AnyAgentTool {
  return {
    ...tool,
    execute: async (toolCallId, args, signal, onUpdate) => {
      const normalized = normalizeToolParams(args);
      ...
      if (typeof filePath === "string" && SecurityGuard.isProtectedPath(filePath)) {
        if (record?.allow_sensitive_write !== true) {
          throw new Error(
            `SECURITY ALERT: Tentativa de modificação de arquivo de infraestrutura protegido (${filePath}). ` +
              `Esta ação exige confirmação explícita do usuário. Se isso for intencional, pergunte ao usuário ` +
              `e, se ele autorizar, use o parâmetro 'allow_sensitive_write: true' na próxima chamada.`,
          );
        }
      }
...
export function createZEROReadTool(base: AnyAgentTool): AnyAgentTool {
