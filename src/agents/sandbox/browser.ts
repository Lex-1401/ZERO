...
import { DEFAULT_ZERO_BROWSER_COLOR } from "../../browser/constants.js";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_ZERO_BROWSER_COLOR,
    ...
    defaultProfile: "zero",
    profiles: {
      zero: { cdpPort: params.cdpPort, color: DEFAULT_ZERO_BROWSER_COLOR },
    },
  };
}
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    args.push("-e", `ZERO_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `ZERO_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `ZERO_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `ZERO_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `ZERO_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing ? resolveProfile(existing.bridge.state.resolved, "zero") : null;
  const shouldReuse =
