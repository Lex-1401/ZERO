...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "zero.sandbox=1");
  args.push("--label", `zero.sessionKey=${params.scopeKey}`);
  args.push("--label", `zero.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `zero.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "zero.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`zero sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`zero sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("zero sandbox recreate --all");
}
