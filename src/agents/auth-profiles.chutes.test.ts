...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.ZERO_AGENT_DIR;
    else process.env.ZERO_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-chutes-"));
    process.env.ZERO_STATE_DIR = tempDir;
    process.env.ZERO_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
