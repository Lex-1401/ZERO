...
import { spawnWithFallback, formatSpawnError } from "../process/spawn-utils.js";
...
import { maybeNotifyOnExit } from "./bash-tools.exec.utils.js";
import { AgentToolResult } from "@mariozechner/pi-agent-core";
import { BashSandboxConfig } from "./bash-tools.shared.js";

/**
 * Executa um processo shell com suporte a sandboxing (Docker), PTY e timeouts.
 */
export async function runExecProcess(opts: {
    ...
    if (opts.sandbox) {
        ...
        stdin = child.stdin as unknown as SessionStdin;
    } else if (opts.usePty) {
    ...
    const promise = new Promise<ExecProcessOutcome>((resolve) => {
        ...
        if (pty) {
            pty.onExit((event) => {
                ...
                handleExit(event.exitCode ?? null, normalizedSignal as any);
            });
        } else if (child) {
