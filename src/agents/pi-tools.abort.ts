...
export function wrapToolWithAbortSignal(
  ...
  return {
    ...
    execute: async (toolCallId, params, signal, onUpdate) => {
      // LLM01: Panic Mode Circuit Breaker
      if (isPanicMode()) {
        return {
          content: [
            {
              type: "text",
              text: "⚠️ MODO PÂNICO ATIVADO: A execução de ferramentas foi bloqueada preventivamente para proteger a integridade do sistema.",
            },
          ],
          details: { status: "panic" },
          isError: true,
        };
      }
