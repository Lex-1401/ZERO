...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "zero-models-" });
}

const MODELS_CONFIG: ZEROConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureZEROModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureZEROModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureZEROModelsJson } = await import("./models-config.js");
      const { resolveZEROAgentDir } = await import("./agent-paths.js");

      await ensureZEROModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveZEROAgentDir(), "models.json");
