...
function isValidGoogleCloudAuthSignature(value: unknown): value is string {
  if (typeof value !== "string") return false;
  const trimmed = value.trim();
  if (!trimmed) return false;
  if (trimmed.length % 4 !== 0) return false;
  return SIGNATURE_RE.test(trimmed);
}

function sanitizeGoogleCloudAuthThinkingBlocks(messages: AgentMessage[]): AgentMessage[] {
  let touched = false;
  ...
  for (const msg of messages) {
    ...
    for (const block of assistant.content) {
      ...
      if (!isValidGoogleCloudAuthSignature(candidate)) {
        contentChanged = true;
        continue;
      }
...
>(params: {
  ...
  if (params.provider !== "google-cloud-auth" && params.provider !== "google-gemini-cli") {
    return params.tools;
  }
...
export async function sanitizeSessionHistory(params: {
  ...
  const sanitizedThinking = policy.normalizeGoogleCloudAuthThinkingBlocks
    ? sanitizeGoogleCloudAuthThinkingBlocks(sanitizedImages)
    : sanitizedImages;
