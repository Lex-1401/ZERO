...
import { normalizeProviderId, routeModel } from "../model-selection.js";
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      // Hybrid Routing: Determine provider/model based on prompt complexity
      const routed = await routeModel({
        prompt: params.prompt,
        provider: params.provider ? params.provider.trim() : undefined,
        model: params.model ? params.model.trim() : undefined,
      });
      ...
      await ensureZEROModelsJson(params.config, agentDir);
      ...
      try {
        while (true) {
          ...
          // Treat timeout as potential rate limit (GoogleCloudAuth hangs on rate limit)
          ...
          if (usage && usage.total) {
            const { telemetry } = await import("../../infra/telemetry.js");
            telemetry.recordTokens(agentMeta.model, usage.total);
            telemetry.recordLatency(duration);
          }
