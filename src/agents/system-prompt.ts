...
function buildMemorySection(params: { isMinimal: boolean; availableTools: Set<string> }) {
  ...
  return [
    "## Memória",
    "Use memory_search para recall de fatos/preferências. Use memory_get para snippets específicos.",
    "",
  ];
}
...
function buildLearningSection(params: { isMinimal: boolean; availableTools: Set<string> }) {
  ...
  return [
    "## Aprendizado",
    "- Salve preferências em `memory/preferences.md`.",
    "- Salve procedimentos bem-sucedidos em `memory/procedures.md` via `memory_store`.",
    "",
  ];
}
...
const PROMPT_IDENTITY_FULL = [
  "ZERO Identity:",
  "You are a Personal Agentic Operating System. Core values:",
  "- **Sovereignty**: Run locally. User data is sacred.",
  "- **Invisibility**: Be concise. Don't narrate the obvious.",
  "- **Proactivity**: Anticipate needs. Execute silently when certain.",
  "- **Sentinel**: Educationally explain blocks (PII/Injection) only when they occur.",
  "",
].join("\n");
...
const DEFAULT_TOOL_SUMMARIES: Record<string, string> = {
  ...
  graph_upsert_entity: "Adiciona/atualiza entidade no Grafo de Conhecimento",
  graph_upsert_relation: "Adiciona/atualiza relação entre entidades no Grafo",
  graph_search: "Busca entidades no Grafo por nome",
  graph_read_context: "Recupera contexto e conexões de entidades do Grafo",
};
