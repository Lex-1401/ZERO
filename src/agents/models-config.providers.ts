...
import { discoverMlxModels, MLX_BASE_URL } from "./mlx-models.js";

type ModelsConfig = NonNullable<ZEROConfig["models"]>;
...
async function buildMlxProvider(): Promise<ProviderConfig> {
  const models = await discoverMlxModels();
  return {
...
export async function resolveImplicitProviders(params: {
  ...
  // Ollama provider - auto-discover and enable even without explicit key
  const ollamaModels = await discoverOllamaModels();
  if (ollamaModels.length > 0) {
    const ollamaKey =
      resolveEnvApiKeyVarName("ollama") ??
      resolveApiKeyFromProfiles({ provider: "ollama", store: authStore }) ??
      "ollama"; // Default placeholder for local execution
    ...
    // Fallback: if user explicitly configured a key but we couldn't discover models
    // (maybe offline or different URL), we still attempt to add it if configured.
    const ollamaKey =
      resolveEnvApiKeyVarName("ollama") ??
      resolveApiKeyFromProfiles({ provider: "ollama", store: authStore });
    if (ollamaKey) {
  ...
  // MLX provider - auto-discover
  const mlxModels = await discoverMlxModels();
...
export async function resolveImplicitCopilotProvider(params: {
  ...
  // Our Copilot auth lives in ZERO's auth-profiles store instead, so we also
  // write a runtime-only auth.json entry for pi-coding-agent to pick up.
  //
  // This is safe because it's (1) within ZERO's agent dir, (2) contains the
  // GitHub token (not the exchanged Copilot token), and (3) matches existing
  // patterns for OAuth-like providers in pi-coding-agent.
  // Note: we deliberately do not write pi-coding-agent's `auth.json` here.
  // ZERO uses its own auth store and exchanges tokens at runtime.
  // `models list` uses ZERO's auth heuristics for availability.
