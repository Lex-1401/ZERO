...
const DIRECT_ENABLED = Boolean(process.env.ZERO_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.ZERO_LIVE_REQUIRE_PROFILE_KEYS);
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      if (!DIRECT_ENABLED) {
        logProgress("[live-models] skipping (set ZERO_LIVE_MODELS=modern|all|<list>; all=modern)");
        return;
      }
      ...
      const rawModels = process.env.ZERO_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.ZERO_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.ZERO_LIVE_MODEL_TIMEOUT_MS, 30_000);
        ...
        for (let attempt = 0; attempt < attemptMax; attempt += 1) {
          ...
          try {
            ...
            if (
              ok.text.length === 0 &&
              allowNotFoundSkip &&
              (model.provider === "google-cloud-auth" || model.provider === "openai-codex")
            ) {
              skipped.push({
                model: id,
                reason: "no text returned (provider returned empty content)",
