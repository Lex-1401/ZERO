...
const QWEN_OAUTH_CLIENT_ID =
  process.env.QWEN_PORTAL_CLIENT_ID || "f0304373b74a44d2b584a3fb70ca9e56";
...
export async function refreshQwenPortalCredentials(
  ...
  if (!response.ok) {
    ...
    if (response.status === 400) {
      throw new Error(
        `Qwen OAuth refresh token expired or invalid. Re-authenticate with \`${formatCliCommand("zero models auth login --provider qwen-portal")}\`.`,
      );
    }
