...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "zero-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".zero", "sessions");
...
function makeCfg(home: string, storePath: string, overrides: Partial<ZEROConfig> = {}): ZEROConfig {
  const base: ZEROConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".zero", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
