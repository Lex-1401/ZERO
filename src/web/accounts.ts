...
export function listWhatsAppAuthDirs(cfg: ZEROConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: ZEROConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: ZEROConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: ZEROConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: ZEROConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: ZEROConfig): ResolvedWhatsAppAccount[] {
