...
// Factory function to create a fresh mock socket for each test
...
// Keep track of current mock socket for test access
let currentMockSocket: ReturnType<typeof createMockSocket> | null = null;

vi.mock("./session.js", () => {
  return {
    createWaSocket: vi.fn().mockImplementation(async () => {
      // Ensure we use the same socket instance within a single test execution
...
describe("web monitor inbox", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    currentMockSocket = null; // Reset the current mock socket
    ...
    authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "zero-auth-"));
  });
