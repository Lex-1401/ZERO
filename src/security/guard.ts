...
export class SecurityGuard {
   ...
   * Leverages high-performance Rust core (SecurityEngine) for sub-millisecond redaction.
   *
  ...
  static scanForPII(text: string): SecurityViolation | null {
    ...
    if (secretCandidates) {
      for (const key of secretCandidates) {
        ...
        // Thresholds: Hex (3.0-3.6), Base64 (4.0-5.0)
        // Note: Max theoretical entropy for hex is 4.0. Threshold 3.5 catches potential keys.
        const isHex = /^[a-fA-F0-9]+$/.test(key);
        const threshold = isHex ? 3.5 : 4.5;
