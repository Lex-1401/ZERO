...
import type { ZEROConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: ZEROConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: ZEROConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep ZERO_STATE_DIR on a local-only volume and re-run "${formatCliCommand("zero security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: ZEROConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (browserToken && !looksLikeEnvRef(browserToken)) {
    findings.push({
      ...
      remediation:
        "Prefer ZERO_BROWSER_CONTROL_TOKEN (env) and remove browser.controlToken from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: ZEROConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  const browserToken =
    typeof cfg.browser?.controlToken === "string" && cfg.browser.controlToken.trim()
      ...
      : process.env.ZERO_BROWSER_CONTROL_TOKEN?.trim() || null;
...
function collectModels(cfg: ZEROConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: ZEROConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: ZEROConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ZEROConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ZEROConfig): boolean {
  const enabled = cfg.tools?.web?.fetch?.enabled;
  if (enabled === false) return false;
  return true;
}

function isBrowserEnabled(cfg: ZEROConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: ZEROConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
export function collectExposureMatrixFindings(cfg: ZEROConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
