...
const log = createSubsystemLogger("security/crypt");
...
/**
 * IntegrityCrypt: Utilitários para criptografia at-rest do estado do ZERO.
 */
export class IntegrityCrypt {
  /**
  ...
  /**
   * Verifica se um buffer parece estar criptografado pelo IntegrityCrypt.
   */
  ...
  static async prepareSessionFile(
    ...
    try {
      ...
      log.error("Erro ao preparar arquivo de sessão", { error: String(err) });
    }
    return { path: filePath, isTemp: false };
  }
  ...
  static async finalizeSessionFile(
    ...
    try {
      ...
      log.error("Erro ao finalizar arquivo de sessão", { error: String(err) });
    } finally {
      if (tempPath !== targetPath) {
