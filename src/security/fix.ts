/**
 * Security Fix — Vulnerability Remediation & Hardening
 *
 * Applies automated security fixes: chmod permissions, policy enforcement,
 * and credential hygiene. Part of the Sentinel subsystem.
 *
 * @module security/fix
 */
...
function setGroupPolicyAllowlist(params: {
  ...
  const section = params.cfg.channels[params.channel as keyof ZEROConfig["channels"]] as
...
function applyConfigFixes(params: { cfg: ZEROConfig; env: NodeJS.ProcessEnv }): {
  cfg: ZEROConfig;
  changes: string[];
  policyFlips: Set<string>;
} {
...
export async function fixSecurityFootguns(opts?: {
  ...
  if (!snap.valid) {
    ...
    errors.push(...issues.map((i: { path: string; message: string }) => `${i.path}: ${i.message}`));
  }
