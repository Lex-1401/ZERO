...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".zero-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + void-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateZeroProfile(userDataDir: string, opts?: { name?: string; color?: string }) {
  const desiredName = opts?.name ?? DEFAULT_ZERO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ZERO_BROWSER_COLOR).toUpperCase();
