...
describe("browser config", () => {
  it("defaults to enabled with loopback control url and void-orange color", () => {
    const resolved = resolveBrowserConfig(undefined);
    expect(resolved.enabled).toBe(true);
    expect(resolved.controlPort).toBe(18791);
    expect(resolved.controlHost).toBe("127.0.0.1");
    expect(resolved.color).toBe("#e63946");
    ...
    const zero = resolveProfile(resolved, "zero");
    expect(zero?.driver).toBe("zero");
    expect(zero?.cdpPort).toBe(18800);
    expect(zero?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from ZERO_GATEWAY_PORT when unset", () => {
    const prev = process.env.ZERO_GATEWAY_PORT;
    process.env.ZERO_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const zero = resolveProfile(resolved, "zero");
      expect(zero?.cdpPort).toBe(19012);
      expect(zero?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.ZERO_GATEWAY_PORT;
      } else {
        process.env.ZERO_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("supports explicit CDP URLs for the default profile", () => {
    ...
    const profile = resolveProfile(resolved, "zero");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      controlUrl: "http://127.0.0.1:18791",
      profiles: {
        zero: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("zero");
  });
});
