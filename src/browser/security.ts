...
function validateASTNode(node: any, depth = 0): void {
  ...
  if (node.type === "CallExpression") {
    ...
    if (node.callee?.type === "Identifier" && DANGEROUS_GLOBALS.has(node.callee.name)) {
 ...
 */
export function createSafeBrowserFunction(fnBody: string, allowedGlobals: string[] = []): string {
