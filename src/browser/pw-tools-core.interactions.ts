...
import { validateAndSanitizeFnBody } from "./security.js";
...
export async function evaluateViaPlaywright(opts: {
  ...
  // LLM01 + LLM08: Validate function body before execution
  // Prevents arbitrary code execution via prompt injection
  try {
    validateAndSanitizeFnBody(fnText);
  } catch (err) {
    throw new Error(
  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        ...
        root.setAttribute("data-zero-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-zero-labels", "1");
          ...
          tag.setAttribute("data-zero-labels", "1");
