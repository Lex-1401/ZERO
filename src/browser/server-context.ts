...
export function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (isExtension) {
      ...
      throw new Error(
        `Chrome extension relay is running, but no tab is connected. Click the ZERO Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
      );
    }
    ...
    if (!httpReachable) {
      ...
      const launched = await launchZeroChrome(current.resolved, profile);
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by zero. ` +
    ...
    await stopZeroChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchZeroChrome(current.resolved, profile);
    ...
    if (tabs1.length === 0) {
      if (profile.driver === "extension") {
        throw new Error(
          ...
          "Click the ZERO Browser Relay toolbar icon on the tab you want to control (badge ON).",
        );
      }
...
export { createBrowserRouteContext } from "./route-context.js";
