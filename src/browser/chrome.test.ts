...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    const userDataDir = await fsp.mkdtemp(path.join(os.tmpdir(), "zero-chrome-test-"));
    try {
      decorateZeroProfile(userDataDir, { color: DEFAULT_ZERO_BROWSER_COLOR });

      const colorInt = parseInt(DEFAULT_ZERO_BROWSER_COLOR.replace("#", ""), 16);
      const expectedSignedArgb = ((0xff << 24) | colorInt) >> 0;
      ...
      expect(def.name).toBe(DEFAULT_ZERO_BROWSER_PROFILE_NAME);
      expect(def.shortcut_name).toBe(DEFAULT_ZERO_BROWSER_PROFILE_NAME);
      ...
      const marker = await fsp.readFile(path.join(userDataDir, ".zero-profile-decorated"), "utf-8");
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await fsp.mkdtemp(path.join(os.tmpdir(), "zero-chrome-test-"));
    try {
      decorateZeroProfile(userDataDir, { color: "void-orange" });
      const localState = await readJson(path.join(userDataDir, "Local State"));
      const profile = localState.profile as Record<string, unknown>;
      const infoCache = profile.info_cache as Record<string, unknown>;
      const def = infoCache.Default as Record<string, unknown>;
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    try {
      ...
      expect(profile.name).toBe(DEFAULT_ZERO_BROWSER_PROFILE_NAME);
    } finally {
      await fsp.rm(userDataDir, { recursive: true, force: true });
    }
  });
...
describe("browser chrome helpers", () => {
  ...
  it("stopZeroChrome no-ops when process is already killed", async () => {
    const proc = { killed: true, exitCode: null, kill: vi.fn() };
    await stopZeroChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopZeroChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopZeroChrome sends SIGTERM and returns once CDP is down", async () => {
