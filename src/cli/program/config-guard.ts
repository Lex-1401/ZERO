...
export async function ensureConfigReady(params: {
  ...
  const snapshotLegacyIssues = snapshot.legacyIssues || [];
  const issues = snapshot.exists && !snapshot.valid ? formatConfigIssues(snapshotIssues) : [];
  const legacyIssues =
    snapshotLegacyIssues.length > 0
      ? snapshotLegacyIssues.map((issue) => `- ${issue.path}: ${issue.message}`)
      : [];
  ...
  params.runtime.error(`${muted("Run:")} ${commandText(formatCliCommand("zero doctor --fix"))}`);
  if (!allowInvalid) {
