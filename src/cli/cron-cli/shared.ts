...
export async function warnIfCronSchedulerDisabled(opts: GatewayRpcOpts) {
  try {
    ...
    defaultRuntime.error(
      [
        "aviso: o agendador cron está desativado no Gateway; as tarefas são salvas mas não serão executadas automaticamente.",
        "Reative com `cron.enabled: true` (ou remova `cron.enabled: false`) e reinicie o Gateway.",
        store ? `armazenamento: ${store}` : "",
      ]
        .filter(Boolean)
        .join("\n"),
    );
...
const formatRelative = (ms: number | null | undefined, nowMs: number) => {
  ...
  return delta >= 0 ? `em ${label}` : `${label} atrás`;
};

const formatSchedule = (schedule: CronSchedule) => {
  if (schedule.kind === "at") return `em ${formatIsoMinute(schedule.atMs)}`;
  if (schedule.kind === "every") return `a cada ${formatDuration(schedule.everyMs)}`;
  return schedule.tz ? `cron ${schedule.expr} @ ${schedule.tz}` : `cron ${schedule.expr}`;
};

const formatStatus = (job: CronJob) => {
  if (!job.enabled) return "desativado";
  if (job.state.runningAtMs) return "executando";
  return job.state.lastStatus ?? "ocioso";
};

export function printCronList(jobs: CronJob[], runtime = defaultRuntime) {
  if (jobs.length === 0) {
    runtime.log("Nenhuma tarefa cron.");
    return;
  }
  ...
  const header = [
    pad("ID", CRON_ID_PAD),
    pad("Nome", CRON_NAME_PAD),
    pad("Agendamento", CRON_SCHEDULE_PAD),
    pad("Próximo", CRON_NEXT_PAD),
    pad("Último", CRON_LAST_PAD),
    pad("Status", CRON_STATUS_PAD),
    pad("Alvo", CRON_TARGET_PAD),
    pad("Agente", CRON_AGENT_PAD),
  ].join(" ");
  ...
  for (const job of jobs) {
    ...
    const agentLabel = pad(truncate(job.agentId ?? "padrão", CRON_AGENT_PAD), CRON_AGENT_PAD);
    ...
    const coloredStatus = (() => {
      ...
      if (statusRaw === "executando") return colorize(rich, theme.warn, statusLabel);
