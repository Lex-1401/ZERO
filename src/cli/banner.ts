...
export function formatCliBannerLine(version: string, options: BannerOptions = {}): string {
  ...
  const commitLabel = commit ?? "desconhecido";
...
const ZERO_ASCII = [
  ...
  "    ██    ██       ██  ██   ██  ██ ",
  " ███████  ███████  ██   ██  ██████ ",
  "       ∅ VAZIO QUE CONTÉM INFINITO ∅",
];

export function formatCliBannerArt(options: BannerOptions = {}): string {
  const rich = options.richTty ?? isRich();
  if (!rich) return ZERO_ASCII.join("\n");
  ...
  const colored = ZERO_ASCII.map((line) => {
    if (line.includes("VAZIO QUE CONTÉM INFINITO")) {
      return (
        theme.muted("       ") +
        theme.accent("∅") +
        theme.info(" VAZIO QUE CONTÉM INFINITO ") +
        theme.accent("∅")
      );
    }
