...
export function formatRuntimeStatus(
  ...
  if (runtime.state && runtime.state.toLowerCase() !== status) {
    details.push(`estado ${runtime.state}`);
  }
  if (runtime.subState) details.push(`sub ${runtime.subState}`);
  if (runtime.lastExitStatus !== undefined) {
    details.push(`última saída ${runtime.lastExitStatus}`);
  }
  if (runtime.lastExitReason) details.push(`motivo ${runtime.lastExitReason}`);
  if (runtime.lastRunResult) details.push(`último resultado ${runtime.lastRunResult}`);
  if (runtime.lastRunTime) details.push(`última execução ${runtime.lastRunTime}`);
...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Serviço não instalado. Execute: ${formatCliCommand("zero gateway install", env)}`);
    if (fileLog) hints.push(`Logs em arquivo: ${fileLog}`);
    return hints;
  }
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.ZERO_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("zero gateway install", env),
    formatCliCommand("zero gateway", env),
  ];
