...
export async function runDaemonUninstall(opts: DaemonLifecycleOptions = {}) {
  ...
  if (resolveIsNixMode(process.env)) {
    fail("Modo Nix detectado; a desinstalação do serviço está desativada.");
    return;
  }
  ...
  try {
    ...
    fail(`Falha ao desinstalar o Gateway: ${String(err)}`);
    return;
  }
  ...
  if (loaded) {
    fail("Serviço do Gateway ainda carregado após desinstalação.");
    return;
  }
  emit({
    ok: true,
...
export async function runDaemonStart(opts: DaemonLifecycleOptions = {}) {
  ...
  try {
    ...
    fail(`Falha ao verificar serviço do Gateway: ${String(err)}`);
    return;
  }
  if (!loaded) {
    ...
    if (!json) {
      defaultRuntime.log(`Serviço do Gateway ${service.notLoadedText}.`);
      for (const hint of hints) {
        defaultRuntime.log(`Inicie com: ${hint}`);
      }
    }
    return;
  }
  try {
    ...
    fail(`Falha ao iniciar o Gateway: ${String(err)}`, hints);
    return;
  }
...
export async function runDaemonStop(opts: DaemonLifecycleOptions = {}) {
  ...
  try {
    ...
    fail(`Falha ao parar o Gateway: ${String(err)}`);
    return;
  }
...
export async function runDaemonRestart(opts: DaemonLifecycleOptions = {}): Promise<boolean> {
  ...
  try {
    ...
    const hints = renderGatewayServiceStartHints();
    fail(`Falha ao reiniciar o Gateway: ${String(err)}`, hints);
    return false;
  }
}
