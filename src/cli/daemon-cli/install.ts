...
export async function runDaemonInstall(opts: DaemonInstallOptions) {
  ...
  if (resolveIsNixMode(process.env)) {
    fail("Modo Nix detectado; a instalação do serviço está desativada.");
    return;
  }
  ...
  if (!isGatewayDaemonRuntime(runtimeRaw)) {
    fail('Opção --runtime inválida (use "node" ou "bun")');
    return;
  }
  ...
  if (loaded) {
    if (!opts.force) {
      ...
      if (!json) {
        ...
        defaultRuntime.log(`Reinstale com: ${formatCliCommand("zero gateway install --force")}`);
      }
      return;
    }
  }
  ...
  const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
    env: process.env,
    port,
    token: opts.token || cfg.gateway?.auth?.token || process.env.ZERO_GATEWAY_TOKEN,
    runtime: runtimeRaw,
    warn: (message) => {
  ...
  try {
    ...
    fail(`Falha na instalação do Gateway: ${String(err)}`);
    return;
  }
