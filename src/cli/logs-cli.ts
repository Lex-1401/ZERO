...
async function fetchLogs(
  ...
  if (!payload || typeof payload !== "object") {
    throw new Error("Resposta logs.tail inesperada");
  }
...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `zero logs: saída ${target} fechada (${code}). Parando o tail.`;
      try {
...
function emitGatewayError(
  ...
  const message = "Gateway não alcançável. Está rodando e acessível?";
  const hint = `Dica: execute \`${formatCliCommand("zero doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    .command("logs")
    .description("Seguir logs de arquivo do gateway via RPC")
    .option("--limit <n>", "Número máximo de linhas a retornar", "200")
    .option("--max-bytes <n>", "Número máximo de bytes a ler", "250000")
    .option("--follow", "Seguir a saída do log", false)
    .option("--interval <ms>", "Intervalo de polling em ms", "1000")
    .option("--json", "Emitir linhas de log em JSON", false)
    .option("--plain", "Saída em texto simples (sem estilo ANSI)", false)
    .option("--no-color", "Desabilitar cores ANSI")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.zero.local/cli/logs")}\n`,
    );
      ...
      if (jsonMode) {
        ...
        if (payload.truncated) {
          if (
            !emitJsonLine({
              type: "notice",
              message: "Saída de log truncada (aumente --max-bytes).",
            })
          ) {
            return;
          }
        ...
        if (payload.reset) {
          if (
            !emitJsonLine({
              type: "notice",
              message: "Cursor de log resetado (arquivo rotacionado).",
            })
          ) {
            return;
          }
        ...
        if (first && payload.file) {
          const prefix = pretty
            ? colorize(rich, theme.muted, "Arquivo de log:")
            : "Arquivo de log:";
        ...
        if (payload.truncated) {
          if (!errorLine("Saída de log truncada (aumente --max-bytes).")) {
            return;
          }
        }
        if (payload.reset) {
          if (!errorLine("Cursor de log resetado (arquivo rotacionado).")) {
            return;
          }
        }
      }
