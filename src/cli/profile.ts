...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.zero${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ZERO_PROFILE = profile;

  const stateDir = env.ZERO_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.ZERO_STATE_DIR?.trim()) env.ZERO_STATE_DIR = stateDir;

  if (!env.ZERO_CONFIG_PATH?.trim()) {
    env.ZERO_CONFIG_PATH = path.join(stateDir, "zero.json");
  }

  if (profile === "dev" && !env.ZERO_GATEWAY_PORT?.trim()) {
    env.ZERO_GATEWAY_PORT = "19001";
  }
}
