...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zero-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      ZERO_SKIP_GMAIL_WATCHER: "1",
      ZERO_SKIP_CRON: "1",
      ZERO_SKIP_BROWSER_CONTROL_SERVER: "1",
      ZERO_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "zero-entry-bootstrap.mjs");
