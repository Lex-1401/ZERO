...
describe("memory cli", () => {
  it("prints vector status when available", async () => {
    ...
    expect(log).toHaveBeenCalledWith(expect.stringContaining("Vetor: pronto"));
    expect(log).toHaveBeenCalledWith(expect.stringContaining("Dimensões do vetor: 1024"));
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("Caminho do vetor: /opt/sqlite-vec.dylib"),
    );
    expect(log).toHaveBeenCalledWith(expect.stringContaining("FTS: pronto"));
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("Cache de embeddings: ativado (123 entradas)"),
    );
  ...
  it("prints vector error when unavailable", async () => {
    ...
    expect(log).toHaveBeenCalledWith(expect.stringContaining("Vetor: indisponível"));
    expect(log).toHaveBeenCalledWith(expect.stringContaining("Erro do vetor: load failed"));
    expect(close).toHaveBeenCalled();
  });
  ...
  it("prints embeddings status when deep", async () => {
    ...
    expect(log).toHaveBeenCalledWith(expect.stringContaining("Embeddings: pronto"));
    expect(close).toHaveBeenCalled();
  });
  ...
  it("logs close failure after status", async () => {
    ...
    expect(error).toHaveBeenCalledWith(
      expect.stringContaining("Falha ao fechar o gerenciador de memória: close boom"),
    );
  ...
  it("closes manager after index", async () => {
    ...
    expect(close).toHaveBeenCalled();
    expect(log).toHaveBeenCalledWith("Índice de memória atualizado (main).");
  });
  ...
  it("closes manager after search error", async () => {
    ...
    expect(search).toHaveBeenCalled();
    expect(close).toHaveBeenCalled();
    expect(error).toHaveBeenCalledWith(expect.stringContaining("Busca de memória falhou: boom"));
