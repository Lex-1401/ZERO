...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveZEROPackageRoot } = await import("../infra/zero-root.js");
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../process/exec.js");
    vi.mocked(resolveZEROPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Status de atualização do ZERO");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-update-"));
    try {
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "zero", version: "1.0.0" }),
        "utf-8",
      );
      ...
      vi.mocked(resolveZEROPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("requires confirmation on downgrade when non-interactive", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "zero", version: "2.0.0" }),
        "utf-8",
      );
      ...
      expect(defaultRuntime.error).toHaveBeenCalledWith(
        expect.stringContaining("Confirmação de downgrade obrigatória."),
      );
      expect(defaultRuntime.exit).toHaveBeenCalledWith(1);
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
  ...
  it("updateWizardCommand requires a TTY", async () => {
    ...
    expect(defaultRuntime.error).toHaveBeenCalledWith(
      expect.stringContaining("O assistente de atualização requer um TTY."),
    );
    expect(defaultRuntime.exit).toHaveBeenCalledWith(1);
  });

  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-update-wizard-"));
    const previousGitDir = process.env.ZERO_GIT_DIR;
    try {
      setTty(true);
      process.env.ZERO_GIT_DIR = tempDir;
      ...
      process.env.ZERO_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
