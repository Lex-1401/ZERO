...
export function requireRef(ref: string | undefined) {
  ...
  if (!refValue) {
    defaultRuntime.error(danger("ref é obrigatório"));
...
export async function readFields(opts: {
  ...
  if (!payload.trim()) throw new Error("campos são obrigatórios");
  const parsed = JSON.parse(payload) as unknown;
  if (!Array.isArray(parsed)) throw new Error("campos devem ser um array");
  return parsed.map((entry, index) => {
    if (!entry || typeof entry !== "object") {
      throw new Error(`campos[${index}] deve ser um objeto`);
    }
    const rec = entry as Record<string, unknown>;
    const ref = typeof rec.ref === "string" ? rec.ref.trim() : "";
    const type = typeof rec.type === "string" ? rec.type.trim() : "";
    if (!ref || !type) {
      throw new Error(`campos[${index}] deve incluir ref e type`);
    }
    if (
    ...
    throw new Error(`campos[${index}].value deve ser string, número, booleano ou null`);
  });
}
