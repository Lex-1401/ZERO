...
export function registerSecurityCli(program: Command) {
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      if (opts.fix) {
        ...
        if (!fixResult) {
          ...
          lines.push(muted(`Correções: falha ao aplicar (${fixResult.errors[0] || "desconhecido"})`));
        } else if (
  ...
  security
    .command("audit")
    .description("Executar auditoria de segurança profunda (Sentinel)")
    .option("--deep", "Executar auditoria profunda incluindo auditoria de segredos", true)
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Auditoria de segurança Sentinel"));
      lines.push(muted(`Resumo: ${formatSummary(report.summary)}`));

      if (report.findings.length === 0) {
        lines.push("");
        lines.push("✅ Nenhuma vulnerabilidade crítica detectada.");
      }
