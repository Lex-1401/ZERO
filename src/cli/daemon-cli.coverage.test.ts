...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ZERO_STATE_DIR: process.env.ZERO_STATE_DIR,
    ZERO_CONFIG_PATH: process.env.ZERO_CONFIG_PATH,
    ZERO_GATEWAY_PORT: process.env.ZERO_GATEWAY_PORT,
    ZERO_PROFILE: process.env.ZERO_PROFILE,
  };

  beforeEach(() => {
    process.env.ZERO_STATE_DIR = "/tmp/zero-cli-state";
    process.env.ZERO_CONFIG_PATH = "/tmp/zero-cli-state/zero.json";
    delete process.env.ZERO_GATEWAY_PORT;
    delete process.env.ZERO_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.ZERO_STATE_DIR !== undefined)
      process.env.ZERO_STATE_DIR = originalEnv.ZERO_STATE_DIR;
    else delete process.env.ZERO_STATE_DIR;

    if (originalEnv.ZERO_CONFIG_PATH !== undefined)
      process.env.ZERO_CONFIG_PATH = originalEnv.ZERO_CONFIG_PATH;
    else delete process.env.ZERO_CONFIG_PATH;

    if (originalEnv.ZERO_GATEWAY_PORT !== undefined)
      process.env.ZERO_GATEWAY_PORT = originalEnv.ZERO_GATEWAY_PORT;
    else delete process.env.ZERO_GATEWAY_PORT;

    if (originalEnv.ZERO_PROFILE !== undefined) process.env.ZERO_PROFILE = originalEnv.ZERO_PROFILE;
    else delete process.env.ZERO_PROFILE;
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ZERO_PROFILE: "dev",
        ZERO_STATE_DIR: "/tmp/zero-daemon-state",
        ZERO_CONFIG_PATH: "/tmp/zero-daemon-state/zero.json",
        ZERO_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/com.zero.gateway.plist",
    });
