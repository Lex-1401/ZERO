...
function renderApprovalsSnapshot(snapshot: ExecApprovalsSnapshot, targetLabel: string) {
  ...
  for (const [agentId, agent] of Object.entries(agents)) {
    ...
    for (const entry of allowlist) {
      ...
      allowlistRows.push({
        ...
        Pattern: pattern,
        LastUsed: lastUsedAt
          ? `há ${formatAge(Math.max(0, now - lastUsedAt))}`
          : muted("desconhecido"),
      });
    }
  }
  ...
  const summaryRows = [
    { Field: "Alvo", Value: targetLabel },
    { Field: "Caminho", Value: snapshot.path },
    { Field: "Existe", Value: snapshot.exists ? "sim" : "não" },
    { Field: "Hash", Value: snapshot.hash },
    { Field: "Versão", Value: String(file.version ?? 1) },
    { Field: "Socket", Value: file.socket?.path ?? "padrão" },
    { Field: "Padrões", Value: defaultsParts.length > 0 ? defaultsParts.join(", ") : "nenhum" },
    { Field: "Agentes", Value: String(Object.keys(agents).length) },
    { Field: "Lista de Permissões", Value: String(allowlistRows.length) },
  ];

  defaultRuntime.log(heading("Aprovações"));
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
      columns: [
        { key: "Field", header: "Campo", minWidth: 8 },
  ...
  if (allowlistRows.length === 0) {
    defaultRuntime.log("");
    defaultRuntime.log(muted("Nenhuma entrada na lista de permissões."));
    return;
  }

  defaultRuntime.log("");
  defaultRuntime.log(heading("Lista de Permissões"));
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
      columns: [
        { key: "Target", header: "Alvo", minWidth: 10 },
        { key: "Agent", header: "Agente", minWidth: 8 },
        { key: "Pattern", header: "Padrão", minWidth: 20, flex: true },
        { key: "LastUsed", header: "Último Uso", minWidth: 10 },
      ],
      rows: allowlistRows,
    }).trimEnd(),
  );
...
export function registerExecApprovalsCli(program: Command) {
  ...
  const approvals = program
    ...
    .description("Gerenciar aprovações de execução (gateway ou host do nó)")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.zero.local/cli/approvals")}\n`,
    );

  const getCmd = approvals
    .command("get")
    .description("Buscar snapshot de aprovações de execução")
    .option("--node <node>", "ID/nome/IP do nó alvo")
    .option("--gateway", "Forçar aprovações do gateway", false)
    .action(async (opts: ExecApprovalsCliOpts) => {
      try {
        ...
        if (source === "local") {
          defaultRuntime.log(muted("Mostrando aprovações locais."));
          defaultRuntime.log("");
        }
        const targetLabel = source === "local" ? "local" : nodeId ? `nó:${nodeId}` : "gateway";
  ...
  const setCmd = approvals
    .command("set")
    .description("Substituir aprovações de execução por um arquivo JSON")
    .option("--node <node>", "ID/nome/IP do nó alvo")
    .option("--gateway", "Forçar aprovações do gateway", false)
    .option("--file <path>", "Caminho do arquivo JSON para upload")
    .option("--stdin", "Ler JSON do stdin", false)
    .action(async (opts: ExecApprovalsCliOpts) => {
      try {
        if (!opts.file && !opts.stdin) {
          defaultRuntime.error("Forneça --file ou --stdin.");
          defaultRuntime.exit(1);
          return;
        }
        if (opts.file && opts.stdin) {
          defaultRuntime.error("Use ou --file ou --stdin (não ambos).");
          defaultRuntime.exit(1);
          return;
        }
        const { snapshot, nodeId, source } = await loadSnapshotTarget(opts);
        if (source === "local") {
          defaultRuntime.log(theme.muted("Escrevendo aprovações locais."));
        }
        const targetLabel = source === "local" ? "local" : nodeId ? `nó:${nodeId}` : "gateway";
        if (!snapshot.hash) {
          defaultRuntime.error(
            "Hash de aprovações de execução ausente; recarregue e tente novamente.",
          );
          defaultRuntime.exit(1);
          return;
        }
        ...
        try {
          ...
          defaultRuntime.error(`Falha ao analisar o JSON de aprovações: ${String(err)}`);
          defaultRuntime.exit(1);
          return;
        }
        file.version = 1;
        ...
        defaultRuntime.log(theme.muted(`Alvo: ${targetLabel}`));
  ...
  const allowlist = approvals
    .command("allowlist")
    .description("Editar a lista de permissões por agente")
    .addHelpText(
      "after",
      () =>
        `\n${theme.heading("Exemplos:")}\n${formatExample(
          'zero approvals allowlist add "~/Projects/**/bin/rg"',
          "Adiciona um padrão de binário local à lista de permissões do agente principal.",
        )}\n${formatExample(
          'zero approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"',
          "Adiciona à lista de permissões em um nó/agente específico.",
        )}\n${formatExample(
          'zero approvals allowlist add --agent "*" "/usr/bin/uname"',
          "Adiciona à lista de permissões para todos os agentes (curinga).",
        )}\n${formatExample(
          'zero approvals allowlist remove "~/Projects/**/bin/rg"',
          "Remove um padrão da lista de permissões.",
        )}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.zero.local/cli/approvals")}\n`,
    );

  const allowlistAdd = allowlist
    .command("add <pattern>")
    .description("Adicionar um padrão glob a uma lista de permissões")
    .option("--node <node>", "ID/nome/IP do nó alvo")
    .option("--gateway", "Forçar aprovações do gateway", false)
    .option("--agent <id>", 'ID do agente (padrão "*")')
    .action(async (pattern: string, opts: ExecApprovalsCliOpts) => {
      try {
        const trimmed = pattern.trim();
        if (!trimmed) {
          defaultRuntime.error("Padrão obrigatório.");
          defaultRuntime.exit(1);
          return;
        }
        const { snapshot, nodeId, source } = await loadSnapshotTarget(opts);
        ...
        if (allowlistEntries.some((entry) => normalizeAllowlistEntry(entry) === trimmed)) {
          defaultRuntime.log("Já está na lista de permissões.");
          return;
        }
  ...
  const allowlistRemove = allowlist
    ...
    .description("Remover um padrão glob de uma lista de permissões")
    .option("--node <node>", "ID/nome/IP do nó alvo")
    .option("--gateway", "Forçar aprovações do gateway", false)
    .option("--agent <id>", 'ID do agente (padrão "*")')
    .action(async (pattern: string, opts: ExecApprovalsCliOpts) => {
      try {
        ...
        if (nextEntries.length === allowlistEntries.length) {
          defaultRuntime.log("Padrão não encontrado.");
          return;
        }
