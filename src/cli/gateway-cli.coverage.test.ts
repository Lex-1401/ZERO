...
describe("gateway-cli coverage", () => {
  ...
  it("registers gateway discover and prints human output with details on new lines", async () => {
    ...
    expect(out).toContain("Descoberta de Gateway");
    expect(out).toContain("Encontrado(s) 1 gateway(s)");
    expect(out).toContain("- Studio zero.internal.");
    expect(out).toContain("  tailnet: studio.tailnet.ts.net");
    expect(out).toContain("  host: studio.zero.internal");
  ...
  it("validates gateway discover timeout", async () => {
    ...
    expect(runtimeErrors.join("\n")).toContain("falha na descoberta de gateway:");
  ...
  it("fails gateway call on invalid params JSON", async () => {
    ...
    expect(runtimeErrors.join("\n")).toContain("Falha na chamada do Gateway:");
  });
  ...
  it("prints stop hints on GatewayLockError when service is loaded", async () => {
    ...
    expect(runtimeErrors.join("\n")).toContain("Falha ao iniciar o Gateway:");
    expect(runtimeErrors.join("\n")).toContain("zero gateway stop");
  });
