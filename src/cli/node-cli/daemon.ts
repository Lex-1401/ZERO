...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("zero node install"), formatCliCommand("zero node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("zero node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ZERO_LOG_PREFIX: baseEnv.ZERO_LOG_PREFIX ?? "node",
