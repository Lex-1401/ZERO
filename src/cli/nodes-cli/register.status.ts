...
function parseSinceMs(raw: unknown, label: string): number | undefined {
  ...
  if (value === null) {
    defaultRuntime.error(`${label}: valor de duração inválido`);
...
export function registerNodesStatusCommands(nodes: Command) {
  nodesCallOpts(
    nodes
      .command("status")
      .description("Listar nós conhecidos com status de conexão e capacidades")
      .option("--connected", "Mostrar apenas nós conectados")
      .option(
        "--last-connected <duration>",
        "Mostrar apenas nós conectados dentro da duração (ex: 24h)",
      )
      .action(async (opts: NodesRpcOpts) => {
        await runNodesCommand("status", async () => {
          ...
          const filteredLabel = filtered.length !== nodes.length ? ` (de ${nodes.length})` : "";
          defaultRuntime.log(
            `Conhecidos: ${filtered.length}${filteredLabel} · Pareados: ${pairedCount} · Conectados: ${connectedCount}`,
          );
          ...
          const rows = filtered.map((n) => {
            ...
            const caps = Array.isArray(n.caps)
              ...
              : "?";
            const paired = n.paired ? ok("pareado") : warn("não pareado");
            const connected = n.connected ? ok("conectado") : muted("desconectado");
            const since =
              typeof n.connectedAtMs === "number"
                ? ` (há ${formatAge(Math.max(0, now - n.connectedAtMs))})`
                : "";
          ...
          defaultRuntime.log(
            renderTable({
              ...
              columns: [
                ...
                { key: "Detail", header: "Detalhe", minWidth: 18, flex: true },
  ...
  nodesCallOpts(
    nodes
      .command("describe")
      .description("Descrever um nó (capacidades + comandos de invocação suportados)")
      ...
      .action(async (opts: NodesRpcOpts) => {
        await runNodesCommand("describe", async () => {
          ...
          const status = `${paired ? ok("pareado") : warn("não pareado")} · ${
            connected ? ok("conectado") : muted("desconectado")
          }`;
          const tableWidth = Math.max(60, (process.stdout.columns ?? 120) - 1);
          const rows = [
            { Field: "ID", Value: nodeId },
            displayName ? { Field: "Nome", Value: displayName } : null,
            ip ? { Field: "IP", Value: ip } : null,
            family ? { Field: "Dispositivo", Value: family } : null,
            model ? { Field: "Modelo", Value: model } : null,
            perms ? { Field: "Permissões", Value: perms } : null,
            versions ? { Field: "Versão", Value: versions } : null,
          ...
          defaultRuntime.log("");
          defaultRuntime.log(heading("Comandos"));
          if (commands.length === 0) {
            defaultRuntime.log(muted("- (nenhum reportado)"));
            return;
          }
  ...
  nodesCallOpts(
    nodes
      .command("list")
      .description("Listar nós pendentes e pareados")
      .option("--connected", "Mostrar apenas nós conectados")
      .option(
        "--last-connected <duration>",
        "Mostrar apenas nós conectados dentro da duração (ex: 24h)",
      ...
      .action(async (opts: NodesRpcOpts) => {
        await runNodesCommand("list", async () => {
          ...
          const filteredLabel =
            hasFilters && filteredPaired.length !== paired.length ? ` (de ${paired.length})` : "";
          defaultRuntime.log(
            `Pendentes: ${pendingRows.length} · Pareados: ${filteredPaired.length}${filteredLabel}`,
          );
              ...
              return {
                ...
                LastConnect:
                  typeof lastConnectedAtMs === "number"
                    ? `há ${formatAge(Math.max(0, now - lastConnectedAtMs))}`
                    : muted("desconhecido"),
              };
            });
            defaultRuntime.log("");
            defaultRuntime.log(heading("Pareados"));
            defaultRuntime.log(
              renderTable({
                width: tableWidth,
                columns: [
                  ...
                  { key: "Id", header: "ID", minWidth: 10 },
                  { key: "IP", header: "IP", minWidth: 10 },
                  { key: "LastConnect", header: "Última Conexão", minWidth: 14 },
                ],
                rows: pairedRows,
              }).trimEnd(),
            );
