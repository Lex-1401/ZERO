...
export function registerNodesInvokeCommands(nodes: Command) {
  nodesCallOpts(
    nodes
      .command("invoke")
      .description("Invocar um comando em um nó pareado")
      .requiredOption("--node <idOrNameOrIp>", "ID, nome ou IP do nó")
      .requiredOption("--command <command>", "Comando (ex: canvas.eval)")
      ...
      .option("--idempotency-key <key>", "Chave de idempotência (opcional)")
      .action(async (opts: NodesRpcOpts) => {
        await runNodesCommand("invoke", async () => {
          ...
          if (!nodeId || !command) {
            ...
            defaultRuntime.error(error("--node e --command são obrigatórios"));
  ...
  nodesCallOpts(
    nodes
      .command("run")
      .description("Executar um comando shell em um nó (apenas mac)")
      .option("--node <idOrNameOrIp>", "ID, nome ou IP do nó")
      .option("--cwd <path>", "Diretório de trabalho")
      .option(
        "--env <key=val>",
        "Sobrescrita de ambiente (repetível)",
        (value: string, prev: string[] = []) => [...prev, value],
      )
      .option("--raw <command>", "Executar uma string de comando shell bruta (sh -lc / cmd.exe /c)")
      .option("--agent <id>", "ID do agente (padrão: agente configurado como padrão)")
      .option("--ask <mode>", "Modo de pergunta do exec (off|on-miss|always)")
      .option("--security <mode>", "Modo de segurança do exec (deny|allowlist|full)")
      .option("--command-timeout <ms>", "Timeout do comando (ms)")
      .option("--needs-screen-recording", "Exigir permissão de gravação de tela")
      .option("--invoke-timeout <ms>", "Timeout de invocação do nó em ms (padrão 30000)", "30000")
      ...
      .action(async (command: string[], opts: NodesRunOpts) => {
        await runNodesCommand("run", async () => {
          ...
          if (raw && Array.isArray(command) && command.length > 0) {
            throw new Error("use --raw ou argv, não ambos");
          }
          if (!raw && (!Array.isArray(command) || command.length === 0)) {
            throw new Error("comando obrigatório");
          }

          const nodeQuery = String(opts.node ?? "").trim() || execDefaults?.node?.trim() || "";
          if (!nodeQuery) {
            throw new Error("nó obrigatório (defina --node ou tools.exec.node)");
          }
          ...
          if (opts.security && !requestedSecurity) {
            throw new Error("--security inválido (use deny|allowlist|full)");
          }
          ...
          if (opts.ask && !requestedAsk) {
            throw new Error("--ask inválido (use off|on-miss|always)");
          }
          ...
          if (!approvalsFile || typeof approvalsFile !== "object") {
            throw new Error("aprovações de exec indisponíveis");
          }
          ...
          if (hostSecurity === "deny") {
            throw new Error("exec negado: host=node security=deny");
          }
          ...
          if (requiresAsk) {
            ...
            const decision =
              decisionResult && typeof decisionResult === "object"
                ...
                : null;
            if (decision === "deny") {
              throw new Error("exec negado: usuário negou");
            }
            if (!decision) {
              if (askFallback === "full") {
                approvedByAsk = true;
                ...
                throw new Error(
                  "exec negado: aprovação necessária (UI de aprovação não disponível)",
                );
              }
            }
          ...
          if (timedOut) {
            const { error } = getNodesTheme();
            defaultRuntime.error(error("execução expirou (timed out)"));
            defaultRuntime.exit(1);
            return;
          }
          ...
          if (exitCode !== null && exitCode !== 0 && !success) {
            const { error } = getNodesTheme();
            defaultRuntime.error(error(`execução saiu com código ${exitCode}`));
            defaultRuntime.exit(1);
            return;
          }
        });
