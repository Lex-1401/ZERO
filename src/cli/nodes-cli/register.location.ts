...
export function registerNodesLocationCommands(nodes: Command) {
  const location = nodes.command("location").description("Buscar localização de um nó pareado");

  nodesCallOpts(
    location
      .command("get")
      .description("Buscar a localização atual de um nó")
      .requiredOption("--node <idOrNameOrIp>", "ID, nome ou IP do nó")
      .option("--max-age <ms>", "Usar localização em cache mais recente que isso (ms)")
      .option(
        "--accuracy <coarse|balanced|precise>",
        "Precisão desejada (padrão: balanced/precise dependendo da configuração do nó)",
      )
      .option("--location-timeout <ms>", "Timeout para fixação da localização (ms)", "10000")
