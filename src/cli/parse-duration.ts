...
export function parseDurationMs(raw: string, opts?: DurationMsParseOptions): number {
  const trimmed = String(raw ?? "")
    .trim()
    .toLowerCase();
  if (!trimmed) throw new Error("duração inválida (vazia)");

  const m = /^(\d+(?:\.\d+)?)(ms|s|m|h|d)?$/.exec(trimmed);
  if (!m) throw new Error(`duração inválida: ${raw}`);

  const value = Number(m[1]);
  if (!Number.isFinite(value) || value < 0) {
    throw new Error(`duração inválida: ${raw}`);
  }
  ...
  if (!Number.isFinite(ms)) throw new Error(`duração inválida: ${raw}`);
  return ms;
}
