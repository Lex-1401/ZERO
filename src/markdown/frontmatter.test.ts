...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { zero?: { emoji?: string } };
    expect(parsed.zero?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { zero?: { events?: string[] } };
