...
function applyDefaultModel(cfg: ZEROConfig, model: string): ZEROConfig {
...
export async function applyAuthChoicePluginProvider(
  ...
  if (!enableResult.enabled) {
    await params.prompter.note(
      `O plugin ${options.label} está desativado (${enableResult.reason ?? "bloqueado"}).`,
      options.label,
    );
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveZEROAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
  ...
  if (!provider) {
    await params.prompter.note(
      `O plugin de autenticação ${options.label} não está disponível. Ative-o e execute o assistente novamente.`,
      options.label,
    );
    return { config: nextConfig };
  }
  ...
  if (!method) {
    await params.prompter.note(
      `Método de autenticação de ${options.label} ausente.`,
      options.label,
    );
    return { config: nextConfig };
  }
  ...
  if (result.defaultModel) {
    if (params.setDefaultModel) {
      ...
      await params.prompter.note(
        `Modelo padrão definido para ${result.defaultModel}`,
        "Modelo configurado",
      );
    } else if (params.agentId) {
      agentModelOverride = result.defaultModel;
      await params.prompter.note(
        `Modelo padrão definido para ${result.defaultModel} para o agente "${params.agentId}".`,
        "Modelo configurado",
      );
    }
  }
