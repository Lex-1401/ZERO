...
type MemoryStatusSnapshot = Awaited<ReturnType<MemoryIndexManager["status"]>> & {
  agentId: string;
};
  ...
  return await withProgress(
    ...
    async (progress) => {
      ...
      const channels = await buildChannelsTable(cfg, {
        ...
        // Set `ZERO_SHOW_SECRETS=0` to force redaction.
        showSecrets: process.env.ZERO_SHOW_SECRETS?.trim() !== "0",
      });
      progress.tick();
