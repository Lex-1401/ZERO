...
function buildLabel(beacon: GatewayBonjourBeacon): string {
  ...
  const hint = host ? `${host}:${port}` : "host desconhecido";
  return `${title} (${hint})`;
}
    ...
    if (host) {
      const mode = await prompter.select({
        ...
        options: [
          {
            value: "direct",
            label: `WS direto do gateway (${host}:${port})`,
          },
      ...
      if (mode === "direct") {
        ...
        await prompter.note(
          [
            "Inicie um túnel antes de usar a CLI:",
  ...
  const urlInput = await prompter.text({
    ...
    validate: (value) =>
      String(value).trim().startsWith("ws://") || String(value).trim().startsWith("wss://")
        ? undefined
        : "A URL deve começar com ws:// ou wss://",
  });
