...
const requireAgentDir = () => {
  const agentDir = process.env.ZERO_AGENT_DIR;
  if (!agentDir) throw new Error("ZERO_AGENT_DIR not set");
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.ZERO_STATE_DIR;
  const previousAgentDir = process.env.ZERO_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.ZERO_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.ZERO_AGENT_DIR;
    } else {
      process.env.ZERO_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.ZERO_OAUTH_DIR;
  });

  it("writes auth-profiles.json under ZERO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-oauth-"));
    process.env.ZERO_STATE_DIR = tempStateDir;
    process.env.ZERO_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ZERO_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ZERO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-minimax-"));
    process.env.ZERO_STATE_DIR = tempStateDir;
    process.env.ZERO_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ZERO_AGENT_DIR;
