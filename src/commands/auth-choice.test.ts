...
describe("applyAuthChoice", () => {
  ...
  it("prompts and writes MiniMax API key when selecting minimax-api", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-auth-"));
    ...
    expect(text).toHaveBeenCalledWith(
      expect.objectContaining({ message: "Insira a chave de API da MiniMax" }),
    );
  ...
  it("prompts and writes Synthetic API key when selecting synthetic-api-key", async () => {
    ...
    expect(text).toHaveBeenCalledWith(
      expect.objectContaining({ message: "Insira a chave de API da Synthetic" }),
    );
  ...
  it("does not override the default model when selecting opencode-zen without setDefaultModel", async () => {
    ...
    expect(text).toHaveBeenCalledWith(
      expect.objectContaining({ message: "Insira a chave de API do OpenCode Zen" }),
    );
