...
import { writeConfigFile, type ZEROConfig } from "../../config/config.js";
...
function resolveCatalogChannelEntry(raw: string, cfg: ZEROConfig | null) {
...
export async function channelsAddCommand(
  ...
  if (useWizard) {
    ...
    await prompter.intro("Configuração de canal");
    ...
    if (selection.length === 0) {
      await prompter.outro("Nenhum canal selecionado.");
      return;
    }

    const wantsNames = await prompter.confirm({
      message: "Adicionar nomes de exibição para essas contas? (opcional)",
      initialValue: false,
    });
    if (wantsNames) {
      for (const channel of selection) {
        ...
        const name = await prompter.text({
          message: `Nome da conta ${channel} (${accountId})`,
    ...
    await prompter.outro("Canais atualizados.");
    return;
  }
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `O plugin ${catalogEntry.meta.label} não pôde ser carregado após a instalação.`
      : `Canal desconhecido: ${String(opts.channel ?? "")}`;
    runtime.error(hint);
    runtime.exit(1);
    return;
  }
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(`O canal ${channel} não suporta adição.`);
    runtime.exit(1);
    return;
  }
  const accountId =
  ...
  await writeConfigFile(nextConfig);
  runtime.log(`Adicionada conta do ${channelLabel(channel)} "${accountId}".`);
}
