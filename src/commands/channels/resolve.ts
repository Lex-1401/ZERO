...
function formatResolveResult(result: ResolveResult): string {
  if (!result.resolved || !result.id) return `${result.input} -> não resolvido`;
...
export async function channelsResolveCommand(opts: ChannelsResolveOptions, runtime: RuntimeEnv) {
  ...
  if (entries.length === 0) {
    throw new Error("Pelo menos uma entrada é obrigatória.");
  }
  ...
  if (!plugin?.resolver?.resolveTargets) {
    throw new Error(`O canal ${selection.channel} não suporta resolução.`);
  }
  ...
  for (const result of results) {
    if (result.resolved && result.id) {
      ...
      runtime.error(
        danger(
          `${result.input} -> não resolvido${result.error ? ` (${result.error})` : result.note ? ` (${result.note})` : ""}`,
        ),
      );
    }
  }
