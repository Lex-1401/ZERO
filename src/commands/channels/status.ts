...
import { type ZEROConfig, readConfigFileSnapshot } from "../../config/config.js";
...
export function formatGatewayChannelsStatusLines(payload: Record<string, unknown>): string[] {
  const lines: string[] = [];
  lines.push(theme.success("Gateway acessível."));
  const accountLines = (provider: ChatChannel, accounts: Array<Record<string, unknown>>) =>
    accounts.map((account) => {
      const bits: string[] = [];
      if (typeof account.enabled === "boolean") {
        bits.push(account.enabled ? "ativado" : "desativado");
      }
      ...
      if (probe && typeof probe.ok === "boolean") {
        bits.push(probe.ok ? "funciona" : "falha na sondagem");
      }
  ...
  if (issues.length > 0) {
    ...
    lines.push(`- Run: ${formatCliCommand("zero doctor")}`);
    lines.push("");
  }
  lines.push(
...
async function formatConfigChannelsStatusLines(
  ...
  const lines: string[] = [];
  lines.push(theme.warn("Gateway não acessível; mostrando status apenas da configuração."));
  if (meta.path) {
...
export async function channelsStatusCommand(
  ...
  const statusLabel = opts.probe
    ? "Verificando status do canal (sondagem)…"
  ...
  try {
    ...
    runtime.error(`Gateway não acessível: ${String(err)}`);
