...
function formatProbeLines(channelId: string, probe: unknown): string[] {
  ...
  if (channelId === "signal") {
    ...
    if (version) {
      lines.push(`Daemon do Signal: ${version}`);
    }
  }
...
export async function channelsCapabilitiesCommand(
  ...
  for (const report of reports) {
    ...
    if (report.actions && report.actions.length > 0) {
      lines.push(`Ações: ${report.actions.join(", ")}`);
    }
    if (report.configured === false || report.enabled === false) {
      const configuredLabel = report.configured === false ? "não configurado" : "configurado";
      const enabledLabel = report.enabled === false ? "desativado" : "ativado";
    ...
    if (probeLines.length > 0) {
      ...
      lines.push(theme.muted("Sondagem: indisponível"));
    }
    if (report.channel === "slack" && report.slackScopes) {
      for (const entry of report.slackScopes) {
        ...
        const label = entry.tokenType === "user" ? "Escopos de usuário" : "Escopos de bot";
    ...
    if (report.channel === "discord" && report.channelPermissions) {
      ...
      if (perms.error) {
        lines.push(`Permissões: ${theme.error(perms.error)}`);
      } else {
        const list = perms.permissions?.length ? perms.permissions.join(", ") : "nenhuma";
        const label = perms.channelId ? ` (${perms.channelId})` : "";
        lines.push(`Permissões${label}: ${list}`);
        if (perms.missingRequired && perms.missingRequired.length > 0) {
          lines.push(`${theme.warn("Obrigatórios ausentes:")} ${perms.missingRequired.join(", ")}`);
        } else {
          lines.push(theme.success("Obrigatórios ausentes: nenhum"));
        }
      }
