...
function formatEnabled(value: boolean | undefined): string {
  return value === false ? theme.error("desativado") : theme.success("ativado");
}

function formatConfigured(value: boolean): string {
  return value ? theme.success("configurado") : theme.warn("não configurado");
}
...
function formatLinked(value: boolean): string {
  return value ? theme.success("vinculado") : theme.warn("não vinculado");
}
...
async function loadUsageWithProgress(
  ...
  try {
    return await withProgress(
      { label: "Buscando resumo de uso…", indeterminate: true, enabled: true },
...
export async function channelsListCommand(
  ...
  const lines: string[] = [];
  lines.push(theme.heading("Canais de chat:"));
  ...
  lines.push("");
  lines.push(theme.heading("Provedores de autenticação (OAuth + chaves de API):"));
  if (authProfiles.length === 0) {
    lines.push(theme.muted("- nenhum"));
  } else {
    for (const profile of authProfiles) {
      const external = profile.isExternal ? theme.muted(" (sincronizado)") : "";
