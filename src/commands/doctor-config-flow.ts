...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (warnings.length > 0) {
    const lines = warnings.map((issue: any) => `- ${issue.path}: ${issue.message}`).join("\n");
    note(lines, "Avisos da configuração");
  }

  const legacyIssues = snapshot.legacyIssues || [];
  if (legacyIssues.length > 0) {
    note(
      legacyIssues.map((issue: any) => `- ${issue.path}: ${issue.message}`).join("\n"),
