...
beforeEach(() => {
  ...
  resolveZEROPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ZERO_STATE_DIR;
  originalUpdateInProgress = process.env.ZERO_UPDATE_IN_PROGRESS;
  process.env.ZERO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zero-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ZERO_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ZERO_UPDATE_IN_PROGRESS;
  } else {
    process.env.ZERO_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveZEROPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadZEROPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH_ZERO: "/tmp/zero.json",
    createConfigIO,
...
vi.mock("../infra/zero-root.js", () => ({
  resolveZEROPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "zero-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.ZERO_STATE_DIR = missingDir;
    note.mockClear();
    ...
    const stateNote = note.mock.calls.find((call) => call[1] === "Integridade do estado");
    expect(stateNote).toBeTruthy();
    expect(String(stateNote?.[0])).toContain("CRÍTICO");
  }, 30_000);
  ...
  it("skips gateway auth warning when ZERO_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/zero.json",
      exists: true,
      raw: "{}",
    ...
    process.env.ZERO_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
