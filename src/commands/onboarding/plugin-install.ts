...
import { loadZEROPlugins } from "../../plugins/loader.js";
...
function addPluginLoadPath(cfg: ZEROConfig, pluginPath: string): ZEROConfig {
...
async function promptInstallChoice(params: {
  ...
  const localOptions: Array<{ value: InstallChoice; label: string; hint?: string }> = localPath
    ? [
        {
          value: "local",
          label: "Usar caminho local do plugin",
          hint: localPath,
        },
      ]
    : [];
  const options: Array<{ value: InstallChoice; label: string; hint?: string }> = [
    { value: "npm", label: `Baixar do npm (${entry.install.npmSpec})` },
    ...localOptions,
  ...
  return await prompter.select<InstallChoice>({
    message: `Instalar o plugin ${entry.meta.label}?`,
    options,
    initialValue,
  });
}
...
export async function ensureOnboardingPluginInstalled(params: {
  ...
  await prompter.note(
    `Falha ao instalar ${entry.install.npmSpec}: ${result.error}`,
    "Instalador de Plugin",
  );

  if (localPath) {
    const fallback = await prompter.confirm({
      message: `Usar caminho local do plugin em vez disso? (${localPath})`,
      initialValue: true,
    });
    if (fallback) {
      next = addPluginLoadPath(next, localPath);
  ...
  runtime.error?.(`Falha na instalação do plugin: ${result.error}`);
  return { cfg: next, installed: false };
}
