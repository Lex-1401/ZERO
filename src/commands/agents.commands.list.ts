...
function formatSummary(summary: AgentSummary) {
  const defaultTag = summary.isDefault ? " (padrão)" : "";
  const header =
  ...
  const lines = [`- ${header}`];
  if (identityLine) {
    lines.push(`  Identidade: ${identityLine}${identitySource ? ` (${identitySource})` : ""}`);
  }
  lines.push(`  Workspace: ${shortenHomePath(summary.workspace)}`);
  lines.push(`  Diretório do agente: ${shortenHomePath(summary.agentDir)}`);
  if (summary.model) lines.push(`  Modelo: ${summary.model}`);
  lines.push(`  Regras de encaminhamento: ${summary.bindings}`);

  if (summary.routes?.length) {
    lines.push(`  Encaminhamento: ${summary.routes.join(", ")}`);
  }
  if (summary.providers?.length) {
    lines.push("  Provedores:");
  ...
  if (summary.bindingDetails?.length) {
    lines.push("  Regras de encaminhamento:");
...
export async function agentsListCommand(
  ...
  for (const summary of summaries) {
    ...
    if (routes.length > 0) {
      ...
      summary.routes = ["padrão (sem regras explícitas)"];
    }
  ...
  const lines = ["Agentes:", ...summaries.map(formatSummary)];
  lines.push(
    "Regras de encaminhamento mapeiam canal/conta/par para um agente. Use --bindings para ver as regras completas.",
  );
  lines.push(
    `O status do canal reflete as credenciais/configurações locais. Para saúde em tempo real: ${formatCliCommand("zero channels status --probe")}.`,
  );
