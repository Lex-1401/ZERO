...
export async function resetCommand(runtime: RuntimeEnv, opts: ResetOptions) {
  ...
  if (!scope) {
    if (!interactive) {
      runtime.error("O modo não interativo requer --scope.");
      runtime.exit(1);
      return;
    }
    const selection = await selectStyled<ResetScope>({
      ...
      options: [
        {
          value: "config",
          label: "Apenas configuração",
          hint: "zero.json",
        },
        {
          value: "config+creds+sessions",
          label: "Configuração + credenciais + sessões",
          hint: "mantém workspace + perfis de autenticação",
        },
        {
          value: "full",
          label: "Reset completo",
          hint: "diretório de estado + workspace",
        },
      ],
      initialValue: "config+creds+sessions",
    });
    if (isCancel(selection)) {
      cancel(stylePromptTitle("Reset cancelado.") ?? "Reset cancelado.");
      runtime.exit(0);
      return;
    }
    scope = selection;
  ...
  if (!["config", "config+creds+sessions", "full"].includes(scope)) {
    runtime.error('Escopo --scope inválido. Esperado "config", "config+creds+sessions" ou "full".');
    runtime.exit(1);
    return;
  }
  ...
  if (interactive && !opts.yes) {
    const ok = await confirm({
      message: stylePromptMessage(`Prosseguir com o reset ${scope}?`),
    });
    if (isCancel(ok) || !ok) {
      cancel(stylePromptTitle("Reset cancelado.") ?? "Reset cancelado.");
      runtime.exit(0);
  ...
  if (scope === "config+creds+sessions") {
    ...
    runtime.log(`Próximo passo: ${formatCliCommand("zero onboard --install-daemon")}`);
    return;
  }
