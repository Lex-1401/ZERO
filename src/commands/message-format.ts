...
function renderObjectSummary(payload: unknown, opts: FormatOpts): string[] {
  ...
  if (keys.length === 0) return [theme.muted("(vazio)")];
  ...
  return [
    renderTable({
      width: opts.width,
      columns: [
        { key: "Chave", header: "Chave", minWidth: 16 },
        { key: "Valor", header: "Valor", flex: true, minWidth: 24 },
      ],
      rows,
    }).trimEnd(),
  ];
...
function renderMessageList(messages: unknown[], opts: FormatOpts, emptyLabel: string): string[] {
  ...
  return [
    renderTable({
      width: opts.width,
      columns: [
        { key: "Time", header: "Hora", minWidth: 14 },
        { key: "Author", header: "Autor", minWidth: 10 },
        { key: "Text", header: "Texto", flex: true, minWidth: 24 },
...
function renderMessagesFromPayload(payload: unknown, opts: FormatOpts): string[] | null {
  ...
  return renderMessageList(messages, opts, "Nenhuma mensagem.");
}
...
function renderPinsFromPayload(payload: unknown, opts: FormatOpts): string[] | null {
  ...
  return renderMessageList(pins, opts, "Nenhum fixado.");
}
...
function renderReactions(payload: unknown, opts: FormatOpts): string[] | null {
  ...
  if (rows.length === 0) return [theme.muted("Nenhuma reação.")];
  ...
  return [
    renderTable({
      ...
      columns: [
        ...
        { key: "Count", header: "Contagem", align: "right", minWidth: 6 },
        { key: "Users", header: "Usuários", flex: true, minWidth: 20 },
      ],
      rows,
    }).trimEnd(),
  ];
...
export function formatMessageCliText(result: MessageActionRunResult): string[] {
  ...
  if (result.handledBy === "dry-run") {
    return [muted(`[dry-run] executaria ${result.action} via ${result.channel}`)];
  }
  ...
  if (result.kind === "broadcast") {
    ...
    const headingLine = ok(
      `✅ Transmissão concluída (${okCount}/${total} com sucesso, ${total - okCount} falharam)`,
    );
    return [
      headingLine,
      renderTable({
        width: opts.width,
        columns: [
          { key: "Channel", header: "Canal", minWidth: 10 },
          { key: "Target", header: "Destino", minWidth: 12, flex: true },
          { key: "Status", header: "Status", minWidth: 6 },
          { key: "Error", header: "Erro", minWidth: 20, flex: true },
        ],
  ...
  if (result.kind === "send") {
    ...
    return [ok(`✅ Enviado via ${label}.${msgId ? ` ID da mensagem: ${msgId}` : ""}`)];
  }
  ...
  if (result.kind === "poll") {
    if (result.handledBy === "core" && result.pollResult) {
      ...
      const lines = [
        ok(
          formatGatewaySummary({
            action: "Enquete enviada",
            channel: poll.channel,
            messageId: msgId,
          }),
        ),
      ];
      if (pollId) lines.push(ok(`ID da enquete: ${pollId}`));
      return lines;
    }

    const label = resolveChannelLabel(result.channel);
    const msgId = extractMessageId(result.payload);
    return [ok(`✅ Enquete enviada via ${label}.${msgId ? ` ID da mensagem: ${msgId}` : ""}`)];
  }
  ...
  if (result.action === "react") {
    ...
    if (typeof added === "string" && added.trim()) {
      lines.push(ok(`✅ Reação adicionada: ${added.trim()}`));
      return lines;
    }
    if (typeof removed === "string" && removed.trim()) {
      lines.push(ok(`✅ Reação removida: ${removed.trim()}`));
      return lines;
    }
    if (Array.isArray(removed)) {
      const list = removed
        ...
        .filter(Boolean)
        .join(", ");
      lines.push(ok(`✅ Reações removidas${list ? `: ${list}` : ""}`));
      return lines;
    }
    lines.push(ok("✅ Reação atualizada."));
    return lines;
  }

  const reactionsTable = renderReactions(payload, opts);
  if (reactionsTable && result.action === "reactions") {
    lines.push(heading("Reações"));
  ...
  if (result.action === "list-pins") {
    ...
    if (pinsTable) {
      lines.push(heading("Mensagens fixadas"));
  ...
  if (result.action === "search") {
    ...
    if (list) {
      lines.push(heading("Resultados da busca"));
      lines.push(renderMessageList(list, opts, "Nenhum resultado.")[0] ?? "");
      return lines;
    }
  }
  ...
  if (summary.length) {
    ...
    lines.push(...summary);
    lines.push("");
    lines.push(muted("Dica: use --json para a saída completa."));
  }
  return lines;
}
