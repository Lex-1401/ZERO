...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.ZERO_DEBUG_HEALTH)) {
...
const formatProbeLine = (probe: unknown, opts: { botUsernames?: string[] } = {}): string | null => {
  ...
  let label = `falhou (${status ?? "desconhecido"})`;
...
function styleHealthChannelLine(line: string): string {
  ...
  if (normalized.startsWith("falhou")) return applyPrefix("falhou", theme.error);
  if (normalized.startsWith("ok")) return applyPrefix("ok", theme.success);
  if (normalized.startsWith("vinculado")) return applyPrefix("vinculado", theme.success);
  if (normalized.startsWith("configurado")) return applyPrefix("configurado", theme.success);
  if (normalized.startsWith("não vinculado")) return applyPrefix("não vinculado", theme.warn);
  if (normalized.startsWith("não configurado")) return applyPrefix("não configurado", theme.muted);
  if (normalized.startsWith("desconhecido")) return applyPrefix("desconhecido", theme.warn);
  ...
  for (const channelId of channelOrder) {
    ...
    if (linked !== null) {
      if (linked) {
        ...
        const authLabel =
          authAgeMs != null ? ` (idade da auth ${Math.round(authAgeMs / 60000)}m)` : "";
        lines.push(`${label}: vinculado${authLabel}`);
      } else {
        lines.push(`${label}: não vinculado`);
      }
      continue;
    }
    ...
    if (configured === false) {
      lines.push(`${label}: não configurado`);
      continue;
    }
    ...
    if (configured === true) {
      lines.push(`${label}: configurado`);
      continue;
    }
    lines.push(`${label}: desconhecido`);
  }
  return lines;
};
...
export async function healthCommand(
  ...
  const summary = await withProgress(
    {
      label: "Verificando saúde do gateway…",
      indeterminate: true,
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.ZERO_DEBUG_HEALTH);
    if (opts.verbose) {
      const details = buildGatewayConnectionDetails({ config: cfg });
      runtime.log(info("Conexão do Gateway:"));
    ...
    if (resolvedAgents.length > 0) {
      const agentLabels = resolvedAgents.map((agent) =>
        agent.isDefault ? `${agent.agentId} (padrão)` : agent.agentId,
      );
    ...
    const heartbeatParts = displayAgents
      .map((agent) => {
        ...
        const label = everyMs ? formatDurationParts(everyMs) : "desativado";
        return `${label} (${agent.agentId})`;
      })
      .filter(Boolean);
    if (heartbeatParts.length > 0) {
      runtime.log(info(`Intervalo de heartbeat: ${heartbeatParts.join(", ")}`));
    }
    if (displayAgents.length === 0) {
      runtime.log(
        info(`Store de sessão: ${summary.sessions.path} (${summary.sessions.count} entradas)`),
      );
      if (summary.sessions.recent.length > 0) {
        for (const r of summary.sessions.recent) {
          runtime.log(
            `- ${r.key} (${r.updatedAt ? `há ${Math.round((Date.now() - r.updatedAt) / 60000)}m` : "sem atividade"})`,
          );
        }
      }
    } else {
      for (const agent of displayAgents) {
        runtime.log(
          info(
            `Store de sessão (${agent.agentId}): ${agent.sessions.path} (${agent.sessions.count} entradas)`,
          ),
        );
