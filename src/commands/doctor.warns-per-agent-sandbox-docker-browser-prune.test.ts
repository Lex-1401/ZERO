...
describe("doctor command", () => {
  it("warns when per-agent sandbox docker/browser/prune overrides are ignored under shared scope", async () => {
    ...
    expect(
      note.mock.calls.some(([message, title]) => {
        ...
        return (
          normalized.includes('agents.list (id "work") sobrescritas de sandbox docker') &&
          normalized.includes('escopo resolve para "shared"')
        );
      }),
    ).toBe(true);
  }, 30_000);
  ...
  it("warns when extra workspace directories exist", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      ...
      valid: true,
      config: {
        agents: { defaults: { workspace: "/Users/steipete/active-workspace" } },
      },
      issues: [],
      legacyIssues: [],
    });
    ...
    expect(
      note.mock.calls.some(
        ([message, title]) =>
          title === "Workspace extra" && typeof message === "string" && message.includes("zero"),
      ),
    ).toBe(true);
