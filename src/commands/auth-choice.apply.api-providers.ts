...
export async function applyAuthChoiceApiProviders(
  ...
  if (authChoice === "openrouter-api-key") {
    ...
    if (!hasCredential) {
      ...
      if (envKey) {
        const useExisting = await params.prompter.confirm({
          message: `Usar OPENROUTER_API_KEY existente (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
          initialValue: true,
        });
        if (useExisting) {
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Insira a chave de API da OpenRouter",
  ...
  if (authChoice === "ai-gateway-api-key") {
    ...
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `Usar AI_GATEWAY_API_KEY existente (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Insira a chave de API do Vercel AI Gateway",
        validate: validateApiKeyInput,
      });
  ...
  if (authChoice === "moonshot-api-key") {
    ...
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `Usar MOONSHOT_API_KEY existente (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Insira a chave de API da Moonshot",
        validate: validateApiKeyInput,
      });
  ...
  if (authChoice === "kimi-code-api-key") {
    ...
    if (!hasCredential) {
      await params.prompter.note(
        [
          "O Kimi Code usa um endpoint e chave de API dedicados.",
          "Obtenha sua chave de API em: https://www.kimi.com/code/en",
        ].join("\n"),
        "Kimi Code",
      );
    }
    ...
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `Usar KIMICODE_API_KEY existente (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Insira a chave de API do Kimi Code",
        validate: validateApiKeyInput,
      });
  ...
  if (authChoice === "gemini-api-key") {
    ...
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `Usar GEMINI_API_KEY existente (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Insira a chave de API do Gemini",
        validate: validateApiKeyInput,
      });
    ...
    if (params.setDefaultModel) {
      ...
      if (applied.changed) {
        await params.prompter.note(
          `Modelo padrão definido para ${GOOGLE_GEMINI_DEFAULT_MODEL}`,
          "Modelo configurado",
        );
      }
    } else {
  ...
  if (authChoice === "zai-api-key") {
    ...
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `Usar ZAI_API_KEY existente (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Insira a chave de API do Z.AI",
        validate: validateApiKeyInput,
      });
  ...
  if (authChoice === "synthetic-api-key") {
    if (params.opts?.token && params.opts?.tokenProvider === "synthetic") {
      ...
      const key = await params.prompter.text({
        message: "Insira a chave de API da Synthetic",
  ...
  if (authChoice === "venice-api-key") {
    ...
    if (!hasCredential) {
      await params.prompter.note(
        [
          "A Venice AI fornece inferência focada em privacidade com modelos sem censura.",
          "Obtenha sua chave de API em: https://venice.ai/settings/api",
          "Suporta os modos 'private' (totalmente privado) e 'anonymized' (proxy).",
        ].join("\n"),
        "Venice AI",
      );
    }
    ...
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `Usar VENICE_API_KEY existente (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Insira a chave de API da Venice AI",
        validate: validateApiKeyInput,
      });
  ...
  if (authChoice === "opencode-zen") {
    ...
    if (!hasCredential) {
      await params.prompter.note(
        [
          "O OpenCode Zen fornece acesso aos modelos Claude, GPT, Gemini e muito mais.",
          "Obtenha sua chave de API em: https://opencode.ai/auth",
          "Requer uma assinatura ativa do OpenCode Zen.",
        ].join("\n"),
        "OpenCode Zen",
      );
    }
    ...
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `Usar OPENCODE_API_KEY existente (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Insira a chave de API do OpenCode Zen",
        validate: validateApiKeyInput,
      });
