...
export function guardCancel<T>(value: T | symbol, runtime: RuntimeEnv): T {
  if (isCancel(value)) {
    cancel(stylePromptTitle("Configuração cancelada.") ?? "Configuração cancelada.");
    runtime.exit(0);
  }
  return value as T;
}

export function summarizeExistingConfig(config: ZEROConfig): string {
  const rows: string[] = [];
  ...
  return rows.length ? rows.join("\n") : "Nenhuma configuração principal detectada.";
}
...
export async function ensureWorkspaceAndSessions(
  ...
  runtime.log(`Sessões OK: ${shortenHomePath(sessionsDir)}`);
}
...
export async function moveToTrash(pathname: string, runtime: RuntimeEnv): Promise<void> {
  ...
  try {
    ...
    runtime.log(`Movido para a Lixeira: ${shortenHomePath(pathname)}`);
  } catch {
    runtime.log(`Falha ao mover para a lixeira (exclua manualmente): ${shortenHomePath(pathname)}`);
  }
}

export async function handleReset(scope: ResetScope, workspaceDir: string, runtime: RuntimeEnv) {
  await moveToTrash(CONFIG_PATH_ZERO, runtime);
