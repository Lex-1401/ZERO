...
export async function deliverAgentCommandResult(params: {
  ...
  const logDeliveryError = (err: unknown) => {
    const message = `Falha na entrega (${deliveryChannel}${deliveryTarget ? ` para ${deliveryTarget}` : ""}): ${String(err)}`;
  ...
  if (deliver) {
    if (!isDeliveryChannelKnown) {
      const err = new Error(`Canal desconhecido: ${deliveryChannel}`);
  ...
  if (!payloads || payloads.length === 0) {
    runtime.log("Nenhuma resposta do agente.");
