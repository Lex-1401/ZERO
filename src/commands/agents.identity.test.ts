...
describe("agents set-identity command", () => {
  ...
  it("sets identity from workspace IDENTITY.md", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zero-identity-"));
  ...
  it("errors when multiple agents match the same workspace", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      expect.stringContaining("Múltiplos agentes correspondem"),
    );
  ...
  it("overrides identity file values with explicit flags", async () => {
    ...
    await fs.writeFile(
      path.join(workspace, "IDENTITY.md"),
      ["- Name: Zero", "- Theme: space void", "- Emoji: :)", "- Avatar: avatars/zero.png", ""].join(
        "\n",
      ),
      "utf-8",
    );
  ...
  it("errors when identity data is missing", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      expect.stringContaining("Nenhum dado de identidade encontrado"),
    );
    expect(runtime.exit).toHaveBeenCalledWith(1);
    expect(configMocks.writeConfigFile).not.toHaveBeenCalled();
  });
