...
function resolveConfiguredModelRaw(cfg: ZEROConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: ZEROConfig): string[] {
...
async function promptManualModel(params: {
  ...
  const modelInput = await params.prompter.text({
    message: params.allowBlank ? "Modelo padrão (deixe em branco para manter)" : "Modelo padrão",
    initialValue: params.initialValue,
    placeholder: "provedor/modelo",
    validate: params.allowBlank
      ? undefined
      : (value) => (value?.trim() ? undefined : "Obrigatório"),
  });
...
export async function promptDefaultModel(
  ...
  if (shouldPromptProvider) {
    const selection = await params.prompter.select({
      message: "Filtrar modelos por provedor",
      options: [
  ...
  if (allowKeep) {
    options.push({
      value: KEEP_VALUE,
      label: configuredRaw
        ? `Manter atual (${configuredRaw})`
        : `Manter atual (padrão: ${resolvedKey})`,
      hint:
        configuredRaw && configuredRaw !== resolvedKey ? `resolve para ${resolvedKey}` : undefined,
    });
  }
  if (includeManual) {
    options.push({ value: MANUAL_VALUE, label: "Inserir modelo manualmente" });
  }
  ...
  const addModelOption = (entry: {
    ...
    if (!hasAuth(entry.provider)) hints.push("autenticação ausente");
    options.push({
      value: key,
      label: key,
  ...
  const selection = await params.prompter.select({
    message: params.message ?? "Modelo padrão",
    options,
    initialValue,
  });
...
export async function promptModelAllowlist(params: {
  ...
  if (catalog.length === 0 && allowedKeys.length === 0) {
    const raw = await params.prompter.text({
      message:
        params.message ??
        "Lista branca de modelos (provedor/modelo separados por vírgula; em branco para manter o atual)",
  ...
  for (const key of supplementalKeys) {
    ...
    options.push({
      value: key,
      label: key,
      hint: allowedKeySet ? "permitido (fora do catálogo)" : "configurado (fora do catálogo)",
    });
    seen.add(key);
  }
  ...
  const selection = await params.prompter.multiselect({
    message: params.message ?? "Modelos no seletor /model (multi-seleção)",
    options,
  ...
  const confirmClear = await params.prompter.confirm({
    message: "Limpar a lista branca de modelos? (exibe todos os modelos)",
    initialValue: false,
  });
...
export function applyPrimaryModel(cfg: ZEROConfig, model: string): ZEROConfig {
...
export function applyModelAllowlist(cfg: ZEROConfig, models: string[]): ZEROConfig {
  const defaults = cfg.agents?.defaults;
...
export function applyModelFallbacksFromSelection(cfg: ZEROConfig, selection: string[]): ZEROConfig {
