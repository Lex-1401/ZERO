...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ZERO_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("Sobrescritas de ambiente do launchctl detectadas");
    expect(message).toContain("ZERO_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ZERO_GATEWAY_TOKEN");
    expect(message).not.toContain("ZERO_GATEWAY_PASSWORD");
  });
