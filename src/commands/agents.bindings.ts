...
export function describeBinding(binding: AgentBinding) {
  ...
  if (match.accountId) parts.push(`conta=${match.accountId}`);
  if (match.peer) parts.push(`par=${match.peer.kind}:${match.peer.id}`);
  if (match.guildId) parts.push(`servidor=${match.guildId}`);
  if (match.teamId) parts.push(`time=${match.teamId}`);
  return parts.join(" ");
}
...
function resolveDefaultAccountId(cfg: ZEROConfig, provider: ChannelId): string {
...
export function parseBindingSpecs(params: {
  ...
  for (const raw of specs) {
    ...
    if (!channel) {
      errors.push(`Canal desconhecido "${channelRaw}".`);
      continue;
    }
    let accountId = accountRaw?.trim();
    if (accountRaw !== undefined && !accountId) {
      errors.push(`Vínculo inválido "${trimmed}" (ID de conta vazio).`);
      continue;
    }
    if (!accountId) {
      const plugin = getChannelPlugin(channel);
