...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "zero-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<ZEROConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<ZEROConfig["telegram"]>>,
  ...
  it("rejects unknown agent overrides", async () => {
    await withTempHome(async (home) => {
      ...
      await expect(agentCommand({ message: "hi", agentId: "ghost" }, runtime)).rejects.toThrow(
        'ID de agente desconhecido "ghost". Use "zero agents list" para ver os agentes configurados.',
      );
    });
  });
