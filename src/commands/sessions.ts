...
const formatAgeCell = (updatedAt: number | null | undefined, rich: boolean) => {
  const ageLabel = updatedAt ? formatAge(Date.now() - updatedAt) : "desconhecido";
  const padded = ageLabel.padEnd(AGE_PAD);
  return rich ? theme.muted(padded) : padded;
};

const formatModelCell = (model: string | null | undefined, rich: boolean) => {
  const label = (model ?? "desconhecido").padEnd(MODEL_PAD);
...
export async function sessionsCommand(
  ...
  if (opts.active !== undefined) {
    ...
    if (Number.isNaN(parsed) || parsed <= 0) {
      runtime.error("--active deve ser um número inteiro positivo (minutos)");
      runtime.exit(1);
      return;
    }
  ...
  runtime.log(info(`Armazém de sessões: ${storePath}`));
  runtime.log(info(`Sessões listadas: ${rows.length}`));
  if (activeMinutes) {
    runtime.log(info(`Filtrado para os últimos ${activeMinutes} minuto(s)`));
  }
  if (rows.length === 0) {
    runtime.log("Nenhuma sessão encontrada.");
    return;
  }

  const rich = isRich();
  const header = [
    "Tipo".padEnd(KIND_PAD),
    "Chave".padEnd(KEY_PAD),
    "Idade".padEnd(AGE_PAD),
    "Modelo".padEnd(MODEL_PAD),
