...
describe("onboard-hooks", () => {
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.zero/hooks",
    hooks: [
      {
        name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("should show informative notes to user", async () => {
      ...
      expect(noteCalls[0][0]).toContain("Os Hooks permitem automatizar ações");
      expect(noteCalls[0][0]).toContain("automatizar ações");

      // Second note should confirm configuration
      expect(noteCalls[1][0]).toContain("1 hook ativado: session-memory");
      expect(noteCalls[1][0]).toContain("zero hooks list");
    });
  });
});
