...
export function listAgentEntries(cfg: ZEROConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ZEROConfig, agentId: string) {
...
function resolveAgentModel(cfg: ZEROConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ZEROConfig): AgentSummary[] {
