...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.ZERO_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.ZERO_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zero-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.ZERO_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "zero");
  ...
  it("auto-enables token auth when binding LAN and persists the token", async () => {
    ...
    process.env.ZERO_STATE_DIR = stateDir;
    process.env.ZERO_CONFIG_PATH = path.join(stateDir, "zero.json");
