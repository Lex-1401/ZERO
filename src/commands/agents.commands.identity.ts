...
export async function agentsSetIdentityCommand(
  ...
  if (!agentId) {
    if (!workspaceDir) {
      runtime.error("Selecione um agente com --agent ou forneça um workspace via --workspace.");
      runtime.exit(1);
      return;
    }
    ...
    if (matches.length === 0) {
      runtime.error(
        `Nenhum workspace de agente corresponde a ${shortenHomePath(workspaceDir)}. Use --agent para atingir um agente específico.`,
      );
      runtime.exit(1);
      return;
    }
    if (matches.length > 1) {
      runtime.error(
        `Múltiplos agentes correspondem a ${shortenHomePath(workspaceDir)}: ${matches.join(", ")}. Use --agent para escolher um.`,
      );
      runtime.exit(1);
      return;
    }
  ...
  if (wantsIdentityFile) {
    ...
    if (!identityFromFile) {
      ...
      runtime.error(`Nenhum dado de identidade encontrado em ${shortenHomePath(targetPath)}.`);
      runtime.exit(1);
      return;
    }
  }
  ...
  if (
    ...
    runtime.error(
      "Nenhum campo de identidade fornecido. Use --name/--emoji/--theme/--avatar ou --from-identity.",
    );
    runtime.exit(1);
    return;
  }
  ...
  if (nextIdentity.name) runtime.log(`Nome: ${nextIdentity.name}`);
  if (nextIdentity.theme) runtime.log(`Tema: ${nextIdentity.theme}`);
