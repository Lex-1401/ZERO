...
import { type ZEROConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function updateConfig(mutator: (cfg: ZEROConfig) => ZEROConfig): Promise<ZEROConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: ZEROConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ZEROConfig): Set<string> {
