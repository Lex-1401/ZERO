...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveZEROAgentDir: vi.fn().mockReturnValue("/tmp/zero-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/zero-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveZEROAgentDir: mocks.resolveZEROAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/zero-agent/auth-profiles.json");
