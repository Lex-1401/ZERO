...
export async function statusCommand(
  ...
  runtime.log(theme.heading("ZERO status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("zero security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("zero security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("zero status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("zero logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("zero status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("zero gateway probe")}`);
  }
}
