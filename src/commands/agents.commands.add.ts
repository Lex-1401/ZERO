...
export async function agentsAddCommand(
  ...
  if (nonInteractive && !workspaceFlag) {
    runtime.error(
      "O modo não interativo requer --workspace. Execute novamente sem sinalizadores (flags) para usar o assistente.",
    );
    runtime.exit(1);
    return;
  }
  ...
  if (nonInteractive) {
    if (!nameInput) {
      runtime.error("O nome do agente é obrigatório no modo não interativo.");
      runtime.exit(1);
      return;
    }
    if (!workspaceFlag) {
    ...
    if (agentId === DEFAULT_AGENT_ID) {
      runtime.error(`"${DEFAULT_AGENT_ID}" é reservado. Escolha outro nome.`);
      runtime.exit(1);
      return;
    }
    if (agentId !== nameInput) {
      runtime.log(`ID do agente normalizado para "${agentId}".`);
    }
    if (findAgentEntryIndex(listAgentEntries(cfg), agentId) >= 0) {
      runtime.error(`O agente "${agentId}" já existe.`);
      runtime.exit(1);
      return;
    }
    ...
    if (opts.json) {
      ...
      runtime.log(`Agente: ${agentId}`);
      runtime.log(`Workspace: ${shortenHomePath(workspaceDir)}`);
      runtime.log(`Diretório do agente: ${shortenHomePath(agentDir)}`);
      if (model) runtime.log(`Modelo: ${model}`);
      if (bindingResult.conflicts.length > 0) {
        runtime.error(
          [
            "Vínculos ignorados porque já foram reivindicados por outro agente:",
  ...
  try {
    await prompter.intro("Adicionar agente ZERO");
    const name =
      nameInput ??
      (await prompter.text({
        ...
        validate: (value) => {
          ...
          if (normalized === DEFAULT_AGENT_ID) {
            return `"${DEFAULT_AGENT_ID}" é reservado. Escolha outro nome.`;
          }
          return undefined;
        },
      }));
    ...
    if (agentName !== agentId) {
      await prompter.note(`ID normalizado para "${agentId}".`, "ID do agente");
    }
    ...
    if (existingAgent) {
      const shouldUpdate = await prompter.confirm({
        message: `O agente "${agentId}" já existe. Deseja atualizá-lo?`,
        initialValue: false,
      });
      if (!shouldUpdate) {
        await prompter.outro("Nenhuma alteração foi feita.");
        return;
      }
    }
    ...
    if (defaultAgentId !== agentId) {
      ...
      if (
        ...
        const shouldCopy = await prompter.confirm({
          message: `Copiar perfis de autenticação de "${defaultAgentId}"?`,
          initialValue: false,
        });
        if (shouldCopy) {
          ...
          await prompter.note(
            `Perfis de autenticação copiados de "${defaultAgentId}".`,
    ...
    const wantsAuth = await prompter.confirm({
      message: "Configurar modelo/autenticação para este agente agora?",
      initialValue: false,
    });
    if (wantsAuth) {
    ...
    if (selection.length > 0) {
      const wantsBindings = await prompter.confirm({
        message: "Encaminhar canais selecionados para este agente agora? (vínculos)",
        initialValue: false,
      });
      if (wantsBindings) {
    ...
    await prompter.outro(`Agente "${agentId}" está pronto.`);
  } catch (err) {
