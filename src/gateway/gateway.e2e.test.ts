...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.ZERO_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zero-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "zero");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.zero-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".zero");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "zero.json");
      ...
      process.env.ZERO_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.ZERO_CONFIG_PATH = prev.configPath;
        process.env.ZERO_GATEWAY_TOKEN = prev.token;
        process.env.ZERO_SKIP_CHANNELS = prev.skipChannels;
        process.env.ZERO_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.ZERO_SKIP_CRON = prev.skipCron;
        process.env.ZERO_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.ZERO_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zero-wizard-home-"));
    process.env.HOME = tempHome;
    delete process.env.ZERO_STATE_DIR;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.ZERO_STATE_DIR = prev.stateDir;
      process.env.ZERO_CONFIG_PATH = prev.configPath;
      process.env.ZERO_GATEWAY_TOKEN = prev.token;
      process.env.ZERO_SKIP_CHANNELS = prev.skipChannels;
      process.env.ZERO_SKIP_GMAIL_WATCHER = prev.skipGmail;
