...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.ZERO_STATE_DIR;
  previousConfigPath = process.env.ZERO_CONFIG_PATH;
  previousSkipBrowserControl = process.env.ZERO_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.ZERO_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.ZERO_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zero-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ZERO_STATE_DIR = path.join(tempHome, ".zero");
  delete process.env.ZERO_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.ZERO_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "zero-test-"))
    : path.join(tempHome, ".zero-test");
  ...
  testState.sessionConfig = null;
  testState.sessionStorePath = null;
  testState.agentConfig = {};
  testState.agentsConfig = null;
  testState.bindingsConfig = null;
  testState.channelsConfig = null;
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.ZERO_STATE_DIR;
    else process.env.ZERO_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.ZERO_CONFIG_PATH;
    else process.env.ZERO_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.ZERO_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.ZERO_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.ZERO_SKIP_GMAIL_WATCHER;
    else process.env.ZERO_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.ZERO_SKIP_CANVAS_HOST;
    else process.env.ZERO_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.ZERO_GATEWAY_TOKEN;
  if (token === undefined) {
