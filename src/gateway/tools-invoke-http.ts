...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-zero-message-channel") ?? "");
  const accountId = getHeader(req, "x-zero-account-id")?.trim() || undefined;
  ...
  const allTools = createZEROTools({
