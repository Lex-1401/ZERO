...
import { resolveZEROAgentDir } from "../agents/agent-paths.js";
...
import { ensureZEROModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { ZEROConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ZERO_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ZERO_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ZERO_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<ZEROConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.ZERO_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.ZERO_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveZEROAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-live-state-"));
  process.env.ZERO_STATE_DIR = tempStateDir;
  ...
  process.env.ZERO_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.zero-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveZEROAgentDir();
  const sanitizedCfg: ZEROConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "ZERO live tool probe (local, safe): " +
    ...
    process.env.ZERO_CONFIG_PATH = previous.configPath;
    process.env.ZERO_GATEWAY_TOKEN = previous.token;
    process.env.ZERO_SKIP_CHANNELS = previous.skipChannels;
    process.env.ZERO_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ZERO_SKIP_CRON = previous.skipCron;
    process.env.ZERO_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ZERO_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ZERO_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureZEROModelsJson(cfg);
      ...
      const rawModels = process.env.ZERO_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.zero-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
