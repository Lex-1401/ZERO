...
export async function handleConnectFrame(params: {
    ...
    if (!device) {
        ...
        if (isControlUi && !allowInsecureControlUi) {
            ...
            send({ type: "res", id: frame.id, ok: false, error: errorShape(ErrorCodes.INVALID_REQUEST, msg) });
            close(1008, msg);
            return;
        }
    ...
    if (device) {
        ...
        if (typeof signedAt !== "number" || Math.abs(Date.now() - signedAt) > DEVICE_SIGNATURE_SKEW_MS) {
            setHandshakeState("failed");
            setCloseCause("device-auth-invalid", { reason: "device-signature-stale", deviceId: device.id });
            send({
                type: "res",
                id: frame.id,
                ok: false,
        ...
        if (providedNonce && providedNonce !== connectNonce) {
            setHandshakeState("failed");
            setCloseCause("device-auth-invalid", { reason: "device-nonce-mismatch", deviceId: device.id });
            send({
                type: "res",
                id: frame.id,
                ok: false,
        ...
        const signatureOk = verifyDeviceSignature(publicKey, buildDeviceAuthPayload(authPayloadParams), signature);
    ...
    if (!authOk) {
        ...
        setCloseCause("unauthorized", { authMode: resolvedAuth.mode, authProvided, authReason: authResult.reason });
        send({ type: "res", id: frame.id, ok: false, error: errorShape(ErrorCodes.INVALID_REQUEST, authMessage) });
        close(1008, truncateCloseReason(authMessage));
        return;
    }

    // 6. Device Pairing
    const skipPairing = allowInsecureControlUi && Boolean(connectParams.auth?.token || connectParams.auth?.password);
    if (device && devicePublicKey && !skipPairing) {
        ...
        const performPairing = async () => {
            ...
            if (pairing.request.silent !== true) {
                setHandshakeState("failed");
                setCloseCause("pairing-required", { deviceId: device.id, requestId: pairing.request.requestId });
                send({
                    type: "res",
                    id: frame.id,
                    ok: false,
        ...
        if (!isPaired) {
            ...
            if ((allowedRoles.size > 0 && !allowedRoles.has(role)) || (scopes.length > 0 && (pairedScopes.length === 0 || missingScope))) {
                if (!(await performPairing())) return;
            }
    ...
    const deviceToken = device ? await ensureDeviceToken({ deviceId: device.id, role, scopes }) : null;
    ...
    if (role === "node") {
        ...
        connectParams.commands = declared.map(c => c.trim()).filter(c => c.length > 0 && allowlist.has(c));
    }

    // 8. Presence and Snapshot
    const shouldTrackPresence = !isGatewayCliClient(connectParams.client);
    const instanceId = connectParams.client.instanceId;
    const presenceKey = shouldTrackPresence ? (connectParams.device?.id ?? instanceId ?? connId) : undefined;
    ...
    if (role === "node") {
        ...
        const nodeIdPairs = new Set([nodeSession.nodeId, ...(instanceId ? [instanceId as string] : [])]);
        for (const nid of nodeIdPairs) {
            void updatePairedNodeMetadata(nid, { lastConnectedAtMs: nodeSession.connectedAtMs }).catch(e =>
                logGateway.warn(`node connect record fail ${nid}: ${String(e)}`)
            );
        }
        ...
        void loadVoiceWakeConfig().then(vw => {
            context.nodeRegistry.sendEvent(nodeSession.nodeId, "voicewake.changed", { triggers: vw.triggers });
