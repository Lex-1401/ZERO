...
async function handleIncomingData(
  params: GatewayWsContext & { data: any; isBinaryFlag: boolean; isWebchatConnect: (p: any) => boolean; securityMap: any },
) {
...
function handleInboundError(err: any, connId: string, getClient: () => any, logGateway: any, close: any) {
...
async function processHandshake(params: any) {
  ...
  if (!validateRequestFrame(parsed) || parsed.method !== "connect" || !validateConnectParams(parsed.params)) {
    ...
    if (validateRequestFrame(parsed)) {
      handlers.send({ type: "res", id: parsed.id, ok: false, error: errorShape(ErrorCodes.INVALID_REQUEST, error) });
    }
...
async function processAuthenticatedRequest(params: {
  parsed: any; client: GatewayWsClient; connId: string;
  extraHandlers: GatewayRequestHandlers; buildRequestContext: () => GatewayRequestContext;
  isWebchatConnect: (p: any) => boolean; send: (obj: unknown) => void; logGateway: SubsystemLogger;
}) {
  const { parsed, client, extraHandlers, buildRequestContext, isWebchatConnect, send, logGateway } = params;

  if (!validateRequestFrame(parsed)) {
    return send({ type: "res", id: parsed?.id ?? "invalid", ok: false, error: errorShape(ErrorCodes.INVALID_REQUEST, "invalid request frame") });
  }

  const req = parsed as RequestFrame;
  const respond = (ok: boolean, payload?: unknown, error?: ErrorShape) => send({ type: "res", id: req.id, ok, payload, error });

  try {
    await handleGatewayRequest({ req, respond, client, isWebchatConnect, extraHandlers, context: buildRequestContext() });
  } catch (err) {
