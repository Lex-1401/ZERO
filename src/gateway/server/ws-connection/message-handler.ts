...
export function attachGatewayWsMessageHandler(params: {
  ...
  // Track open time for duration logging
  ...
  socket.on("message", async (data, isBinaryFlag) => {
    ...
    // Voice 2.0: Binary Audio Handling
    const isBinary = isBinaryFlag;
    if (isBinary) {
      ...
      if (getClient()) {
        const voiceSession = getOrCreateVoiceSession(connId, send);
        voiceSession.handleAudio(buf);
        return;
      }
    }
    ...
    try {
      ...
      console.log("[debug-ws] processing frame", {
        id: frameId,
        method: frameMethod,
        hasClient: !!client,
      });
      if (!client) {
        ...
        if (
          ...
          console.log("[debug-ws] handshake failed", { id: frameId, error: handshakeError });
        ...
        console.log("[debug-ws] handling connect", frame.id);
        ...
        const helloOk = {
          ...
          server: {
            version: process.env.ZERO_VERSION ?? process.env.npm_package_version ?? "dev",
      ...
      const respond = (
        ...
        console.log("[debug-ws] sending res", req.id, ok);
