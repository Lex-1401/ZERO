...
describe("gateway server auth/connect", () => {
  describe("default auth", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.ZERO_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.ZERO_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.ZERO_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.ZERO_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("connect (req) handshake returns hello-ok payload", async () => {
      const { CONFIG_PATH_ZERO, STATE_DIR_ZERO } = await import("../config/config.js");
      const ws = await openWs(port);
      ...
      expect(payload?.snapshot?.configPath).toBe(CONFIG_PATH_ZERO);
      expect(payload?.snapshot?.stateDir).toBe(STATE_DIR_ZERO);
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.ZERO_GATEWAY_TOKEN;
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "zero-device-scope-"));
