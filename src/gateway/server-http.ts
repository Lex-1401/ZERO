  ...
  router.all("*", async (ctx) => {
    ...
    if (token) {
      ...
      if (authResult.ok) {
        validToken = token;
        if (fromQuery) {
          // SECURITY: Cookie de sessão para persistência (apenas HTTP-only)
        ...
        // [PT] Se o token foi explicitamente fornecido na URL e falhou, rejeite.
        sendUnauthorized(ctx.res);
        return true;
      }
    }
...
export function attachGatewayUpgradeHandler(opts: {
  ...
  httpServer.on("upgrade", (req, socket, head) => {
    ...
    console.log(`[WS-Upgrade] origin=${origin} user-agent=${userAgent} url=${req.url}`);
    const config = loadConfig();
