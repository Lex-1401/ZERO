...
export const setTestConfigRoot = (root: string) => {
  ...
  process.env.ZERO_CONFIG_PATH = path.join(root, "zero.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "zero.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "zero-gateway-test"),
        ...fileDefaults,
