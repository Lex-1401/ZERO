...
export function resolveGatewayAuth(params: {
  ...
  const token = authConfig.token ?? env.ZERO_GATEWAY_TOKEN ?? undefined;
  const password = authConfig.password ?? env.ZERO_GATEWAY_PASSWORD ?? undefined;
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or ZERO_GATEWAY_TOKEN)",
    );
  }
...
import { authProtector } from "./auth-defense.js";
...
export async function authorizeGatewayConnect(params: {
  ...
  if (clientIp && authProtector.isBlocked(clientIp)) {
    return {
      ok: false,
      reason: authProtector.getReason(clientIp) || "too_many_attempts",
    };
  }
  ...
  if (clientIp) {
    if (result.ok) {
      authProtector.recordSuccess(clientIp);
    } else if (
...
async function authorizeGatewayConnectInternal(params: {
  ...
  if (auth.mode === "token") {
    ...
    const token = connectAuth?.token;
    if (!token) {
      return { ok: false, reason: "token_missing" };
    }
    if (!safeEqual(token, auth.token)) {
