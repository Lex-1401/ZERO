...
export function resolveGatewayClientIp(params: {
  ...
  // If the remote address is not a trusted proxy, it IS the client IP.
  if (!isTrustedProxyAddress(remote, params.trustedProxies)) {
    return remote;
  }

  // If remote is trusted, we look into forwarded headers.
  if (params.forwardedFor) {
    const ips = params.forwardedFor
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    // Search from right to left (closest proxy to furthest client)
    // We trust the right-most IPs as they come from our trusted proxies.
    // The first IP from the right that is NOT a trusted proxy is the real client IP.
    for (let i = ips.length - 1; i >= 0; i--) {
      const ip = normalizeIp(stripOptionalPort(ips[i]));
      if (ip && !isTrustedProxyAddress(ip, params.trustedProxies)) {
        return ip;
      }
    }
    // If all IPs in XFF are trusted, the "client" might be the first one listed.
    if (ips.length > 0) return normalizeIp(stripOptionalPort(ips[0]));
  }

  return parseRealIp(params.realIp) ?? remote;
}
...
export async function resolveGatewayListenHosts(
  ...
  if ((bindHost === "127.0.0.1" || bindHost === "0.0.0.0") && (await canBind("::1"))) {
    hosts.push("::1");
  }
  return hosts;
}
