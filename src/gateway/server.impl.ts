...
/**
 * Main logger for the gateway subsystem.
 */
...
export type GatewayServerOptions = {
   ...
   * - `auto`: Heuristic-based selection, prioritizing loopback with dynamic fallback to LAN.
   */
 ...
 * 3.  Initializes agent sub-registries and plugin ecosystems.
 ...
 * @throws {Error} if configuration is invalid, legacy migration fails, or critical ports are unavailable.
 */
export async function startGatewayServer(
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("zero doctor")}" to migrate.`,
      );
    }
