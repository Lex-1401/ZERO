...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ZERO_SKIP_CHANNELS;
    prevSkipGmail = process.env.ZERO_SKIP_GMAIL_WATCHER;
    process.env.ZERO_SKIP_CHANNELS = "0";
    delete process.env.ZERO_SKIP_GMAIL_WATCHER;
  });

  afterEach(() => {
    if (prevSkipChannels === undefined) {
      delete process.env.ZERO_SKIP_CHANNELS;
    } else {
      process.env.ZERO_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ZERO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ZERO_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
