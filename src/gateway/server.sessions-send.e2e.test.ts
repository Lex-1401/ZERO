...
import { createZEROTools } from "../agents/zero-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.ZERO_GATEWAY_PORT;
  prevGatewayToken = process.env.ZERO_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.ZERO_GATEWAY_PORT = String(gatewayPort);
  process.env.ZERO_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.ZERO_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.ZERO_GATEWAY_TOKEN;
  } else {
    process.env.ZERO_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createZEROTools().find((candidate) => candidate.name === "sessions_send");
