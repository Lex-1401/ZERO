...
describe("gateway server chat", () => {
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      spy.mockClear();
      vi.mocked(getReplyFromConfig).mockResolvedValue({
        text: "mock context reply",
      });
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-gw-"));
  ...
  test("routes chat.send slash commands without agent runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-gw-"));
    try {
