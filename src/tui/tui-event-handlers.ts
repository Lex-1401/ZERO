...
export function createEventHandlers(context: EventHandlerContext) {
  ...
  const handleChatEvent = (payload: unknown) => {
    ...
    if (evt.state === "delta") {
      ...
      setActivityStatus("transmitindo");
    }
    if (evt.state === "final") {
      ...
      noteFinalizedRun(evt.runId);
      state.activeChatRunId = null;
      setActivityStatus(stopReason === "error" ? "erro" : "ocioso");
      // Refresh session info to update token counts in footer
      void refreshSessionInfo?.();
    }
    if (evt.state === "aborted") {
      chatLog.addSystem("execução abortada");
      streamAssembler.drop(evt.runId);
      sessionRuns.delete(evt.runId);
      state.activeChatRunId = null;
      setActivityStatus("abortado");
      void refreshSessionInfo?.();
    }
    if (evt.state === "error") {
      chatLog.addSystem(`erro na execução: ${evt.errorMessage ?? "desconhecido"}`);
      streamAssembler.drop(evt.runId);
      sessionRuns.delete(evt.runId);
      state.activeChatRunId = null;
  ...
  const handleAgentEvent = (payload: unknown) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "start") setActivityStatus("em execução");
      if (phase === "end") setActivityStatus("ocioso");
      if (phase === "error") setActivityStatus("erro");
      tui.requestRender();
    }
  };
