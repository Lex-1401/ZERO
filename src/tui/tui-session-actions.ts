...
export function createSessionActions(context: SessionActionContext) {
  ...
  const refreshAgents = async () => {
    try {
      ...
      chatLog.addSystem(`falha ao listar agentes: ${String(err)}`);
    }
  };
  ...
  const loadHistory = async () => {
    try {
      ...
      chatLog.clearAll();
      chatLog.addSystem(`sessão ${state.currentSessionKey}`);
      ...
      chatLog.addSystem(`falha ao carregar histórico: ${String(err)}`);
    }
  ...
  const abortActive = async () => {
    if (!state.activeChatRunId) {
      chatLog.addSystem("nenhuma execução ativa");
      tui.requestRender();
      return;
    }
    try {
      ...
      chatLog.addSystem(`falha ao abortar: ${String(err)}`);
      setActivityStatus("falha ao abortar");
    }
    tui.requestRender();
  };
