...
This directory contains hooks that ship with ZERO. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/zero`)
...
**Docs**: https://docs.zero.bot/hooks/soul-evil
...
homepage: https://docs.zero.bot/hooks#my-hook
...
- **Managed hooks**: `~/.zero/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.zero/zero.json`:
...
2. Restart gateway: `pkill -9 -f 'zero.*gateway' && pnpm zero gateway`
3. Enable the hook: `zero hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.zero.bot/hooks
