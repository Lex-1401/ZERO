...
homepage: https://docs.zero.bot/hooks#command-logger
metadata:
  {
    "zero":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.zero/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.zero/logs/commands.log ~/.zero/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/zero`:
   ```
   /home/username/.zero/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
