...
function resolveHookKey(entry: HookEntry): string {
  return entry.zero?.hookKey ?? entry.hook.name;
}

function normalizeInstallOptions(entry: HookEntry): HookInstallOption[] {
  const install = entry.zero?.install ?? [];
...
function buildHookStatus(
  ...
  const managedByPlugin = entry.hook.source === "zero-plugin";
  const disabled = managedByPlugin ? false : hookConfig?.enabled === false;
  const always = entry.zero?.always === true;
  const emoji = entry.zero?.emoji ?? entry.frontmatter.emoji;
  const homepageRaw =
    entry.zero?.homepage ??
  ...
  const events = entry.zero?.events ?? [];
  ...
  const requiredOs = entry.zero?.os ?? [];
