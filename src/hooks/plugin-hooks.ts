...
import type { ZEROPluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: ZEROPluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: ZEROPluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
    ...
    zero: {
      ...entry.zero,
      hookKey: entry.zero?.hookKey ?? `${api.id}:${entry.hook.name}`,
      events: entry.zero?.events ?? [],
    },
  };
}
...
async function loadHookHandler(
  ...
  try {
    ...
    const exportName = entry.zero?.export ?? "default";
...
export async function registerPluginHooksFromDir(
  ...
  for (const entry of hooks) {
    ...
    const events = normalizedEntry.zero?.events ?? [];
    if (events.length === 0) {
