...
export async function runGmailSetup(opts: GmailSetupOptions) {
  ...
  if (!configSnapshot.valid) {
    throw new Error(`Config invalid: ${CONFIG_PATH_ZERO}`);
  }
  ...
  defaultRuntime.log(`- config: ${displayPath(CONFIG_PATH_ZERO)}`);
  defaultRuntime.log(`Next: ${formatCliCommand("zero webhooks gmail run")}`);
}
