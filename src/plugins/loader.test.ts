...
import { loadZEROPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.ZERO_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `zero-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "zero.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ZERO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadZEROPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.ZERO_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadZEROPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadZEROPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
