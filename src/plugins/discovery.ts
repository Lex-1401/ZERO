...
import type { ZEROPackageManifest, PackageManifest } from "./manifest.js";
...
function resolvePackageExtensions(manifest: PackageManifest): string[] {
  const raw = manifest.zero?.extensions;
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @zero/voice-call -> voice-call).
...
export function discoverZEROPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  if (workspaceDir) {
    ...
    const workspaceExt = path.join(workspaceRoot, ".zero", "extensions");
    discoverInDirectory({
      dir: workspaceExt,
      origin: "workspace",
