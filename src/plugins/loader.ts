...
import { discoverZEROPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ZEROPluginDefinition;
  register?: ZEROPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ZEROPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ZEROPluginDefinition;
...
export function loadZEROPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverZEROPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias ? { alias: { "zero/plugin-sdk": pluginSdkAlias } } : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ZEROPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as ZEROPluginModule;
    } catch (err) {
