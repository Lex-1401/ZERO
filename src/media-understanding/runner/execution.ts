...
import { resolveMaxBytes, resolveMaxChars, resolveTimeoutMs, resolvePrompt } from "../resolve.js";
import {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (isMediaUnderstandingSkipError(err)) {
        ...
        if (shouldLogVerbose()) {
          logVerbose(
            `Skipping ${capability} model due to ${(err as any).reason}: ${(err as any).message}`,
          );
        }
        continue;
      }
