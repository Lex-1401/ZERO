...
import type { MediaUnderstandingAttachmentsConfig } from "../config/zod-schema.tools.js";
...
export class MediaAttachmentCache {
  ...
  async getPath(params: {
    ...
    const tmpPath = path.join(os.tmpdir(), `zero-media-${crypto.randomUUID()}${extension}`);
