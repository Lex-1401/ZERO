...
function setSignalDmPolicy(cfg: ZEROConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function setSignalAllowFrom(cfg: ZEROConfig, accountId: string, allowFrom: string[]): ZEROConfig {
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "ZERO"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("zero gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
