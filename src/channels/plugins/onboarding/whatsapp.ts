...
import { loginWeb } from "../../../web/index.js";
...
function setWhatsAppDmPolicy(cfg: ZEROConfig, dmPolicy: DmPolicy): ZEROConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: ZEROConfig, allowFrom?: string[]): ZEROConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: ZEROConfig, selfChatMode: boolean): ZEROConfig {
...
async function detectWhatsAppLinked(cfg: ZEROConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so ZERO can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for ZERO" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("zero channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
