...
import { discoverZEROPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { ZEROPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["ZERO_PLUGIN_CATALOG_PATHS", "ZERO_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<ZEROPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageZERO?: ZEROPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageZERO;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverZEROPlugins({ workspaceDir: options.workspaceDir });
