...
export function listIMessageAccountIds(cfg: ZEROConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: ZEROConfig): string {
...
function mergeIMessageAccountConfig(cfg: ZEROConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: ZEROConfig): ResolvedIMessageAccount[] {
