...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.ZERO_WINDOWS_TASK_NAME?.trim();
  if (override) return override;
  return resolveGatewayWindowsTaskName(env.ZERO_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.ZERO_TASK_SCRIPT?.trim();
  if (override) return override;
  const scriptName = env.ZERO_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export async function installScheduledTask({
  ...
  const taskDescription =
    ...
    formatGatewayServiceDescription({
      profile: env.ZERO_PROFILE,
      version: environment?.ZERO_SERVICE_VERSION ?? env.ZERO_SERVICE_VERSION,
    });
