...
export type ServiceConfigIntegrity = {
    ok: boolean;
    issues: ServiceConfigIssue[];
};

export const SERVICE_INTEGRITY_CODES = {
...
export function needsNodeRuntimeMigration(issues: ServiceConfigIssue[]): boolean {
    return issues.some(
        (issue) =>
            issue.code === SERVICE_INTEGRITY_CODES.gatewayRuntimeBun ||
            issue.code === SERVICE_INTEGRITY_CODES.gatewayRuntimeNodeVersionManager,
    );
}
...
async function verifySystemdUnit(
    ...
    if (!parsed.after.has("network-online.target")) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.systemdAfterNetworkOnline,
    ...
    if (!parsed.wants.has("network-online.target")) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.systemdWantsNetworkOnline,
    ...
    if (!isRestartSecPreferred(parsed.restartSec)) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.systemdRestartSec,
...
async function verifyLaunchdPlist(
    env: Record<string, string | undefined>,
    issues: ServiceConfigIssue[],
) {
    ...
    if (!hasRunAtLoad) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.launchdRunAtLoad,
    ...
    if (!hasKeepAlive) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.launchdKeepAlive,
...
function verifyGatewayCommand(programArguments: string[] | undefined, issues: ServiceConfigIssue[]) {
    if (!programArguments || programArguments.length === 0) return;
    if (!hasGatewaySubcommand(programArguments)) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.gatewayCommandMissing,
...
function verifyGatewayServicePath(
    ...
    if (!servicePath) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.gatewayPathMissing,
    ...
    if (missing.length > 0) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.gatewayPathMissingDirs,
    ...
    if (nonMinimal.length > 0) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.gatewayPathNonMinimal,
            message:
...
async function verifyGatewayRuntime(
    env: Record<string, string | undefined>,
    command: GatewayServiceCommand,
    issues: ServiceConfigIssue[],
    platform: NodeJS.Platform,
    ...
    if (isBunRuntime(execPath)) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.gatewayRuntimeBun,
    ...
    if (isVersionManagedNodePath(execPath, platform)) {
        issues.push({
            code: SERVICE_INTEGRITY_CODES.gatewayRuntimeNodeVersionManager,
        ...
        if (!isSystemNodePath(execPath, env, platform)) {
            ...
            if (!systemNode) {
                issues.push({
                    code: SERVICE_INTEGRITY_CODES.gatewayRuntimeNodeSystemMissing,
                    message:
...
export async function verifyGatewayServiceConfig(params: {
    env: Record<string, string | undefined>;
    command: GatewayServiceCommand;
    platform?: NodeJS.Platform;
}): Promise<ServiceConfigIntegrity> {
    const issues: ServiceConfigIssue[] = [];
    const platform = params.platform ?? process.platform;

    verifyGatewayCommand(params.command?.programArguments, issues);
    verifyGatewayServicePath(params.command, issues, params.env, platform);
    await verifyGatewayRuntime(params.env, params.command, issues, platform);

    if (platform === "linux") {
        await verifySystemdUnit(params.env, issues);
    } else if (platform === "darwin") {
        await verifyLaunchdPlist(params.env, issues);
    }
