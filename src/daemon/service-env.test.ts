...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ZERO_GATEWAY_PORT).toBe("18789");
    expect(env.ZERO_GATEWAY_TOKEN).toBe("secret");
    expect(env.ZERO_SERVICE_MARKER).toBe("zero");
    expect(env.ZERO_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ZERO_SERVICE_VERSION).toBe("string");
    expect(env.ZERO_SYSTEMD_UNIT).toBe("zero-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ZERO_LAUNCHD_LABEL).toBe("com.zero.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ZERO_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ZERO_SYSTEMD_UNIT).toBe("zero-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ZERO_LAUNCHD_LABEL).toBe("com.zero.work");
    }
  });
});
