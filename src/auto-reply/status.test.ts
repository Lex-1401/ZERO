...
describe("buildStatusMessage", () => {
  ...
  it("keeps provider prefix from configured model", () => {
    const text = buildStatusMessage({
      agent: {
        model: "google-cloud-auth/claude-sonnet-4-5",
      },
      sessionScope: "per-sender",
      queue: { mode: "collect", depth: 0 },
      modelAuth: "api-key",
    ...
    expect(normalizeTestText(text)).toContain("Model: google-cloud-auth/claude-sonnet-4-5");
  });
  ...
  it("prefers cached prompt tokens from the session log", async () => {
    await withTempHome(
      async (dir) => {
        ...
        const logPath = path.join(dir, ".zero", "agents", "main", "sessions", `${sessionId}.jsonl`);
