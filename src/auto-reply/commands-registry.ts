...
export function isCommandEnabled(cfg: ZEROConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ZEROConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: ZEROConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ZEROConfig) {
