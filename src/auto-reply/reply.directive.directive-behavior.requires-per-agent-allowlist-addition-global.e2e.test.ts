...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    ...
    {
      env: {
        ZERO_AGENT_DIR: (home) => path.join(home, ".zero", "agent"),
        PI_CODING_AGENT_DIR: (home) => path.join(home, ".zero", "agent"),
      },
      prefix: "zero-reply-",
    },
  );
...
describe("directive behavior", () => {
  ...
  it("allows elevated when both global and per-agent allowlists match", async () => {
    await withTempHome(async (home) => {
      ...
      const text = Array.isArray(res) ? res[0]?.text : res?.text;
      expect(text).toContain("Modo elevado definido para perguntar");
      expect(runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
  it("warns when elevated is used in direct runtime", async () => {
    await withTempHome(async (home) => {
      ...
      const text = Array.isArray(res) ? res[0]?.text : res?.text;
      expect(text).toContain("Modo elevado desativado.");
      expect(text).toContain("Runtime é direto; o sandbox não se aplica.");
      expect(runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
  it("rejects invalid elevated level", async () => {
    await withTempHome(async (home) => {
      ...
      const text = Array.isArray(res) ? res[0]?.text : res?.text;
      expect(text).toContain("Nível elevado não reconhecido");
      expect(runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
  it("handles multiple directives in a single message", async () => {
    await withTempHome(async (home) => {
      ...
      const text = Array.isArray(res) ? res[0]?.text : res?.text;
      expect(text).toContain("Modo elevado desativado.");
      expect(text).toContain("Log detalhado ativado.");
      expect(runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
});
