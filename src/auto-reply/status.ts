...
type AgentConfig = Partial<NonNullable<NonNullable<ZEROConfig["agents"]>["defaults"]>>;
...
const formatVoiceModeLine = (config?: ZEROConfig, sessionEntry?: SessionEntry): string | null => {
  ...
  return `🔊 Voice: ${autoMode} · provider=${provider} · limit=${maxLength} · summarize=${summarize}`;
};
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `∅ ZERO ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: ZEROConfig): string {
  const options = [
    "/think <level>",
    "/verbose on|full|off",
    "/reasoning on|off",
...
export function buildCommandsMessage(cfg?: ZEROConfig, skillCommands?: SkillCommandSpec[]): string {
