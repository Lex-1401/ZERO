...
describe("runReplyAgent typing (heartbeat)", () => {
  it("still replies even if session reset fails to persist", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "zero-session-reset-fail-"));
    ...
    try {
      ...
      expect(res).toMatchObject({
        text: expect.stringContaining("histórico da sessão foi corrompido"),
      });
  ...
  it("rewrites Bun socket errors into friendly text", async () => {
    ...
    expect(payloads[0]?.text).toContain("conexão com o LLM falhou");
