...
type AgentDefaults = NonNullable<ZEROConfig["agents"]>["defaults"];
...
export async function resolveReplyDirectives(params: {
  ...
  const channelCfg = (cfg.channels as any)?.[sessionCtx.Provider ?? ""];
  const streamMode = channelCfg?.streamMode;
