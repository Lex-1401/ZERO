    ...
    try {
      ...
      if (
        ...
        return {
          kind: "final",
          payload: {
            text: "⚠️ Limite de contexto excedido. Reiniciei nossa conversa para começar do zero - tente novamente.\n\nPara evitar isso, aumente o buffer de compactação definindo `agents.defaults.compaction.reserveTokensFloor` como 4000 ou superior em sua configuração.",
          },
        };
      }
      if (embeddedError?.kind === "role_ordering") {
        ...
        if (didReset) {
          return {
            kind: "final",
            payload: {
              text: "⚠️ Conflito de ordenação de mensagens. Reiniciei a conversa - tente novamente.",
            },
          };
        }
      }
      ...
      if (
        ...
        return {
          kind: "final",
          payload: {
            text: "⚠️ Limite de contexto excedido durante a compactação. Reiniciei nossa conversa para começar do zero - tente novamente.\n\nPara evitar isso, aumente o buffer de compactação definindo `agents.defaults.compaction.reserveTokensFloor` como 4000 ou superior em sua configuração.",
          },
        };
      }
      ...
      if (
        ...
        return {
          kind: "final",
          payload: {
            text: "⚠️ O histórico da sessão foi corrompido. Reiniciei a conversa - tente novamente!",
          },
        };
      }
      ...
      const fallbackText = isContextOverflow
        ? "⚠️ Estouro de contexto — prompt muito grande para este modelo. Tente uma mensagem mais curta ou um modelo com contexto maior."
        : isRoleOrderingError
          ? "⚠️ Conflito de ordenação de mensagens - tente novamente. Se isso persistir, use /new para iniciar uma nova sessão."
          : `⚠️ O agente falhou antes de responder: ${trimmedMessage}.\nLogs: zero logs --follow`;
