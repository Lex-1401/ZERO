...
import { scheduleGatewaySigusr1Restart, triggerZERORestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting zero in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerZERORestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting zero via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
...
export const handleForkCommand: CommandHandler = async (params, allowTextCommands) => {
  if (!allowTextCommands) return null;
  if (params.command.commandBodyNormalized !== "/fork") return null;
  if (!params.command.isAuthorizedSender) {
    return { shouldContinue: false };
  }

  if (params.sessionEntry && params.sessionKey && params.sessionStore) {
    // A ramificação real ocorre via initSessionState quando ParentSessionKey está presente.
    // O comando /fork sinaliza que o usuário deseja iniciar um novo ramo a partir daqui.
    return {
      shouldContinue: false,
      reply: {
        text: "🌱 Ramificação de sessão iniciada. A próxima mensagem criará um novo ramo a partir deste contexto.",
      },
    };
  }
