...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-commands-"));
...
function buildParams(commandBody: string, cfg: ZEROConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
