...
/**
 * Orchestrates the extraction of the session fork directive (/fork).
 *
 * [PT] Orquestra a extração da diretiva de ramificação de sessão (/fork).
 */
export function extractForkDirective(body?: string): {
  cleaned: string;
  hasDirective: boolean;
} {
  if (!body) return { cleaned: "", hasDirective: false };
  return extractSimpleDirective(body, ["fork"]);
}
