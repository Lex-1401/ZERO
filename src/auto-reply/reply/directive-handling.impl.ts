...
export async function handleDirectiveOnly(params: {
  ...
  if (directives.hasExecDirective) {
    if (directives.invalidExecHost) {
      return {
        text: `Unrecognized execution host "${directives.rawExecHost ?? ""}". Valid hosts: sandbox, gateway, node.`,
      };
    }
    if (directives.invalidExecSecurity) {
      return {
        text: `Unrecognized execution security "${directives.rawExecSecurity ?? ""}". Valid: deny, allowlist, full.`,
      };
    }
    if (directives.invalidExecAsk) {
      return {
        text: `Unrecognized execution ask "${directives.rawExecAsk ?? ""}". Valid: off, on-miss, always.`,
      };
    }
    if (directives.invalidExecNode) {
      return {
        text: "Execution node requires a value.",
      };
    }
    if (!directives.hasExecOptions) {
      ...
      return {
        text: withOptions(
          `Current execution defaults: host=${execDefaults.host}, security=${execDefaults.security}, ask=${execDefaults.ask}, ${nodeLabel}.`,
  ...
  if (
    ...
    return {
      text: `"xhigh" thinking level is only supported for ${formatXHighModelHint()}.`,
    };
  }
  ...
  if (directives.hasVerboseDirective && directives.verboseLevel) {
    parts.push(
      directives.verboseLevel === "off"
        ? formatDirectiveAck("Detailed logging disabled.")
        : directives.verboseLevel === "full"
          ? formatDirectiveAck("Detailed logging set to full.")
          : formatDirectiveAck("Detailed logging enabled."),
    );
  }
  if (directives.hasReasoningDirective && directives.reasoningLevel) {
    parts.push(
    ...
    parts.push(
      directives.elevatedLevel === "off"
        ...
        : directives.elevatedLevel === "full"
          ? formatDirectiveAck("Elevated mode set to full (auto-approval).")
  ...
  if (directives.hasExecDirective && directives.hasExecOptions) {
    ...
    if (execParts.length > 0) {
      parts.push(formatDirectiveAck(`Execution defaults set (${execParts.join(", ")}).`));
    }
  }
  if (shouldDowngradeXHigh) {
    parts.push(
  ...
  if (directives.hasQueueDirective && typeof directives.cap === "number") {
    parts.push(formatDirectiveAck(`Queue capacity set to ${directives.cap}.`));
  }
  if (directives.hasQueueDirective && directives.dropPolicy) {
    parts.push(formatDirectiveAck(`Queue drop policy set to ${directives.dropPolicy}.`));
  }
