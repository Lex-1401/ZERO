...
describe("runReplyAgent typing (heartbeat)", () => {
  it("resets corrupted Gemini sessions and deletes transcripts", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "zero-session-reset-"));
    ...
    try {
      ...
      expect(res).toMatchObject({
        text: expect.stringContaining("O histórico da sessão foi corrompido"),
      });
  ...
  it("keeps sessions intact on other errors", async () => {
    const prevStateDir = process.env.ZERO_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "zero-session-noreset-"));
    process.env.ZERO_STATE_DIR = stateDir;
    try {
      ...
      expect(res).toMatchObject({
        text: expect.stringContaining("O agente falhou antes de responder"),
      });
