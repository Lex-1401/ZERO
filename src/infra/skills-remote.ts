...
function listWorkspaceDirs(cfg: ZEROConfig): string[] {
...
function collectRequiredBins(entries: SkillEntry[], targetPlatform: string): string[] {
  ...
  for (const entry of entries) {
    const os = entry.zero?.os ?? [];
    if (os.length > 0 && !os.includes(targetPlatform)) continue;
    const required = entry.zero?.requires?.bins ?? [];
    const anyBins = entry.zero?.requires?.anyBins ?? [];
...
export async function refreshRemoteBinsForConnectedNodes(cfg: ZEROConfig) {
  if (!remoteRegistry) return;
