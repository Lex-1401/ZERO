...
export async function detectLegacyStateMigrations(params: {
  ...
  if (hasLegacySessions) {
    preview.push(`- Sessões: ${sessionsLegacyDir} → ${sessionsTargetDir}`);
  }
  if (legacyKeys.length > 0) {
    preview.push(`- Sessões: canonizar chaves legadas em ${sessionsTargetStorePath}`);
  }
  if (hasLegacyAgentDir) {
    preview.push(`- Diretório do agente: ${legacyAgentDir} → ${targetAgentDir}`);
  }
  if (hasLegacyWhatsAppAuth) {
    preview.push(
      `- Autenticação WhatsApp: ${oauthDir} → ${targetWhatsAppAuthDir} (manter oauth.json)`,
    );
  }
...
async function migrateLegacySessions(
  ...
  if (!merged[mainKey]) {
    ...
    if (latest?.sessionId) {
      ...
      changes.push(`Migrada a sessão de chat direto mais recente → ${mainKey}`);
    }
  }

  if (!legacyParsed.ok) {
    warnings.push(
      `O armazenamento de sessões legadas não pôde ser lido; deixado no local em ${detected.sessions.legacyStorePath}`,
    );
  }
  ...
  if (
    ...
    changes.push(`Armazenamento de sessões mesclado → ${detected.sessions.targetStorePath}`);
    if (canonicalizedTarget.legacyKeys.length > 0) {
      changes.push(`Canonizadas ${canonicalizedTarget.legacyKeys.length} chaves de sessão legadas`);
    }
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      changes.push(`Movido ${entry.name} → agents/${detected.targetAgentId}/sessions`);
    } catch (err) {
      warnings.push(`Falha ao mover ${from}: ${String(err)}`);
    }
  }
  ...
  if (legacyLeft.length > 0) {
    ...
    try {
      ...
      warnings.push(`Sessões legadas deixadas em ${backupDir}`);
    } catch {
      // ignore
    }
  }
...
export async function migrateLegacyAgentDir(
  ...
  for (const entry of entries) {
    ...
    try {
      fs.renameSync(from, to);
      changes.push(
        `Arquivo do agente movido ${entry.name} → agents/${detected.targetAgentId}/agent`,
      );
    } catch (err) {
      warnings.push(`Falha ao mover ${from}: ${String(err)}`);
    }
  ...
  if (!emptyDirOrMissing(detected.agentDir.legacyDir)) {
    ...
    try {
      ...
      warnings.push(`Diretório do agente legado deixado em ${backupDir}`);
    } catch (err) {
      warnings.push(`Falha ao realocar diretório do agente legado: ${String(err)}`);
    }
  }
...
async function migrateLegacyWhatsAppAuth(
  ...
  for (const entry of entries) {
    ...
    try {
      fs.renameSync(from, to);
      changes.push(`Autenticação WhatsApp movida ${entry.name} → whatsapp/default`);
    } catch (err) {
      warnings.push(`Falha ao mover ${from}: ${String(err)}`);
    }
  }
...
export async function autoMigrateLegacyState(params: {
  ...
  const env = params.env ?? process.env;
  if (env.ZERO_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return { migrated: false, skipped: true, changes: [], warnings: [] };
  }
  ...
  if (changes.length > 0) {
    logger.info(
      `Estado legado migrado automaticamente:\n${changes.map((entry) => `- ${entry}`).join("\n")}`,
    );
  }
  if (warnings.length > 0) {
    logger.warn(
      `Avisos de migração de estado legado:\n${warnings.map((entry) => `- ${entry}`).join("\n")}`,
    );
  }
