...
export async function ensureFunnel(
  ...
  try {
    ...
    if (stdout.trim()) {
      logVerbose(`tailscale funnel output: ${stdout.trim()}`);
    }
  } catch (err) {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for ZERO. You can keep running the web gateway without it: \`${formatCliCommand("zero gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
