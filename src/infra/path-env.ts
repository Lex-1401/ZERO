...
function candidateBinDirs(opts: EnsureZEROPathOpts): string[] {
  ...
  // Bundled macOS app: `zero` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingZERO = path.join(execDir, "zero");
    if (isExecutable(siblingZERO)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/zero` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "zero"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `zero` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureZEROCliOnPath(opts: EnsureZEROPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ZERO_PATH_BOOTSTRAPPED)) return;
  process.env.ZERO_PATH_BOOTSTRAPPED = "1";
