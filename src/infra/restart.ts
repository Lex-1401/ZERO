...
export function triggerZERORestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(process.env.ZERO_SYSTEMD_UNIT, process.env.ZERO_PROFILE);
  ...
  const label =
    process.env.ZERO_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.ZERO_PROFILE);
