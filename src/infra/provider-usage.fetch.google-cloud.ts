...
function buildUsageWindows(opts: {
  ...
  if (opts.modelQuotas && opts.modelQuotas.size > 0) {
    ...
    logDebug(
      `[google-cloud-auth] Built ${topModels.length} model windows from ${opts.modelQuotas.size} total models`,
    );
    for (const w of topModels) {
      logDebug(
        `[google-cloud-auth]   ${w.label}: ${w.usedPercent.toFixed(1)}% used${w.resetAt ? ` (resets at ${new Date(w.resetAt).toISOString()})` : ""}`,
      );
    }
...
export async function fetchGoogleCloudAuthUsage(
  token: string,
  timeoutMs: number,
  ...
  try {
    ...
    if (res.ok) {
      ...
      logDebug(
        `[google-cloud-auth] Credits: ${credits ? `${credits.available}/${credits.monthly}` : "none"}${planInfo ? ` (plan: ${planInfo})` : ""}`,
      );
    } else {
      ...
      if (res.status === 401) {
        return {
          ...
          displayName: PROVIDER_LABELS["google-cloud-auth"],
          windows: [],
          error: "Token expired",
        };
      }
  ...
  if (!projectId) {
    logDebug("[google-cloud-auth] Missing project id; requesting available models without project");
  }
  try {
    ...
    if (res.ok) {
      ...
      logDebug(`[google-cloud-auth] Extracted ${modelQuotas.size} model quotas from API`);
      for (const [modelId, quota] of modelQuotas) {
        logDebug(
          `[google-cloud-auth]   ${modelId}: ${(quota.remainingFraction * 100).toFixed(1)}% remaining${quota.resetTime ? ` (resets ${new Date(quota.resetTime).toISOString()})` : ""}`,
        );
      }
    } else {
  ...
  if (windows.length === 0 && lastError) {
    logDebug(`[google-cloud-auth] Returning error snapshot: ${lastError}`);
    return {
      provider: "google-cloud-auth",
      displayName: PROVIDER_LABELS["google-cloud-auth"],
      windows: [],
  ...
  logDebug(
    `[google-cloud-auth] Returning snapshot with ${windows.length} windows${planInfo ? ` (plan: ${planInfo})` : ""}`,
  );
  logDebug(`[google-cloud-auth] Snapshot: ${JSON.stringify(snapshot, null, 2)}`);
