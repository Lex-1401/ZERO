  ...
  for (const dir of candidates) {
    ...
    for (let i = 0; i < 12; i += 1) {
      ...
      try {
        ...
        if (parsed?.name === "zero") return current;
      } catch {
        // ignore
      }
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zero-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "zero doctor",
        managerScriptArgs(manager, "zero", ["doctor", "--non-interactive"]),
        gitRoot,
        { ZERO_UPDATE_IN_PROGRESS: "1" },
      ),
    );
  ...
  if (globalManager) {
    const spec = `zero@${normalizeTag(opts.tag)}`;
