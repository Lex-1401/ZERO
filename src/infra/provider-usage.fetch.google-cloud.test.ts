...
import { fetchGoogleCloudAuthUsage } from "./provider-usage.fetch.google-cloud.js";
...
describe("fetchGoogleCloudAuthUsage", () => {
  it("returns 3 windows when both endpoints succeed", async () => {
    ...
    const snapshot = await fetchGoogleCloudAuthUsage("token-123", 5000, mockFetch);

    expect(snapshot.provider).toBe("google-cloud-auth");
    expect(snapshot.displayName).toBe("GoogleCloudAuth");
  ...
  it("uses cloudaicompanionProject string as project id", async () => {
    ...
    await fetchGoogleCloudAuthUsage("token-123", 5000, mockFetch);
