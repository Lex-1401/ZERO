...
function resolveActiveHoursTimezone(cfg: ZEROConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ZEROConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ZEROConfig) {
...
function resolveHeartbeatConfig(cfg: ZEROConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: ZEROConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatAckMaxChars(cfg: ZEROConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function resolveHeartbeatSession(cfg: ZEROConfig, agentId?: string, heartbeat?: HeartbeatConfig) {
