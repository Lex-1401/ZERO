...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ZEROConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: ZEROConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ZEROConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: ZEROConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: ZEROConfig): void {
