...
describe("runHeartbeatOnce", () => {
  ...
  it("uses the last non-empty payload for delivery", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-hb-"));
    ...
    try {
      ...
      if (res.status !== "ran") {
        throw new Error("HEARTBEAT SKIPPED REASON: " + (res as any).reason);
      }
      expect(res).toMatchObject({ status: "ran" });
