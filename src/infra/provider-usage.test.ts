...
describe("provider usage loading", () => {
  ...
  it("discovers Claude usage from token auth profiles", async () => {
    await withTempHome(
      ...
      {
        env: {
          ZERO_STATE_DIR: (home) => path.join(home, ".zero"),
        },
        prefix: "zero-provider-usage-",
      },
    );
  });
  ...
  it("prefers claude-cli token for Anthropic usage snapshots", async () => {
    await withTempHome(
      async () => {
        const stateDir = process.env.ZERO_STATE_DIR;
        if (!stateDir) throw new Error("Missing ZERO_STATE_DIR");
