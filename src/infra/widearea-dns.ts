...
export const WIDE_AREA_DISCOVERY_DOMAIN = "zero.internal.";
export const WIDE_AREA_ZONE_FILENAME = "zero.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*zero-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "zero";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "zero");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "zero-gw");
  ...
  records.push(`_zero-gw._tcp IN PTR ${instanceLabel}._zero-gw._tcp`);
  records.push(`${instanceLabel}._zero-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._zero-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; zero-content-hash: ${contentHash}\n${contentBody}`;
}
