...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/zero-telegram-${Math.random().toString(16).slice(2)}.json`,
}));
...
vi.mock("../auto-reply/reply/provider-dispatcher.js", () => ({
  dispatchReplyWithBufferedBlockDispatcher: vi.fn(async (opts) => {
    ...
    // Chama o replySpy (getReplyFromConfig) que está mockado em reply.js
    const { getReplyFromConfig } = await import("../auto-reply/reply.js");
    await getReplyFromConfig(opts.ctx, {
      onReplyStart: opts.dispatcherOptions.onReplyStart,
    });
