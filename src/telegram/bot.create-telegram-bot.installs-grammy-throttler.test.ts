...
vi.mock("./bot-message-context.js", () => ({ buildTelegramMessageContext: buildTelegramMessageContextSpy }));
vi.mock("./bot-message-dispatch.js", () => ({ dispatchTelegramMessage: dispatchTelegramMessageSpy }));
...
vi.mock("./accounts.js", () => ({
  resolveTelegramAccount: (params: any) => {
    ...
    return { config: { capabilities: ["inlineButtons"], ...account }, accountId: accountId || "default" };
  },
...
vi.mock("grammy", () => ({
  Bot: class {
    ...
    stop = vi.fn();
    command = vi.fn();
    constructor(public token: string, public options?: any) { botCtorSpy(token, options); }
  },
  InputFile: class { },
...
describe("createTelegramBot", () => {
  beforeEach(async () => {
    ...
    (configMod.loadConfig as any).mockReturnValue({
      ...
      channels: { telegram: { dmPolicy: "open", allowFrom: ["*"], capabilities: ["inlineButtons"] } },
    });

    buildTelegramMessageContextSpy.mockImplementation(async (params) => {
      const callbackQuery = (params.primaryCtx as any).callbackQuery || (params.primaryCtx as any).update?.callback_query;
      const msg = params.primaryCtx.message || callbackQuery?.message || (params.primaryCtx as any).update?.message || {};
      const chatId = msg.chat?.id || 1234;
      return {
        ctxPayload: { Body: msg.text || callbackQuery?.data || "hello", From: `telegram:${chatId}`, MessageSid: "1" },
        primaryCtx: params.primaryCtx,
        msg,
        chatId,
        sendTyping: async () => { await sendChatActionSpy(chatId, "typing", undefined); },
      };
    });
  ...
  afterEach(() => { process.env.TZ = undefined; });
  ...
  it("resolves sequential keys correctly", () => {
    ...
    expect(keyFn({ message: { chat: { id: 123 }, message_thread_id: 456 } })).toBe("telegram:123:topic:456");
    expect(keyFn({ chat: { id: 123 } })).toBe("telegram:123");
    expect(keyFn({ update: { callback_query: { message: { chat: { id: 789 } } } } })).toBe("telegram:789");
  ...
  it("dedupes duplicate updates by update_id", async () => {
    ...
    const ctx = { update: { update_id: 111 }, message: { text: "hello", chat: { id: 1234, type: "private" }, from: { id: 1234 } }, me: { username: "bot" } };
    await handler(ctx);
    await handler(ctx);
  ...
  it("passes timeoutSeconds and prefes overrides", async () => {
    ...
    expect(botCtorSpy).toHaveBeenCalledWith("tok", expect.objectContaining({ client: expect.objectContaining({ timeoutSeconds: 62 }) }));
  });

  it("routes callback_query payloads as messages and answers callbacks", async () => {
    createTelegramBot({ token: "tok" });
    const handler = getHandler("callback_query");
    const callbackData = { id: "cbq-1", data: "cmd:a", from: { id: 1234 }, message: { chat: { id: 1234, type: "private" }, message_id: 10 } };
    const ctx = {
  ...
  it("wraps inbound message with Telegram envelope", async () => {
    buildTelegramMessageContextSpy.mockImplementationOnce(async (params) => {
      ...
      return { ctxPayload: { Body: expected, From: "telegram:1234" }, primaryCtx: params.primaryCtx, chatId: 1234, sendTyping: async () => { } };
    });
    createTelegramBot({ token: "tok" });
    const handler = getHandler("message");
    await handler({
      message: { text: "hello world", chat: { id: 1234, type: "private" }, from: { id: 1234, first_name: "Ada Lovelace", username: "ada_bot" }, date: 1736380800 },
      me: { username: "bot" },
  ...
  it("requests pairing by default for unknown DM senders", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getHandler("message");
    await handler({ message: { chat: { id: 888, type: "private" } }, me: { username: "bot" }, getFile: async () => ({}) });
    expect(replySpy).not.toHaveBeenCalled();
    expect(sendMessageSpy).toHaveBeenCalledWith(888, expect.stringContaining("Pairing code: PAIRCODE"));
  });

  it("does not resend pairing code when a request is already pending", async () => {
    upsertTelegramPairingRequest.mockResolvedValueOnce({ code: "PAIRCODE", created: true }).mockResolvedValueOnce({ code: "PAIRCODE", created: false });
    buildTelegramMessageContextSpy.mockImplementation(async (params) => {
  ...
  it("triggers typing cue", async () => {
    createTelegramBot({ token: "tok" });
    const handler = getHandler("message");
    await handler({ message: { chat: { id: 42, type: "private" } }, me: { username: "bot" }, getFile: async () => ({}) });
