...
### 2.2 Registro de Integridade Imutável

Cada execução de ferramenta e transição de estado interno é registrada no **Registro de Integridade**. Este log foi projetado para análise forense pós-incidente e monitoramento de segurança em tempo real.
...
- **Verificação Local**: Utilize `zero security check --deep` para verificar a integridade da sua instalação.
