...
- Realtime: Implementação da `MultimodalSession`, `AudioSessionManager` e `SemanticRouter`.
- Docs: Atualização da "Anatomia do Sistema" no README.md e fluxo de agentes multimodais.
- Build: Correção de tipagens estritas em `src/realtime` e validação integral do CI.
...
- **Revisão de Interface (V5)**: Revisão completa da Interface Altair e Laboratório, garantindo estabilidade na sincronização de tokens e conectividade do Gateway em ambientes de desenvolvimento.
...
- **Mission Control**: Novo painel central de telemetria nativa (tokens/s, latência, detalhamento por modelo) e monitoramento de sistema.
- **Protocolo de Emergência (Panic Button)**: Interrupção global imediata ("Global Halt") via UI e hardware (VAD panic detection).
- **Zero Sentinel (Segurança Nativa)**: Integração da `SecurityEngine` em Rust para detecção de injeção de prompt e redação de PII com performance crítica.
- **VAD Nativo (Rust)**: Migração para `VadEngine` em Rust para detecção de voz com latência ultrabaixa e integração com modo de pânico.
- **Métricas nativas**: Implementação do `MetricsEngine` em Rust para coleta de telemetria de performance em tempo real.
- **Redação de PII Avançada**: Suporte nativo para documentos brasileiros (CPF, CNPJ) e detecção de segredos via análise de entropia em tempo real.

### Changes

- UI: Nova aba "Mission Control" na Interface Altair com design Premium e Glassmorphism.
- Telemetria: Rastreamento automático de tokens e latência em todas as execuções de agentes.
- Core: Integração de módulos Rust via N-API para performance crítica.
- Docs: Atualização completa do README.md e README_EN.md com as novas capacidades Mission Control.
...
- Security check: flag loopback Control UI with auth disabled as critical. (#1795) Thanks @orlyjamie.
...
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara. <https://docs.zero.bot/providers/ollama> <https://docs.zero.bot/providers/venice>
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight. <https://docs.zero.bot/tts>
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj. <https://docs.zero.bot/tools/exec-approvals> <https://docs.zero.bot/tools/slash-commands>
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x. <https://docs.zero.bot/channels/telegram>

### Changes

- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete. <https://docs.zero.bot/tts>
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight. <https://docs.zero.bot/tts>
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x. <https://docs.zero.bot/channels/telegram>
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis. <https://docs.zero.bot/tools/web>
- UI: refresh Control UI dashboard design system (typography, colors, spacing). (#1786) Thanks @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj. <https://docs.zero.bot/tools/exec-approvals> <https://docs.zero.bot/tools/slash-commands>
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @Glucksberg.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override). <https://docs.zero.bot/diagnostics/flags>
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical. <https://docs.zero.bot/bedrock>
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204. <https://docs.zero.bot/channels/bluebubbles>
...
- Signal: add configurable signal-cli startup timeout + external daemon mode docs. (#1677) <https://docs.zero.bot/channels/signal>
...
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg. <https://docs.zero.bot/tts>
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07. <https://docs.zero.bot/gateway/tools-invoke-http-api>
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer. <https://docs.zero.bot/gateway/heartbeat>
- Deploy: add Fly.io deployment support + guide. (#1570) <https://docs.zero.bot/platforms/fly>
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a. <https://docs.zero.bot/channels/tlon>

### Changes

- Channels: allow per-group tool allow/deny policies across built-in + plugin channels. (#1546) Thanks @adam91holt. <https://docs.zero.bot/multi-agent-sandbox-tools>
- Agents: add Bedrock auto-discovery defaults + config overrides. (#1553) Thanks @fal3. <https://docs.zero.bot/bedrock>
- CLI: add `zero system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) <https://docs.zero.bot/cli/system>
- CLI: add live auth probes to `zero models status` for per-profile verification. (commit 40181afde) <https://docs.zero.bot/cli/models>
- CLI: restart the gateway by default after `zero update`; add `--no-restart` to skip it. (commit 2c85b1b40)
- Browser: add node-host proxy auto-routing for remote gateways (configurable per gateway/node). (commit c3cb26f7c)
- Plugins: add optional `llm-task` JSON-only tool for workflows. (#1498) Thanks @vignesh07. <https://docs.zero.bot/tools/llm-task>
...
- Docs: add cron vs heartbeat decision guide (with VOID workflow notes). (#1533) Thanks @JustYannicc. <https://docs.zero.bot/automation/cron-vs-heartbeat>
- Docs: clarify HEARTBEAT.md empty file skips heartbeats, missing file still runs. (#1535) Thanks @JustYannicc. <https://docs.zero.bot/gateway/heartbeat>
...
- Linux: include env-configured user bin roots in systemd PATH and align PATH checks. (#1512) Thanks @robbyczgw-cla.
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `zero models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Providers: add Google Cloud Auth usage tracking to status output. (#1490) Thanks @patelhiren.
...
- Doctor: honor ZERO_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar. <https://docs.zero.bot/cli/agents> <https://docs.zero.bot/web/control-ui>
...
- Highlight: VOID optional plugin tool for typed workflows + approval gates. <https://docs.zero.bot/tools/void>
- VOID: allow workflow file args via `argsJson` in the plugin tool. <https://docs.zero.bot/tools/void>
...
- CLI: flatten node service commands under `zero node` and remove `service node` docs.
- CLI: move gateway service commands under `zero gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `zero update wizard` for interactive channel selection and restart prompts. <https://docs.zero.bot/cli/update>
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start. <https://docs.zero.bot/gateway/troubleshooting>
- Docs: add /model allowlist troubleshooting note. (#1405)
- Docs: add per-message Gmail search example for gog. (#1220) Thanks @mbelinky.

### Breaking

- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth. <https://docs.zero.bot/web/control-ui#insecure-http>
...
- Control UI: add copy-as-markdown with error feedback. (#1345) <https://docs.zero.bot/web/control-ui>
- Control UI: drop the legacy list view. (#1345) <https://docs.zero.bot/web/control-ui>
- TUI: add syntax highlighting for code blocks. (#1200) <https://docs.zero.bot/tui>
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271) <https://docs.zero.bot/tui>
- TUI: add a searchable model picker for quicker model selection. (#1198) <https://docs.zero.bot/tui>
- TUI: add input history (up/down) for submitted messages. (#1348) <https://docs.zero.bot/tui>
- ACP: add `zero acp` for IDE integrations. <https://docs.zero.bot/cli/acp>
- ACP: add `zero acp client` interactive harness for debugging. <https://docs.zero.bot/cli/acp>
- Skills: add download installs with OS-filtered options. <https://docs.zero.bot/tools/skills>
- Skills: add the local sherpa-onnx-tts skill. <https://docs.zero.bot/tools/skills>
- Memory: add hybrid BM25 + vector search (FTS5) with weighted merging and fallback. <https://docs.zero.bot/concepts/memory>
- Memory: add SQLite embedding cache to speed up reindexing and frequent updates. <https://docs.zero.bot/concepts/memory>
- Memory: add OpenAI batch indexing for embeddings when configured. <https://docs.zero.bot/concepts/memory>
- Memory: enable OpenAI batch indexing by default for OpenAI embeddings. <https://docs.zero.bot/concepts/memory>
- Memory: allow parallel OpenAI batch indexing jobs (default concurrency: 2). <https://docs.zero.bot/concepts/memory>
- Memory: render progress immediately, color batch statuses in verbose logs, and poll OpenAI batch status every 2s by default. <https://docs.zero.bot/concepts/memory>
- Memory: add `--verbose` logging for memory status + batch indexing details. <https://docs.zero.bot/concepts/memory>
- Memory: add native Gemini embeddings provider for memory search. (#1151) <https://docs.zero.bot/concepts/memory>
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336) <https://docs.zero.bot/tools/browser>
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323) <https://docs.zero.bot/channels/nostr>
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298) <https://docs.zero.bot/channels/matrix>
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143) <https://docs.zero.bot/channels/slack>
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090) <https://docs.zero.bot/channels/telegram>
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219) <https://docs.zero.bot/plugins/zalouser>
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272) <https://docs.zero.bot/plugins/manifest>
- Plugins: move channel catalog metadata into plugin manifests. (#1290) <https://docs.zero.bot/plugins/manifest>
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290) <https://docs.zero.bot/plugins/manifest>
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306) <https://docs.zero.bot/web/control-ui>
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329) <https://docs.zero.bot/gateway/configuration>
- Plugins: add plugin slots with a dedicated memory slot selector. <https://docs.zero.bot/plugins/agent-tools>
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default). <https://docs.zero.bot/channels/bluebubbles>
- Plugins: migrate bundled messaging extensions to the plugin SDK and resolve plugin-sdk imports in the loader.
- Plugins: migrate the Zalo plugin to the shared plugin SDK runtime. <https://docs.zero.bot/channels/zalo>
- Plugins: migrate the Zalo Personal plugin to the shared plugin SDK runtime. <https://docs.zero.bot/plugins/zalouser>
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide. <https://docs.zero.bot/plugins/agent-tools>
- Plugins: auto-enable bundled channel/provider plugins when configuration is present.
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `zero update`.
- Plugins: share npm plugin update logic between `zero update` and `zero plugins update`.
...
- Usage: add `/usage cost` summaries and macOS menu cost charts. <https://docs.zero.bot/reference/api-usage-costs>
- Security: warn when <=300B models run without sandboxing while web tools are enabled. <https://docs.zero.bot/cli/security>
- Exec: add host/security/ask routing for gateway + node exec. <https://docs.zero.bot/tools/exec>
- Exec: add `/exec` directive for per-session exec defaults (host/security/ask/node). <https://docs.zero.bot/tools/exec>
- Exec approvals: migrate approvals to `~/.zero/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. <https://docs.zero.bot/tools/exec-approvals>
- Nodes: add headless node host (`zero node start`) for `system.run`/`system.which`. <https://docs.zero.bot/cli/node>
- Nodes: add node daemon service install/status/start/stop/restart. <https://docs.zero.bot/cli/node>
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146) <https://docs.zero.bot/concepts/session>
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs. <https://docs.zero.bot/tools/subagents>
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers. <https://docs.zero.bot/concepts/groups>
- Models: add Qwen Portal OAuth provider support. (#1120) <https://docs.zero.bot/providers/qwen>
- Onboarding: add allowlist prompts and username-to-id resolution across core and extension channels. <https://docs.zero.bot/start/onboarding>
- Docs: clarify allowlist input types and onboarding behavior for messaging channels. <https://docs.zero.bot/start/onboarding>
- Docs: refresh Android node discovery docs for the Gateway WS service type. <https://docs.zero.bot/platforms/android>
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289) <https://docs.zero.bot/bedrock>
- Docs: clarify WhatsApp voice notes. <https://docs.zero.bot/channels/whatsapp>
- Docs: clarify Windows WSL portproxy LAN access notes. <https://docs.zero.bot/platforms/windows>
- Docs: refresh bird skill install metadata and usage notes. (#1302) <https://docs.zero.bot/tools/browser-login>
...
- Build: use tsgo for dev/watch builds by default (opt out with `ZERO_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `zero doctor --fix` to repair, then update plugins (`zero plugins update`) if you use any.

### Fixes

- Discovery: shorten Bonjour DNS-SD service type to `_zero-gw._tcp` and update discovery clients/docs.
...
- CLI: keep `zero logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `zero memory status`.
...
- Memory: probe sqlite-vec availability in `zero memory status`.
...
- Hooks: add hooks system with bundled hooks, CLI tooling, and docs. (#1028) — thanks @ThomsenDrake. <https://docs.zero.bot/hooks>
...
- **BREAKING:** `zero message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `zero hooks` is now `zero webhooks`; hooks live under `zero hooks`. <https://docs.zero.bot/cli/webhooks>
- **BREAKING:** `zero plugins install <path>` now copies into `~/.zero/extensions` (use `--link` to keep path-based loading).

### Changes

- Plugins: ship bundled plugins disabled by default and allow overrides by installed versions. (#1066) — thanks @ItzR3NO.
- Plugins: add bundled Google Cloud Auth + Gemini CLI OAuth + Copilot Proxy provider plugins. (#1066) — thanks @ItzR3NO.
- Tools: improve `web_fetch` extraction using Readability (with fallback).
- Tools: add Firecrawl fallback for `web_fetch` when configured.
- Tools: send Chrome-like headers by default for `web_fetch` to improve extraction on bot-sensitive sites.
- Tools: Firecrawl fallback now uses bot-circumvention + cache by default; remove basic HTML fallback when extraction fails.
- Tools: default `exec` exit notifications and auto-migrate legacy `tools.bash` to `tools.exec`.
- Tools: add `exec` PTY support for interactive sessions. <https://docs.zero.bot/tools/exec>
...
- Directory: unify `zero directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `zero.hooks` manifests and `zero hooks install/update`.
...
- Security: default-deny slash/control commands unless a channel computed `CommandAuthorized` (fixes accidental “open” behavior), and ensure WhatsApp + Zalo plugin channels gate inline `/…` tokens correctly. <https://docs.zero.bot/gateway/security>
- Security: redact sensitive text in gateway WS logs.
- Tools: cap pending `exec` process output to avoid unbounded buffers.
- CLI: speed up `zero sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- Plugins: add provider auth registry + `zero models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@zero/msteams` via `zero plugins install @zero/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes

- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `zero-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `zero security audit`.
- Apps: store node auth tokens encrypted (Keychain/SecurePrefs).
- Daemon: share profile/state-dir resolution across service helpers and honor `ZERO_STATE_DIR` for Windows task scripts.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `zero models status`, and update docs.
- CLI: add `--json` output for `zero daemon` lifecycle/install commands.
...
- Fix: make `zero update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `zero configure`, even if no other sections are chosen.
- Daemon: fix profile-aware service label resolution (env-driven) and add coverage for launchd/systemd/schtasks. (#969) — thanks @bjesuiter.
- Agents: avoid false positives when logging unsupported Google tool schema keywords.
- Agents: skip Gemini history downgrades for google-cloud-auth to preserve tool calls. (#894) — thanks @mukhtharcm.
...
- Browser control: Chrome extension relay takeover mode + remote browser control via `zero browser serve`.
- Plugins: channel plugins (gateway HTTP hooks) + Zalo plugin + onboarding install flow. (#854) — thanks @longmaba.
- Security: expanded `zero security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `zero security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `zero browser extension install/path` and remote browser control via `zero browser serve` + `browser.controlToken`.
...
- Agents: harden Google Cloud Auth Claude history/tool-call sanitization. (#968) — thanks @rdev.
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.zero-dev`).
...
- Memory: new `zero memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.zero/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `zero dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
- Install: run `zero doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Onboarding/Auth: honor `ZERO_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) — thanks @roshanasingh4.
...
- Gateway/CLI: honor `ZERO_LAUNCHD_LABEL` / `ZERO_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `zero doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `zero plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `zero` CLI when missing in local mode; install via `zero.bot/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch zero.bot scripts and run install sh/cli with onboarding skipped.
- Installer UX: support `ZERO_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `zero.extensions`; load plugin packages from config dirs; extract archives without system tar.
- Config: expand `~` in `ZERO_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) — thanks @pasogott.
...
- CLI: `zero status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `zero update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `zero reset` and `zero uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `zero gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `zero status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `zero sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: respect `ZERO_STATE_DIR` for node pairing + voice wake settings storage. (#664) — thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`ZERO_LIVE_GATEWAY_IMAGE_PROBE=1`).
- CLI: `zero sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize user-facing “ZeroBot”/“ZERO” → “ZERO” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `zero update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/zero`. (#660)
...
- CLI: `zero message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `zero models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- CLI UX: logs output modes (pretty/plain/JSONL) + colorized health/daemon output; global `--no-color`; void palette in onboarding/config.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.zero/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- Gateway/Daemon/Doctor: atomic config writes; repair gateway service entrypoint + install switches; non-interactive legacy migrations; systemd unit alignment + KillMode=process; node bridge keepalive/pings; Launch at Login persistence; bundle ZEROKit resources + Swift 6.2 compat dylib; relay version check + remove smoke test; regen Swift GatewayModels + keep agent provider string; cron jobId alias + channel alias migration + main session key normalization; heartbeat Telegram accountId resolution; avoid WhatsApp fallback for internal runs; gateway listener error wording; serveBaseUrl param; honor gateway --dev; fix wide-area discovery updates; align agents.defaults schema; provider account metadata in daemon status; refresh Carbon patch for gateway fixes; restore doctor prompter initialValue handling.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `zero pairing list <provider>` + `zero pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `zero logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- **Docs:** new FAQ/ZeroHub/config examples/showcase entries and clarified auth, sandbox, and systemd docs.
...
- Env: load global `$ZERO_STATE_DIR/.env` (`~/.zero/.env`) as a fallback after CWD `.env`.
...
- Commands: unify /status (inline) and command auth across providers; group bypass for authorized control commands; remove Discord /zero slash handler.
- CLI: run `zero agent` via the Gateway by default; use `--local` to force embedded mode.
