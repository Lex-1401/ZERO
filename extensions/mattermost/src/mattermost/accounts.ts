import type { ZeroConfig } from "zero/plugin-sdk";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "zero/plugin-sdk";
...
function listConfiguredAccountIds(cfg: ZeroConfig): string[] {
  const accounts = cfg.channels?.mattermost?.accounts;
  if (!accounts || typeof accounts !== "object") return [];
  return Object.keys(accounts).filter(Boolean);
}

export function listMattermostAccountIds(cfg: ZeroConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: ZeroConfig): string {
...
function mergeMattermostAccountConfig(cfg: ZeroConfig, accountId: string): MattermostAccountConfig {
...
export function listEnabledMattermostAccounts(cfg: ZeroConfig): ResolvedMattermostAccount[] {
