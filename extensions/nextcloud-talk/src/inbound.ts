...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ZeroConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ZeroConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ZeroConfig);
