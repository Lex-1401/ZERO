...
Zero extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
zero message send --channel zalouser --target <threadId> --message "Hello from Zero!"
```
...
```bash
zero channels login --channel zalouser              # Login via QR
zero channels login --channel zalouser --account work
zero channels status --probe
zero channels logout --channel zalouser
```
...
zero directory peers list --channel zalouser --query "name"
zero directory groups list --channel zalouser --query "work"
...
```bash
# Text
zero message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
zero message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `zero channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
zero channels login --channel zalouser --account work
zero message send --channel zalouser --account work --target <id> --message "Hello"
