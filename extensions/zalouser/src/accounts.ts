...
export function listZalouserAccountIds(cfg: ZeroConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZalouserAccountId(cfg: ZeroConfig): string {
...
function mergeZalouserAccountConfig(cfg: ZeroConfig, accountId: string): ZalouserAccountConfig {
