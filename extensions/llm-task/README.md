...
Designed to be called from workflow engines (for example, Zero via
`zero.invoke --each`) without adding new Zero code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Zero) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on Zero internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Zero extension (like `void`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.zero/extensions` as a standalone plugin directory.
