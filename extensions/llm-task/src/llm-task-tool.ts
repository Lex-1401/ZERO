...
// NOTE: This extension is intended to be bundled with Zero.
// When running from source (tests/dev), Zero internals live under src/.
...
export function createLlmTaskTool(api: ZeroPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via zero.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero-llm-task-"));
