...
function resolveZeroRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.ZERO_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    const found = findPackageRoot(start, "zero");
    if (found) {
      coreRootCache = found;
      return found;
    }
  ...
  throw new Error("Unable to resolve Zero root. Set ZERO_ROOT to the package root.");
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveZeroRoot();
