...
const DEFAULT_MODEL = "google-cloud-auth/claude-opus-4-5-thinking";
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Zero Google Cloud Auth</title>
  </head>
  <body>
    <main>
...
async function loginGoogleCloudAuth(params: {
  isRemote: boolean;
  ...
  params.progress.stop("Google Cloud Auth OAuth complete");
...
const googleCloudAuthPlugin = {
  ...
  description: "OAuth flow for Google Cloud Auth (Cloud Code Assist)",
      ...
      auth: [
        {
          ...
          kind: "oauth",
          run: async (ctx) => {
            const spin = ctx.prompter.progress("Starting Google Cloud Auth OAuth…");
            try {
              const result = await loginGoogleCloudAuth({
              ...
              const profileId = `google-cloud-auth:${result.email ?? "default"}`;
              return {
                profiles: [
                  {
                    profileId,
                ...
                notes: [
                  "GoogleCloudAuth uses Google Cloud project quotas.",
                  "Enable Gemini for Google Cloud on your project if requests fail.",
                ],
              };
            } catch (err) {
              spin.stop("Google Cloud Auth OAuth failed");
              throw err;
            }
          },
        },
...
export default googleCloudAuthPlugin;
