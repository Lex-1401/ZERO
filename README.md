...
7. **Doc-to-LoRA (D2L) - Internalização Instantânea**:
    - **Context Internalization**: Transformação de documentos longos em adapters LoRA dinâmicos em milissegundos.
    - **Performance Exponencial**: Memória agêntica que expande a janela de contexto em até 4x sem sobrecarregar o KV-cache.
    - **Soul-to-LoRA**: O `SOUL.md` é internalizado matematicamente no modelo, garantindo que a personalidade seja parte do "DNA" da inferência.
8. **Graph-RAG & Camada Semântica**:
    - **Knowledge Graph Integrado**: Extração automática de entidades e relacionamentos para memória de longo prazo.
    - **Contextual Retrieval**: Injeção dinâmica de fatos do grafo durante a busca, superando as limitações do RAG tradicional.
9. **Navegação de Sessão em Árvore**:
    - **Conversational Branching**: Use `/fork` para criar ramificações de contexto e explorar diferentes caminhos de raciocínio.
    - **Prompt Minimalista**: Refatoração estrutural para reduzir o overhead de tokens em 30%.
10. **TUI Master & Differential Rendering**:
    - **Zero-Flicker UI**: Renderização diferencial para uma experiência de terminal ultra-suave.
    - **MCP Host (Alpha)**: Suporte inicial ao Model Context Protocol para integração com ecossistemas externos de ferramentas.
...
> _Honramos o código que veio antes (Peter Steinberger & Comunidade), enquanto construímos o futuro que precisamos e aspiramos agora._
...
Se você é um usuário comum, não precisa se preocupar com a engenharia pesada (Rust, WebSockets, Vectors). O ZERO foi projetado para abstrair essa complexidade brutal em uma interface fluida que _simplesmente funciona_.
...
> _Não podemos abstrair a complexidade, mas podemos torná-la invisível._
...
1. **Soberania de Comunicação**:
   ...
   - _Exemplo_: _"Resuma todas as mensagens de trabalho das últimas 2 horas e me diga apenas o que requer ação imediata."_
2. **Memória Pessoal Infinita (Local RAG)**:
   - Indexa seus arquivos locais (PDFs, Docs, Código) sem enviá-los para a nuvem.
   - _Exemplo_: _"Encontrar aquele contrato que assinei em 2023 sobre 'prestação de serviços' e me diga a cláusula de rescisão."_
3. **Execução Real de Tarefas (Agêntico)**:
   - Ele não apenas "fala", ele **faz**. Agenda reuniões, envia e-mails, controla o terminal.
   - _Exemplo_: _"Verifique meu calendário, cancele a reunião das 15h e avise o time no Slack que estou focado no deploy."_
4. **Coding Autônomo**:
   - Atua como um Engenheiro de Software Sênior que conhece todo o seu codebase local.
   - _Exemplo_: _"Analise os logs de erro do projeto X e proponha uma correção para o memory leak."_
...
_Esquema do Córtex Agêntico: Integração entre a Rust Engine e a Interface Altair._
...
1. **Soberania Local-First (O "Google" Pessoal Ético)**:
   - _Potencial de Escala Exponencial_: O Google escala construindo Datacenters massivos; o **ZERO** pretende escalar utilizando o hardware ocioso de bilhões de dispositivos pessoais.
   - _Visão_: O Google organizou a web pública; o ZERO organiza sua vida privada (Arquivos, Chats, Compromissos, Finanças) usando `sqlite-vec` localmente.
   - _Índice de Vida Ético_: Indexamos sua existência digital para _você_, e apenas para você. Diferente da nuvem, onde "escalar" significa "mais vigilância", aqui escalar a inteligência não custa sua privacidade.
...
4. **Arquitetura ClearCode**: Rigor técnico com limites de complexidade impostos (máximo de 500 linhas por arquivo). Sistema 100% modularizado sob padrões de doutorado para máxima resiliência e manutenibilidade.
...
> _Se o ZERO é o sistema operacional de inteligência, o Altair é o monitor e painel de controle que torna essa inteligência tangível e operável._
...
| :-------------------------------------------------------------- | :----------------------------------------------- |
| ![Chat UI](assets/snapshots/chat-altair.png)                    | ![Marketplace](assets/snapshots/marketplace.png) |
| _Interface de chat unificada com streaming de alta fidelidade._ | _Ecossistema de Skills e Marketplace local._     |

| Configurações de Aparência                              | Memória Procedural (RAG)                         |
| :------------------------------------------------------ | :----------------------------------------------- |
| ![Settings](assets/snapshots/settings-appearance.png)   | ![Memory](assets/snapshots/brain-memory.png)     |
| _Controle granular de temas e estética Quantum Altair._ | _Indexação local e gestão de contexto agêntico._ |

| Catálogo de Skills (Marketplace)                                                                    |
| :-------------------------------------------------------------------------------------------------- |
| ![Skills Catalog](assets/screenshots/skills-ui-v4.png)                                              |
| _Extensões inteligentes agrupadas por compatibilidade, com diagnóstico automático de dependências._ |
...
- **Defina a Persona**: _"Você é o Jarvis, um mordomo sarcástico."_ ou _"Você é o TARS, focado em precisão técnica."_
...
> _O ZERO lê sua alma a cada reinicialização e a incorpora no nível mais profundo do sistema através do motor **Doc-to-LoRA (D2L)**, gerando um adapter de personalidade instantâneo._
...
_(O script fará tudo: instalar dependências, configurar o Rust e iniciar o assistente de onboarding)_.
...
2. **Orquestração Inicial**:
   ...
   _💡 Se o comando acima falhar com "command not found", certifique-se que o pnpm está configurado corretamente (`pnpm setup`) ou prefira usar `pnpm run zero onboard`._

   _Isso iniciará o assistente de configuração para preparar sua "Origem" (diretório Home), chaves de API e canais de mensageria._
...
2. **Regex Nativa (Rust `regex` crate)**:
   - Compilação AOT (Ahead-Of-Time) de padrões complexos para números sensíveis (CPF, CNPJ, Cartões de Crédito). O custo de sanitizar 1MB de texto é desprezível, permitindo que _tudo_ seja validado em tempo real sem "lag" na conversa.
3. **Trade-off de Autonomia vs Inteligência Coletiva**:
   ...
   - **Modelo Mental**: Usamos o "conhecimento coletivo congelado" (o LLM pré-treinado) e o especializamos com o "contexto soberano vivo" (seu RAG local). Você não precisa enviar seus dados para treinar a IA dos outros; a IA vem treinada para servir aos _seus_ dados.

> _Segurança não é uma feature. É o estado padrão._
...
_ZERO é uma ferramenta de precisão. Use-a com intenção._
_Desenvolvido pelo Time de Engenharia ZERO. A soberania é inegociável._
